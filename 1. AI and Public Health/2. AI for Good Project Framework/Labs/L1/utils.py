...
def create_histogram_plot(df: pd.core.frame.DataFrame, bins: int):
    ...
    def _interactive_histogram_plot(station, pollutant):
        ...
        try:
            ...
            plt.title(
                f"Pollutant: {pollutant} - Station: {station}", fontsize=FONT_SIZE_TITLE
            )
...
def create_boxplot(df: pd.core.frame.DataFrame):
    ...
    def _interactive_boxplot(cat_var):
        ...
        plt.yscale(scale)
        sns.boxplot(
            data=df, y=cat_var, x="Station", order=labels[orderInd], color="seagreen"
        )
...
def create_scatterplot(df: pd.core.frame.DataFrame):
    ...
    cont_y_widget = widgets.Dropdown(
        options=pollutants_list, description="Y-Axis", value="PM10"
    )
...
def create_time_series_plot(
    df: pd.core.frame.DataFrame, start_date: str, end_date: str
):
...
def add_extra_features(df: pd.core.frame.DataFrame) -> pd.core.frame.DataFrame:
    ...
    stations = stations.rename(
        columns={"Sigla": "Station", "Latitud": "Latitude", "Longitud": "Longitude"}
    )
...
def create_map_with_plots(
    full_data: pd.core.frame.DataFrame, x_variable: str, y_variable: str = "PM2.5"
) -> folium.Map:
    """
    ...
    for lt, ln, pol, station in data_grouped_grid_array:
        fg.add_child(
            folium.CircleMarker(
                location=[lt, ln],
                radius=15,
                popup=f"<img src='img/tmp/{station}.png'>",
