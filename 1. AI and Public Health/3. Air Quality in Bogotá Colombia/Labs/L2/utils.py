...
def predict_on_bogota(
    model: KNeighborsRegressor, n_points: int = 64
...
def calculate_mae_for_k(
    data: pd.core.frame.DataFrame, k: int = 1, target_pollutant: str = "PM2.5"
) -> float:
    ...
    df2.insert(
        0,
        "time_discriminator",
        (df2["DateTime"].dt.dayofyear * 10000 + df2["DateTime"].dt.hour * 100).values,
        True,
    )
        ...
        if len(df_day_station) > 0:
            ...
            if len(df_day_no_station) >= k:
                neigh = KNeighborsRegressor(
                    n_neighbors=k, weights="distance", metric="sqeuclidean"
                )
                knn_model = neigh.fit(
                ...
                prediction = knn_model.predict(
                    df_day_station[["Latitude", "Longitude", "time_discriminator"]]
                )
                if len(predictions) == 0:
                    predictions = np.array(
                        [df_day_station[target_pollutant].values, prediction[:, 0]]
                    ).T
                else:
...
def create_heat_map_with_date_range(
    ...
    k = k_neighbors
    neigh = KNeighborsRegressor(
        n_neighbors=k, weights="distance", metric=distance_metric
    )
...
def create_animation_features(
    ...
    for timestamp in unique_dates:
        ...
        k_neighbors_model.fit(
            df_day[["Latitude", "Longitude"]], df_day[[target_pollutant]]
        )
...
def color_producer(pollutant_type, pollutant_value):
    ...
    for threshold in thresholds:
        if pollutant_value < threshold:
            ...
            colors = list(
                Color(colors_dict[previous]).range_to(Color(colors_dict[threshold]), 11)
            )
...
def create_polygon(
    p: List[float], dlat: float, dlon: float, time: datetime, pollutant: str
) -> Dict[str, Any]:
