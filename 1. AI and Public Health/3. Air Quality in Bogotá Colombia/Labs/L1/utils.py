...
def create_time_series_plot(
    df: pd.core.frame.DataFrame, start_date: datetime, end_date: datetime
):
    ...
    def _interactive_time_series_plot(station, date_range, target):
        ...
        if f"{target}_imputed_flag" in data:
            ...
            imputed_data.loc[
                imputed_data[f"{target}_imputed_flag"].isnull(), target
            ] = None
            original_data = data[["DateTime", target, f"{target}_imputed_flag"]]
            original_data.loc[
                imputed_data[f"{target}_imputed_flag"].notnull(), target
            ] = None
            plt.plot(
                imputed_data["DateTime"], imputed_data[target], "r-", label="Imputed"
            )
            plt.plot(
                original_data["DateTime"], original_data[target], "-", label="Real"
            )
            plt.legend()
        else:
            # Plot the data
    ...
    station_selection = widgets.Dropdown(
        options=df.Station.unique(), description="Station"
    )

    target_pollutant_selection = widgets.Dropdown(
        options=pollutants_list, description="Pollutant", value="PM2.5"
    )
...
def visualize_missing_values_estimation(df: pd.core.frame.DataFrame, day: datetime):
    ...
    def draw(sample, station, missing_index, target):
        ...
        sample.insert(
            ...
            "time_discriminator",
            (
                sample["DateTime"].dt.dayofyear * 100000
                + sample["DateTime"].dt.hour * 100
            ).values,
            True,
        )
        ...
        to_nan = sample_copy.apply(
            lambda row: str(row["DateTime"]) in dates and row["Station"] == station,
            axis=1,
        )
        sample_copy.loc[to_nan, target] = float("NaN")
        imputer = KNNImputer(n_neighbors=1)
        imputer.fit(
            sample_copy[["time_discriminator", "Latitude", "Longitude", target]]
        )
    ...
    station_selection = widgets.Dropdown(
        options=df.Station.unique(), description="Station", value="USM"
    )
    target_pollutant_selection = widgets.Dropdown(
        options=pollutants_list, description="Pollutant", value="PM2.5"
    )
...
def calculate_mae_for_nearest_station(
    df: pd.core.frame.DataFrame, target: str
) -> Dict[str, float]:
...
def create_plot_with_preditions(
    ...
    def draw_example3(sample, station, predicted2, missing_index):
        ...
        plt.plot(
            missing_before_after,
            real_data.loc[missing_before_after][target],
            "r--o",
            label="actual values",
        )
        ...
        imputer = KNNImputer(n_neighbors=1)
        imputer.fit(
            copy_of_data[["time_discriminator", "Latitude", "Longitude", target]]
        )
        ...
        plt.plot(
            missing_before_after,
            example1.loc[missing_before_after][f"new_{target}"],
            "g--o",
            label="nearest neighbor",
        )
        plt.plot(example1.index, example1[target], "-*")
        ...
        plt.plot(
            missing_before_after,
            example1.loc[missing_before_after][f"nn_{target}"],
            "y--*",
    ...
    def plot_predictions(station, size, start_index):
        try:
            ...
            plt.figure(figsize=(10, 5))
            draw_example3(
                data, station, y_predicted, list(range(start_index, start_index + size))
            )
            plt.show()
        except Exception as e:
            print(
                "The selected range cannot be plotted due to missing values. Please select other values.\n"
            )
            print(e)
    ...
    windows_size_selection = widgets.Dropdown(
        options=list([1, 2, 3, 5, 6, 12, 24]), description="Window"
    )
    # Widget for selecting index of data
    index_selector = widgets.IntSlider(
        value=1, min=1, max=48, step=1, description="Index"
    )

    interact(
        plot_predictions,
        station=station_selection,
        size=windows_size_selection,
        start_index=index_selector,
    )
...
def impute_nontarget_missing_values_interpolate(
    ...
    for pollutant in pollutants_except_target:
        ...
        if np.any(
            df_with_missing.loc[[df_with_missing.index[0]], [pollutant]].isnull()
        ):
...
def impute_target_missing_values_neural_network(
    ...
    baseline_imputed_data_and_flags = baseline_imputed[
        [
            ...
            if i in pollutants_list or "flag" in i
        ]
    ]
    ...
    data_with_imputed = (
        data_with_imputed[metadata_columns + [target]]
        .join(imputed_flag)
        .join(baseline_imputed_data_and_flags)
    )
