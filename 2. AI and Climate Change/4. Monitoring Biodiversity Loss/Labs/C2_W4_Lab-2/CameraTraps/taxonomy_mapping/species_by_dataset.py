...
def query_species_by_dataset(megadb_utils: MegadbUtils, output_dir: str) -> None:
    ...
    queried_datasets = set(
        i.split(".json")[0] for i in os.listdir(output_dir) if i.endswith(".json")
    )
...
def get_example_images(
    megadb_utils: MegadbUtils, dataset_name: str, class_name: str
) -> List[Optional[str]]:
        ...
        for i_row, cell in enumerate(ws_col):
            if i_row > 0 and cell.value is not None:
                if not isinstance(cell.value, str):
                    print(
                        f"WARNING cell.value is {cell.value}, "
