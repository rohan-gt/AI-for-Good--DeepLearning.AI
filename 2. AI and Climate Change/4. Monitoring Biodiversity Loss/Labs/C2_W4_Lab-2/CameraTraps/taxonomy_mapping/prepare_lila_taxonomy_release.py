...
lila_taxonomy_file = os.path.expanduser(
    "~/git/agentmorrisprivate/lila-taxonomy/lila-taxonomy-mapping.csv"
)
release_taxonomy_file = os.path.expanduser(
    "~/lila/lila-taxonomy-mapping_release.22.08.22.0000.csv"
)

# Created by get_lila_category_list.py... contains counts for each category
lila_dataset_to_categories_file = os.path.expanduser(
    "~/lila/lila_categories_list/lila_dataset_to_categories.json"
)
...
for s in levels_used:
    assert s in levels_to_exclude or s in levels_to_include, (
        "Unrecognized level {}".format(s)
    )
