...
from typing import ClassVar, Container, Dict, Iterable, List, Optional, Set, Tuple
...
class TaxonNode:
    ...
    def __init__(self, level: str, name: str, graph: Optional[nx.DiGraph] = None):
    ...
    def get_dataset_labels(
        self, include_datasets: Optional[Container[str]] = None
    ) -> Set[Tuple[str, str]]:
    ...
    @classmethod
    def lowest_common_ancestor(cls, nodes: Iterable[TaxonNode]) -> Optional[TaxonNode]:
...
def build_taxonomy_graph(
    ...
    nx.DiGraph, Dict[Tuple[str, str], TaxonNode], Dict[Tuple[str, str], TaxonNode]
]:
    ...
    for _, row in taxonomy_df.iterrows():
        ...
        for i, taxon in enumerate(taxa_ancestry):
            ...
            if key not in taxon_to_node:
                taxon_to_node[key] = TaxonNode(
                    level=taxon_level, name=taxon_name, graph=graph
                )
...
def dag_to_tree(
    graph: nx.DiGraph, taxon_to_node: Dict[Tuple[str, str], TaxonNode]
) -> nx.DiGraph:
