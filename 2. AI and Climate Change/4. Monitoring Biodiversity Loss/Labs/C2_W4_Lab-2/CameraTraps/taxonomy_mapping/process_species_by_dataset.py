...
master_table_file = (
    r"C:\git\ai4edev\camera-traps-private\camera_trap_taxonomy_mapping.csv"
)
...
for i_row, row in df.iterrows():
    ...
    # on another.
    if (isinstance(taxa_ancestry, str) and taxa_ancestry == "") or (
        pd.isna(taxa_ancestry)
    ):
        continue
    ...
    if row["taxonomy_level"] != taxon_level:
        ...
        print(
            f"- taxonomy_level column: {row['taxonomy_level']}, "
    ...
    if row["scientific_name"] != taxon_name:
        print(f"row: {i_row}, {row['dataset_name']}, {row['query']}")
        print(
            f"- scientific_name column: {row['scientific_name']}, "
...
for i_row, row in df.iterrows():
    ...
    if False:
        if row.source == "manual" and row.taxonomy_string != "":
            print(
                "Warning: manual row with taxonomy string: {}.{}".format(
    ...
    if row.source != "" and len(row.scientific_name) == 0:
        print(
            "Unsourced row with no scientific name: {}.{}".format(
                row.dataset_name, row.query
            )
        )
    ...
    if row.source == "manual":
        ...
        if common_name == "":
            print(
                "Warning: manual row with no common name: {}.{}".format(
                    row.dataset_name, row.query
                )
            )
