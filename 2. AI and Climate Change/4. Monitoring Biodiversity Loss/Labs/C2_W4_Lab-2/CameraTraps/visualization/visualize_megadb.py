...
def visualize_sequences(datasets_table, sequences, args):
    ...
    for seq in sequences:
        ...
        images_in_seq = (
            sorted(seq["images"], key=lambda x: x["frame_num"])
            if len(seq["images"]) > 1
            else seq["images"]
        )
        ...
        for im in images_in_seq:
            ...
            ).replace("\\", args.pathsep_replacement)
            ...
            images_html.append(
                {
                    ...
                    "title": "Seq ID: {}. Frame number: {}<br/> Image file: {}<br/> number of boxes: {}, image class labels: {}".format(
                        seq_id, frame_num, blob_path, len(rendering["bbox"]), im_class
                    ),
...
def main():
    ...
    parser.add_argument(
        "megadb_entries", type=str, help="Path to a json list of MegaDB entries"
    )
    parser.add_argument(
        "output_dir",
        action="store",
