...
def image_filename_to_path(image_file_name, image_base_dir, pathsep_replacement=""):
    if len(pathsep_replacement) > 0:
        image_file_name = os.path.normpath(image_file_name).replace(
            os.pathsep, pathsep_replacement
        )
...
def process_images(db_path, output_dir, image_base_dir, options=None):
    ...
    assert not (
        (options.classes_to_exclude is not None)
        and (options.classes_to_include is not None)
    ), "Cannot specify an inclusion and exclusion list"

    if (options.classes_to_exclude is not None) or (
    ...
    if options.num_to_visualize is not None:
        df_img = df_img.sample(
            n=options.num_to_visualize, random_state=options.random_seed
        )
    ...
    for iImage in tqdm(range(len(df_img))):
        ...
        for iAnn, anno in annos_i.iterrows():
            ...
            if options.add_search_links:
                ...
                categoryName = (
                    '<a href="https://www.bing.com/images/search?q={}">{}</a>'.format(
        ...
        if "frame_num" in img and "seq_num_frames" in img:
            frameString = " frame: {} of {}, ".format(
                img["frame_num"], img["seq_num_frames"]
            )
        else:
        ...
        try:
            ...
            if options.viz_size[0] == -1 and options.viz_size[1] == -1:
                ...
                image = vis_utils.resize_image(
                    original_image, options.viz_size[0], options.viz_size[1]
                )
        except Exception as e:
            print("Image {} failed to open. Error: {}".format(img_path, e))
            return False

        vis_utils.render_db_bounding_boxes(
            boxes=bboxes,
            classes=bboxClasses,
            image=image,
    ...
    if options.parallelize_rendering:
        ...
        rendering_success = tqdm(
            list(pool.imap(render_image_info, rendering_info)),
    ...
    print(
        "Rendered {} images in {} ({} successful)".format(
            len(rendering_info),
            humanfriendly.format_timespan(elapsed),
            sum(rendering_success),
        )
    )
    ...
    write_html_image_list(
        filename=htmlOutputFile, images=images_html, options=htmlOptions
    )

    print(
        "Visualized {} images, wrote results to {}".format(
...
def main():
    ...
    parser.add_argument(
        "db_path", action="store", type=str, help=".json file to visualize"
    )
    parser.add_argument(
        "output_dir",
        action="store",
