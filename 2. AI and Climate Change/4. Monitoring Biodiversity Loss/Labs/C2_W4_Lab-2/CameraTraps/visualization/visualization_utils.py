    ...
    for detection in detections:
        ...
        if score >= confidence_threshold:
            ...
            if "classifications" in detection:
                ...
                clss = annotation_constants.NUM_DETECTOR_CATEGORIES + int(
                ...
                for classification in classifications:
                    ...
                    if (classification_label_map is not None) and (
                        class_key in classification_label_map
                    ):
                        class_name = classification_label_map[class_key]
                    else:
                        class_name = class_key
                    displayed_label += [
                        "{}: {:5.1%}".format(class_name.lower(), classification[1])
                    ]
...
def render_iMerit_boxes(
    ...
    display_boxes = np.array(display_boxes)
    draw_bounding_boxes_on_image(
        image, display_boxes, classes, display_strs=display_strs
    )
...
def render_megadb_bounding_boxes(boxes_info, image):
    ...
    for b in boxes_info:
        ...
        classes.append(
            annotation_constants.detector_bbox_category_name_to_id[b["category"]]
        )
...
def render_db_bounding_boxes(
    boxes, classes, image, original_size=None, label_map=None, thickness=4, expansion=0
):
    """
