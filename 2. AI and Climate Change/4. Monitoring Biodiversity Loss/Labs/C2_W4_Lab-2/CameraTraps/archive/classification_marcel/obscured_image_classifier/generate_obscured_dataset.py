...
images = sorted(
    list(glob.glob("/data2/idfg/obscured_dataset/coco-style/obscured_images/*JPG"))
)
images = images[: len(images) // 2]
labels = [1] * len(images)
images += list(
    glob.glob(
        "/data2/idfg/obscured_dataset/coco-style/sampled_clean_train_images/**/*JPG",
        recursive=True,
    )
)
...
j = dict(
    ...
    annotations=[],
    categories=[
        dict(id=0, name="clean", supercategory="e"),
...
images += list(
    glob.glob(
        "/data2/idfg/obscured_dataset/coco-style/sampled_clean_test_images_large/**/*JPG",
        recursive=True,
    )
)
labels += [0] * (len(images) - len(labels))
