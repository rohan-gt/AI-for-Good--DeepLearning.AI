...
def _compute_precision_recall(per_image_detections, per_image_gts, num_gt_classes):
    ...
    detection_tp_fp = defaultdict(
        list
    )  # key is the category; in each list, 1 is tp, 0 is fp
    ...
    for i in range(num_gt_classes):
        ...
        print(
            "Number of ground truth in category {} is {}".format(cat, num_total_gt[cat])
        )
...
def compute_precision_recall(results, exclude_person_images):
    ...
    per_image_detections, per_image_gts = utils.group_detections_by_image(
        results, use_im
    )
    print("Length of per_image_detections:", len(per_image_detections))

    per_cat_metrics = _compute_precision_recall(
        per_image_detections, per_image_gts, num_gt_classes
    )
    ...
    mAP_from_cats = (
        sum(
            [
                v["average_precision"]
                if k != "one_class" and not math.isnan(v["average_precision"])
                else 0
    ...
    print(
        "mAP as the average of AP across the {} categories is {:.4f}".format(
            num_gt_classes, mAP_from_cats
        )
    )

    fig = plt.figure(
        figsize=(PLOT_WIDTH + 0.2, PLOT_WIDTH * len(per_cat_metrics))
    )  # (width, height) in inches

    per_cat_metrics = sorted(
        per_cat_metrics.items(), key=lambda x: abs(hash(x[0]))
    )  #  cast 'one_class' as int

    for i, (cat, cat_metrics) in enumerate(per_cat_metrics):
        ax = fig.add_subplot(num_gt_classes + 1, 1, i + 1)  # nrows, ncols, and index
        ax.set_aspect("equal")
        ax.set_title(
            "Category {}, AP {:.4f}".format(cat, cat_metrics["average_precision"])
        )
...
def main():
    ...
    per_cat_metrics, fig_precision_recall = compute_precision_recall(
        p, args.exclude_person_images
    )

    fig_precision_recall.savefig(os.path.join(args.out_dir, "precicision_recall.png"))
    pickle.dump(
        per_cat_metrics,
        open(os.path.join(args.out_dir, "per_category_metrics.p"), "wb"),
    )
