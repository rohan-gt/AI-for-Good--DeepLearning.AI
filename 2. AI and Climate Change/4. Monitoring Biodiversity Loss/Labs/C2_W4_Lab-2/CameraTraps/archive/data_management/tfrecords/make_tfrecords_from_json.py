...
if False:
    ...
    input_json_file = os.path.join(
        baseDir, "imerit_batch7_renamed_uncorrupted_filtered_oneclass.json"
    )
    output_tfrecords_folder = os.path.join(
        baseDir, "imerit_batch7_renamed_uncorrupted_filtered_oneclass_tfrecords"
    )
    ...
    tfr_images = make_tfrecords_from_json(
        input_json_file,
        output_tfrecords_folder,
        image_file_root,
        dataset_name,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Make tfrecords from a CCT style json file"
    )
