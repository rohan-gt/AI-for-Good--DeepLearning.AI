...
tfrecords_format_json_path = (
    "/home/yasiyu/yasiyu_temp/eMammal_db/eMammal_20180929_tfrecord_format.json"
)
...
output_path = (
    "/home/yasiyu/yasiyu_temp/tf_records_eMammal/eMammal_loc_splits_20180929.json"
)
...
if split_by == "location":
    print(
        "Previously had data for {} locations. Train {}, val {}, test {}.".format(
    ...
    new_locs = defaultdict(
        int
    )  # new location ID : how many new images at that location
    for entry in data:
        loc = entry["location"]
    ...
    for loc, img_count in new_locs_list:
        if train_imgs_added < num_train:
            ...
            test.append(loc)
    print(
        "New train locations has {} images, val had {} images, test had {} images".format(
            train_imgs_added,
            val_imgs_added,
            num_new_imgs - train_imgs_added - val_imgs_added,
        )
    )
    ...
    print(
        "Appending to {} train imgs, {} val imgs, {} test images - total of {}.".format(
...
print(
    "Now have {} train entries, {} val entries, {} test entries".format(
        len(train), len(val), len(test)
    )
)

# do NOT sort the IDs to keep the shuffled order
updated_splits = {"train": train, "val": val, "test": test}
