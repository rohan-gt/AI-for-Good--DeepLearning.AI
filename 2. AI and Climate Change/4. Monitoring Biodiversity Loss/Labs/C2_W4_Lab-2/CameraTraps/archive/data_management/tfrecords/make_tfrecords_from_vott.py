    ...
    * :obj:`label.dtype == numpy.int32`
    ...
    def __init__(self, root, class_names=[], store_empty_images=False):
        ...
        for image_id, impath in enumerate(self.impaths):
            with open(
                os.path.splitext(impath)[0] + ".bboxes.labels.tsv", "rt"
            ) as labelfile:
            ...
            bbox_coords = np.loadtxt(
                os.path.splitext(impath)[0] + ".bboxes.tsv", dtype=np.int32
            )
        ...
        if not store_empty_images:
            for empty_image_id in empty_images[::-1]:
                print(
                    "Deleting image {} as all bounding boxes are outside".format(
                        empty_image_id
                    )
        ...
        print(
            "The dataset has {} images containing {} classes".format(
                len(self.image_ids), len(self.classes)
            )
        )
        os.chdir(old_dir)
...
def create_tfrecords_format(
    root,
    class_names,
    output_tfrecords_folder,
    dataset_name,
    num_threads=5,
    ims_per_record=200,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Make tfrecords from a VOTT style detection dataset."
    )
    parser.add_argument(
        "--train_dataset_root",
        help="Root dir of VOTT dataset used for training, "
        + " should contain images without any subdirectories.",
        type=str,
        required=True,
    )
    parser.add_argument(
        "--test_dataset_root",
        help="Root dir of VOTT dataset used for testing, "
        + " should contain images without any subdirectories.",
        type=str,
        required=True,
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.path.isdir(args.output_tfrecords_folder):
            raise Exception(
                "Directory {} already exists, ".format(args.output_tfrecords_folder)
                + "please remove any existing content"
            )
        else:
            raise e

    failed_train, classnames = create_tfrecords_format(
    ...
    failed_test, final_classnames = create_tfrecords_format(
