...
imageBase = os.path.join(
    BASE_DIR, r"imerit_batch7_snapshotserengeti_2018.10.26\to_label"
)
...
for iSequence, sequence in enumerate(annData):
    ...
    for iImage, im in enumerate(sequenceImages):
        ...
        if (new_im["height"] != im["height"]) or (new_im["width"] != im["width"]):
...
print(
    "{} empty sequences containing {} images".format(
        len(empty_sequences), nImagesInEmptySequences
    )
)
print("{} empty images".format(nEmptyImages))
print(
    "Writing {} annotations on {} images".format(len(new_images), len(new_annotations))
)
