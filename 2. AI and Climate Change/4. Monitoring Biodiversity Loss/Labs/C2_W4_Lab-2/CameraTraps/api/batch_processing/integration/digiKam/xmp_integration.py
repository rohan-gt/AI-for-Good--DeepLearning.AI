    ...
    try:
        ...
        for detection in image["detections"]:
            ...
            if (
                ...
                and len(options.min_threshold) > 0
                and detection["conf"]
                > original_image_cats_conf.get(categories[detection["category"]], 0)
            ):
                original_image_cats_conf[categories[detection["category"]]] = detection[
                    "conf"
                ]
        ...
        if not (options.rename_conf is None and options.rename_cats is None):
            ...
            if options.min_threshold != None and len(options.min_threshold) > 0:
                for matching_cat in matching_cats:
                    if original_image_cats_conf[matching_cat] < float(
                        options.rename_conf
                    ):
                        is_conf_low = True
            if (
                options.min_threshold != None
                and len(options.min_threshold) > 0
                and len(image["detections"]) == 0
                or (
                    len(options.rename_conf) > 0
                    and is_conf_low is True
                    and len(matching_cats) > 0
                )
            ):
                parent_folder = os.path.dirname(img_path)
                file_name = ntpath.basename(img_path)
                manual_file_name = (
                    file_name.split(".")[0] + "_check" + "." + file_name.split(".")[1]
                )
        ...
        if options.xmp_gui is not None:
            ...
            options.xmp_gui.style.configure(
                "text.Horizontal.Tprogress_bar", text="{:g} %".format(percentage)
            )
        ...
        if False:
            ...
            file_name = ntpath.basename(img_path)
            failed_file_name = (
                file_name.split(".")[0] + "_failed" + "." + file_name.split(".")[1]
            )
            os.rename(img_path, os.path.join(parent_folder, failed_file_name))
...
def process_input_data(options):
    ...
    if options.xmp_gui is not None:
        if (options.image_folder is None) or (len(options.image_folder) == 0):
            tkinter.messagebox.showerror(
                title="Error", message="Image folder is not selected"
            )
            sys.exit()
    ...
    try:
        ...
        if options.xmp_gui is None:
            func = partial(
                update_xmp_metadata, categories, options, rename_cats, n_images
            )
...
def create_gui(options):
    ...
    e1 = tkinter.Entry(
        frame, width=50, textvariable=folder_path_var, highlightthickness=1
    )
    ...
    b1 = tkinter.Button(
        ...
        text="Browse",
        fg="blue",
        command=lambda: browse_folder(options, folder_path_var),
    )
    ...
    e2 = tkinter.Entry(
        frame, width=50, textvariable=file_path_var, highlightthickness=1
    )
    ...
    b2 = tkinter.Button(
        ...
        text="Browse",
        fg="blue",
        command=lambda: browse_file(options, file_path_var),
    )
    ...
    l4 = tkinter.Label(
        frame, text="Confidence level to move images requires manual check (optional)"
    )
    ...
    sb = tkinter.Button(
        ...
        text="Submit",
        fg="black",
        command=lambda: start_input_processing(options),
        padx=10,
    )
    ...
    progress_bar = tkinter.ttk.Progressbar(
        root,
        style="text.Horizontal.Tprogress_bar",
        length=700,
        maximum=100,
        value=0,
        mode="determinate",
...
def main():
    ...
    parser.add_argument(
        "--input_file", help="Path to the MegaDetector .json file", default=None
    )
    parser.add_argument(
        "--image_folder", help="Path to the folder containing images", default=None
    )
    parser.add_argument(
        "--min_threshold",
        help="Minimum detection confidence that will be treated as a detection event",
        default=None,
    )
    parser.add_argument(
        "--remove_path",
        help="Prefix to remove from image paths in the .json file",
        default=None,
    )
    parser.add_argument(
        "--rename_conf",
        help="Below this confidence level, images will be renamed for manual check",
        default=None,
    )
    parser.add_argument(
        "--rename_cat",
        help="Category (or comma-delimited categories) to apply renaming behavior to",
        default=None,
    )
    parser.add_argument(
        "--num_threads", help="Number of threads to use for image processing", default=1
    )
    ...
    if options.gui:
        ...
        assert options.image_folder is None, (
            "Command-line argument specified in GUI mode"
        )
        assert options.min_threshold is None, (
            "Command-line argument specified in GUI mode"
        )
        assert options.remove_path is None, (
            "Command-line argument specified in GUI mode"
        )
        assert options.rename_conf is None, (
            "Command-line argument specified in GUI mode"
        )
