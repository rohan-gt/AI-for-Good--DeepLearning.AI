...
def hello():
    return (
        f"Camera traps batch processing API. Instance: {api_config.API_INSTANCE_NAME}"
    )
...
def request_detections():
    ...
    if caller_id is None or caller_id not in app_config.get_allowlist():
        msg = (
            "Parameter caller is not supplied or is not on our allowlist. "
    ...
    if use_url and isinstance(
        use_url, str
    ):  # in case it is included but is intended to be False
    ...
    if not input_container_sas and not use_url:
        msg = (
            "input_container_sas with read and list access is a required "
    ...
    if use_url and images_requested_json_sas is None:
        return make_error(
            400, "images_requested_json_sas is required since use_url is true."
        )
    ...
    if model_version != "":
        ...
        if (
            model_version not in api_config.MD_VERSIONS_TO_REL_PATH
        ):  # TODO use AppConfig to store model version info
    ...
    if request_name != "":
        ...
        if not set(request_name) <= allowed:
            msg = (
                "request_name contains invalid characters (only letters, "
    ...
    try:
        ...
        job_status_table.create_job_status(
            ...
            status=get_job_status(
                "created", "Request received. Listing images next..."
            ),
    ...
    try:
        ...
        return make_error(
            500, f"Error creating or starting the batch processing thread: {e}"
        )
...
def cancel_request():
    ...
    if caller_id is None or caller_id not in app_config.get_allowlist():
        return make_error(
            401, "Parameter caller is not supplied or is not on our allowlist."
        )
    ...
    if "status" not in item_read:
        return make_error(
            404, "Something went wrong. This task does not have a status field."
        )
    ...
    try:
        ...
        job_status_table.update_job_status(
            job_id,
            {
...
def retrieve_job_status(job_id: str):
    ...
    item_read = job_status_table.read_job_status(
        job_id
    )  # just what the monitoring thread wrote to the DB
    if item_read is None:
        return make_error(404, "Task is not found.")
    if (
        "status" not in item_read
        or "last_updated" not in item_read
        or "call_params" not in item_read
    ):
        return make_error(
            404, "Something went wrong. This task does not have a valid status."
        )
    ...
    last_updated = datetime.fromisoformat(
        item_read["last_updated"][:-1]
    )  # get rid of "Z" (required by Cosmos DB)
    time_passed = datetime.utcnow() - last_updated
    job_is_unmonitored = (
        True
        if time_passed > timedelta(minutes=(api_config.MONITOR_PERIOD_MINUTES + 1))
        else False
    )

    if (
        isinstance(status, dict)
        and "request_status" in status
        and status["request_status"] in ["running", "problem"]
        and "num_tasks" in status
        and job_id not in get_thread_names()
        and job_is_unmonitored
    ):
        ...
        model_version = item_read["call_params"].get(
