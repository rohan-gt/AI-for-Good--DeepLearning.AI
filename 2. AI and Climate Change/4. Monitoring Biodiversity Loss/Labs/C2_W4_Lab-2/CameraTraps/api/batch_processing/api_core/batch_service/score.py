...
IMAGE_ROTATIONS = {3: 180, 6: 270, 8: 90}
...
def open_image(input_file: Union[str, BytesIO]) -> Image:
    ...
    try:
        exif = image._getexif()
        orientation: int = exif.get(
            274, None
        )  # 274 is the key for the Orientation field
        if orientation is not None and orientation in IMAGE_ROTATIONS:
            image = image.rotate(
                IMAGE_ROTATIONS[orientation], expand=True
...
class TFDetector:
    ...
    def __convert_coords(tf_coords):
        ...
        new = [
            ...
            width,
            height,
        ]  # must be a list instead of np.array

        # convert numpy floats to Python floats
        for i, d in enumerate(new):
            new[i] = TFDetector.round_and_make_float(
                d, precision=TFDetector.COORD_DIGITS
            )
        return new
    ...
    def generate_detections_one_image(
        self, image, image_id, detection_threshold=DEFAULT_OUTPUT_CONFIDENCE_THRESHOLD
    ):
        ...
        try:
            ...
            print(
                "TFDetector: image {} failed during inference: {}".format(
                    image_id, str(e)
                )
            )
...
class BatchScorer:
    ...
    def __init__(self, **kwargs):
        ...
        if not self.use_url:
            ...
            self.input_container_client = ContainerClient.from_container_url(
                input_container_sas
            )
        ...
        self.metadata_available = (
            True if isinstance(self.image_ids_to_score[0], list) else False
        )
    ...
    def score_images(self) -> list:
        ...
        for i in self.image_ids_to_score:
            ...
            try:
                ...
                print(
                    f"score.py BatchScorer, score_images, download_image exception: {e}"
                )
                result = {"file": image_id, "failure": TFDetector.FAILURE_IMAGE_OPEN}
            else:
...
def main():
    ...
    input_container_sas = os.environ.get(
        "JOB_CONTAINER_SAS", None
    )  # could be None if use_url
    ...
    print(
        f"score.py, main(), api_instance_name: {api_instance_name}, job_id: {job_id}, task_id: {task_id}, "
    ...
    job_folder_mounted = os.path.join(
        mount_point, "batch-api", f"api_{api_instance_name}", f"job_{job_id}"
    )
    ...
    detector_path = os.path.join(
        mount_point, "models", "megadetector_copies", detector_model_rel_path
    )
    assert os.path.exists(detector_path), (
        f"detector is not found at the specified path: {detector_path}"
    )
