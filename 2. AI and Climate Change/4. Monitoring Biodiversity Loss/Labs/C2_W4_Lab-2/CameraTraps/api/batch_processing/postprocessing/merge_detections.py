...
def merge_detections(source_files, target_file, output_file, options=None):
    ...
    assert not (
        (options.categories_to_exclude is not None)
        and (options.categories_to_include is not None)
    ...
    for i_source_file, source_file in enumerate(source_files):
        ...
        output_data["info"]["detections_transferred_from"].append(
            os.path.basename(source_file)
        )
        output_data["info"]["detector"] = (
            output_data["info"]["detector"] + " + " + source_detector_name
        )

        assert (
            source_data["detection_categories"] == output_data["detection_categories"]
        )

        source_confidence_threshold = options.source_confidence_thresholds[
            i_source_file
        ]
        ...
        for source_im in tqdm(source_data["images"]):
            ...
            for detection_category in detection_categories:
                target_detections_this_category = [
                    det
                    for det in target_detections_this_image
                    if det["category"] == detection_category
                ]

                max_target_confidence_this_category = 0.0

                if len(target_detections_this_category) > 0:
                    max_target_confidence_this_category = max(
                        [det["conf"] for det in target_detections_this_category]
                    )

                # This is already a detection, no need to proceed looking for detections to
                # transfer
                if (
                    max_target_confidence_this_category
                    >= options.target_confidence_threshold
                ):
                    continue

                source_detections_this_category_raw = [
                    det
                    for det in source_detections_this_image
                    if det["category"] == detection_category
                ]
                ...
                source_detections_this_category_filtered = [
                    det
                    for det in source_detections_this_category_raw
                    if (det["bbox"][2] * det["bbox"][3] <= options.max_detection_size)
                    and (det["bbox"][2] * det["bbox"][3] >= options.min_detection_size)
                ]
            ...
            if len(detections_to_transfer) > 0:
                ...
                fn_to_image[image_filename]["max_detection_conf"] = max(
