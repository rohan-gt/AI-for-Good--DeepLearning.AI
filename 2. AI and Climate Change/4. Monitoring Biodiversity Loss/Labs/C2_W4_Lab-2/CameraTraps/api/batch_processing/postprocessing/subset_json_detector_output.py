...
def write_detection_results(data, output_filename, options):
    ...
    if (
        options.copy_jsons_to_folders
        and options.copy_jsons_to_folders_directories_must_exist
    ):
...
def subset_json_detector_output_by_confidence(data, options):
    ...
    print(
        "done, found {} matches (of {}), {} max conf changes".format(
...
def subset_json_detector_output_by_query(data, options):
    ...
    print(
        "Subsetting by query {}, replacement {}...".format(
...
def split_path(path, maxdepth=100):
    ...
    return (
        split_path(head, maxdepth - 1) + [tail]
...
def subset_json_detector_output(input_filename, output_filename, options, data=None):
    ...
    if not options.split_folders:
        ...
        for im in tqdm(data["images"]):
            ...
            if options.split_folder_mode == "bottom":
                ...
                if n_tokens_to_keep > len(tokens):
                    raise ValueError(
                        "Cannot walk {} folders from the top in path {}".format(
                ...
                raise ValueError(
                    "Unrecognized folder split mode {}".format(
                        options.split_folder_mode
                    )
                )
        ...
        if options.make_folder_relative:
            print(
                "Converting database-relative paths to individual-json-relative paths..."
            )
...
def main():
    ...
    parser.add_argument(
        "--replacement", type=str, default=None, help="Replace [query] with this"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--overwrite_json_files", action="store_true", help="Overwrite output files"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--create_folders",
        action="store_true",
        help="When using copy_jsons_to_folders, create folders that dont exist",
    )
