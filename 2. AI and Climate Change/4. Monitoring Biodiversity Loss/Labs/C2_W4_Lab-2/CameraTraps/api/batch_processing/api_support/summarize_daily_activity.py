...
def send_message():
    ...
    result_iterable = db_jobs_client.query_items(
    ...
    for job in result_iterable:
        ...
        if status["request_status"] == "completed":
            instance_num_images[api_instance]["num_images_completed"] += status.get(
                "num_images", 0
            )
        instance_num_images[api_instance]["num_images_total"] += status.get(
            "num_images", 0
        )
    ...
    print(
        f"send_message, sent summary to webhook, response code: {response.status_code}"
    )
    print("")
...
def main():
    ...
    future = current.replace(
        day=current.day, hour=0, minute=5, second=0, microsecond=0
    ) + timedelta(days=1)  # current has been modified
