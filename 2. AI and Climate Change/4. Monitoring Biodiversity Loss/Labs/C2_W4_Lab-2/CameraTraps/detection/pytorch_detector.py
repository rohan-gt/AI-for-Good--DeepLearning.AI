...
class PTDetector:
    ...
    def generate_detections_one_image(
        self, img_original, image_id, detection_threshold, image_size=None
    ):
        ...
        try:
            ...
            if image_size is not None:
                ...
                if not self.printed_image_size_warning:
                    print(
                        "Warning: using user-supplied image size {}".format(image_size)
                    )
            ...
            img = letterbox(
                img_original, new_shape=target_size, stride=PTDetector.STRIDE, auto=True
            )[0]  # JIT requires auto=False
            ...
            if self.device == "mps":
                ...
                pred = non_max_suppression(
                    prediction=pred.cpu(), conf_thres=detection_threshold
                )
            else:
                pred = non_max_suppression(
                    prediction=pred, conf_thres=detection_threshold
                )

            # format detections/bounding boxes
            gn = torch.tensor(img_original.shape)[
                [1, 0, 1, 0]
            ...
            for det in pred:
                if len(det):
                    ...
                    det[:, :4] = scale_coords(
                        img.shape[2:], det[:, :4], img_original.shape
                    ).round()
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        conf = ct_utils.truncate_float(
                            conf.tolist(), precision=CONF_DIGITS
                        )
                        ...
                        detections.append(
                            {
                                ...
                                "bbox": ct_utils.truncate_float_array(
                                    api_box, precision=COORD_DIGITS
                                ),
                            }
                        )
            ...
            print(
                "PTDetector: image {} failed during inference: {}".format(
                    image_id, str(e)
                )
            )
...
if __name__ == "__main__":
    ...
    res = detector.generate_detections_one_image(
        image, im_file, detection_threshold=0.00001
    )
