...
def process_video(options):
    ...
    if options.frame_folder is not None:
        ...
        frame_output_folder = os.path.join(
            tempdir,
            os.path.basename(options.input_video_file) + "_frames_" + str(uuid1()),
        )
    ...
    if options.render_output_video:
        ...
        if not options.keep_output_frames:
            try:
                ...
                print(
                    "Warning: error deleting rendered detection folder {}:\n{}".format(
    ...
    if not options.keep_output_frames:
        try:
            ...
            print(
                "Warning: error deleting frame folder {}:\n{}".format(
...
def process_video_folder(options):
    ...
    assert os.path.isdir(options.input_video_file), "{} is not a folder".format(
        options.input_video_file
    )
    ...
    frame_filenames, Fs = video_folder_to_frames(
        input_folder=options.input_video_file,
    ...
    if options.reuse_results_if_available and os.path.isfile(frames_json):
        ...
        run_detector_batch.write_results_to_file(
            results, frames_json, relative_path_base=frame_output_folder
        )
...
if False:
    ...
    cmd += " --rendering_confidence_threshold " + str(
    ...
    with open(results_file_raw, "r") as f:
        ...
        for im in d["images"]:
            ...
            for det in im["detections"]:
                ...
                if det["category"] in allowed_categories and (
                    (det_size is not None)
                    and (det_size < max_box_size)
                    and (det_size > min_box_size)
                ):
        ...
        print(
            "Kept {} of {} detections (min conf {})".format(
...
def main():
    ...
    parser.add_argument(
        "input_video_file", type=str, help="video file (or folder) to process"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="video output file (or folder), defaults to [video file].mp4 for files, or [video file]_annotated] for folders",
    )
    ...
    parser.add_argument(
        "--keep_output_frames",
        action="store_true",
        help="Disable the deletion of intermediate images (pre- and post-detection rendered frames)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="don't include boxes in the .json file with confidence below this threshold",
    )
