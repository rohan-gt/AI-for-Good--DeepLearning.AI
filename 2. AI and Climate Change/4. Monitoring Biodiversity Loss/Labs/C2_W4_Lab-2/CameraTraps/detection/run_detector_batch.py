...
def producer_func(q, image_files):
    ...
    for im_file in image_files:
        try:
            ...
            print(
                "Producer process: image {} cannot be loaded. Exception: {}".format(
                    im_file, e
                )
            )
            raise
...
def consumer_func(q, return_queue, model_file, confidence_threshold, image_size=None):
    ...
    print(
        "Loaded model (before queueing) in {}".format(
    ...
    while True:
        ...
        if verbose or ((n_images_processed % 10) == 0):
            ...
            print(
                "De-queued image {} ({}/s) ({})".format(
                    n_images_processed, images_per_second, im_file
                )
            )
            sys.stdout.flush()
        results.append(
...
def run_detector_with_image_queue(
    image_files, model_file, confidence_threshold, quiet=False, image_size=None
):
    """
...
def process_images(
    ...
    if isinstance(detector, str):
        ...
        elapsed = time.time() - start_time
        print(
            "Loaded model (batch level) in {}".format(
                humanfriendly.format_timespan(elapsed)
            )
        )
    ...
    if use_image_queue:
        run_detector_with_image_queue(
            im_files, detector, confidence_threshold, quiet=quiet, image_size=image_size
        )
    else:
        results = []
...
def process_image(
    im_file, detector, confidence_threshold, image=None, quiet=False, image_size=None
):
    """
    ...
    try:
        ...
        result = {"file": im_file, "failure": FAILURE_INFER}
        return result
...
def load_and_run_detector_batch(
    ...
    if isinstance(image_file_names, str):
        ...
        if os.path.isdir(image_file_names):
            ...
            print(
                "{} image files found in folder {}".format(
                    len(image_file_names), image_dir
                )
            )
        ...
        # A json list of image paths
            ...
            print(
                "Loaded {} image filenames from list file {}".format(
                    len(image_file_names), list_file
                )
            )

        # A single image file
        elif os.path.isfile(image_file_names) and ImagePathUtils.is_image_file(
            image_file_names
        ):
            ...
            raise ValueError(
                "image_file_names is a string, but is not a directory, a json "
    ...
    if use_image_queue:
        assert n_cores <= 1
        results = run_detector_with_image_queue(
            image_file_names,
            model_file,
            confidence_threshold,
            quiet,
...
def write_results_to_file(
    results, output_file, relative_path_base=None, detector_file=None, info=None
):
    """
    ...
    if relative_path_base is not None:
        ...
        for r in results:
            ...
            r_relative["file"] = os.path.relpath(
                r_relative["file"], start=relative_path_base
            )
    ...
    # If the caller supplied the entire "info" struct
        ...
        if detector_file is not None:
            print(
                "Warning (write_results_to_file): info struct and detector file "
...
if False:
    ...
    image_size = None
    image_file_names = image_file_names = ImagePathUtils.find_images(
        image_dir, recursive=False
    )
...
def main():
    ...
    parser.add_argument(
        "detector_file", help="Path to detector model file (.pb or .pt)"
    )
    parser.add_argument(
        "image_file",
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Suppress per-image console output"
    )
    parser.add_argument(
        "--image_size",
        type=int,
    ...
    if args.checkpoint_frequency != -1:
        assert args.checkpoint_frequency > 0, (
            "Checkpoint_frequency needs to be > 0 or == -1"
        )
    ...
    if os.path.exists(args.output_file):
        print(
            "Warning: output_file {} already exists and will be overwritten".format(
                args.output_file
            )
        )
    ...
    if args.resume_from_checkpoint is not None:
        ...
        with open(args.resume_from_checkpoint) as f:
            print(
                "Loading previous results from checkpoint file {}".format(
    ...
    assert len(image_file_names) > 0, (
        "Specified image_file does not point to valid image files"
    )
    ...
    if args.checkpoint_frequency != -1:
        ...
        if (
            ...
            and (not args.allow_checkpoint_overwrite)
    ...
    elapsed = time.time() - start_time
    print(
        "Finished inference for {} images in {}".format(
