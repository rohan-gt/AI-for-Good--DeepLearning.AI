...
def is_video_file(s: str, video_extensions: Container[str] = VIDEO_EXTENSIONS) -> bool:
    """
...
def get_video_fs(input_video_file):
    ...
    assert os.path.isfile(input_video_file), "File {} not found".format(
        input_video_file
    )
...
def video_to_frames(
    input_video_file, output_folder, overwrite=True, every_n_frames=None, verbose=False
):
    """
    ...
    if verbose:
        print(
            "Reading {} frames at {} Hz from {}".format(n_frames, Fs, input_video_file)
        )
    ...
    for frame_number in range(0, n_frames):
        ...
        if not success:
            ...
            if verbose:
                print(
                    "Read terminating at frame {} of {}".format(frame_number, n_frames)
                )
            break
        ...
        if overwrite == False and os.path.isfile(frame_filename):
            ...
            try:
                ...
                print(
                    "Error on frame {} of {}: {}".format(frame_number, n_frames, str(e))
                )
...
def video_folder_to_frames(
    ...
    input_files_relative_paths = [
        os.path.relpath(s, input_folder) for s in input_files_full_paths
    ]
    input_files_relative_paths = [
        s.replace("\\", "/") for s in input_files_relative_paths
    ]
    ...
    if n_threads == 1:
        ...
        results = list(
            tqdm(
                pool.imap(process_video, input_files_relative_paths),
...
def frame_results_to_video_results(
    input_file, output_file, options: FrameToVideoOptions = None
):
    """
    ...
    print(
        "Found {} unique videos in {} frame-level results".format(
    ...
    for video_name in tqdm(video_to_frames):
        ...
        for category_id in detection_categories:
            category_detections = [
                det
                for det in all_detections_this_video
                if det["category"] == category_id
            ]
            ...
            if len(category_detections) > options.nth_highest_confidence:
                category_detections_by_confidence = sorted(
                    category_detections, key=lambda i: i["conf"], reverse=True
                )
                canonical_detection = category_detections_by_confidence[
...
if False:
    ...
    for folder in folders:
        ...
        folder_relative = folder.replace(
            (frame_folder_base + "/").replace("\\", "/"), ""
        )
        detection_results_this_folder = [
            d for d in detections if folder_relative in d["file"]
        ]
        print(
            "Found {} detections in folder {}".format(
                len(detection_results_this_folder), folder
            )
        )
        assert len(frame_files_this_folder) == len(detection_results_this_folder)

        rendered_frame_output_folder = os.path.join(
            detected_frame_folder_base, folder_relative
        )
        ...
        for d in tqdm(detection_results_this_folder):
            ...
            vis_utils.draw_bounding_boxes_on_file(
                input_file, output_file, d["detections"], confidence_threshold
            )
