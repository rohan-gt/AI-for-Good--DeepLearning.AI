...
class_list = ["empty"] + [
    cname.lower() for cname in open(args.class_list, "r").read().splitlines()
]
...
for detectionid in crops_json:
    ...
    existing_image_entries = Image.select().where(
        (Image.file_name == detection_data["file_name"])
    )
    try:
        ...
        image_entry = Image.create(
            ...
            grayscale=detection_data["grayscale"],
            source_file_name=detection_data["source_file_name"],
            relative_size=detection_data["relative_size"],
            seq_id=detection_data["seq_id"],
            seq_num_frames=detection_data["seq_num_frames"],
        ...
        detection_entry = Detection.create(
            id=detectionid,
            image=detectionid,
            bbox_confidence=detection_data["bbox_confidence"],
    ...
    if counter % 100 == 0:
        print(
            "Updated database with Image and Detection table entries for %d out of %d crops in %0.2f seconds"
            % (counter, num_detections, time.time() - timer)
        )
