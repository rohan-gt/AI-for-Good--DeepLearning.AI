...
class Image(Model):
    ...
    relative_size = FloatField(
        null=True
    )  # cropped image size relative to original image size
    source_file_name = CharField(
        null=True
    )  # original image file name from which cropped image was generated
...
class Detection(Model):
    ...
    image = ForeignKeyField(
        Image
    )  # pointer to cropped image the detection corresponds to
    ...
    category_confidence = FloatField(
        null=True
