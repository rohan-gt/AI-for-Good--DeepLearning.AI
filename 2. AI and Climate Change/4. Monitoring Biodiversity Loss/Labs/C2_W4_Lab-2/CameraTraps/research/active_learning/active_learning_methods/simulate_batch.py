...
class SimulateBatchSampler(WrapperSamplingMethod):
    ...
    def simulate_batch(
        self, sampler, N, already_selected, y, model, X_test, y_test, **kwargs
    ):
        ...
        for _ in range(self.n_sims):
            ...
            for _ in range(n_minibatch):
                ...
                if batch_size > 0:
                    ...
                    # already_selected
                    kwargs["already_selected"] = sorted(
                        kwargs["already_selected"] + new_inds
                    )
                    kwargs["y"][new_inds] = new_y
                    kwargs["model"].fit(
                        self.X[kwargs["already_selected"]],
            ...
            if acc_hallucinated > best_acc:
                ...
                best_inds = inds
            kwargs["model"].fit(
                self.X[kwargs["already_selected"]], y[kwargs["already_selected"]]
            )
            ...
            print(
                "Hallucinated acc: %.3f, Actual Acc: %.3f"
                % (acc_hallucinated, acc_true)
            )
    ...
    def select_batch_(self, N, already_selected, y, model, X_test, y_test, **kwargs):
        ...
        assert all(
            [
                copy_selected[i] == already_selected[i]
