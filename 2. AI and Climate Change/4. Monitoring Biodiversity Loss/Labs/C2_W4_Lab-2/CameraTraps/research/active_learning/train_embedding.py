...
parser.add_argument("--train_data", help="path to train dataset", default="crops")
parser.add_argument("--val_data", help="path to validation dataset", default="smalls")
parser.add_argument(
    "--arch",
    "-a",
    default="resnet18",
...
parser.add_argument(
    "--epochs", default=5, type=int, help="number of total epochs to run"
)
parser.add_argument(
    "-b", "--batch_size", default=256, type=int, help="mini-batch size (default: 256)"
)
parser.add_argument(
    "--lr", "--learning_rate", default=0.00001, type=float, help="initial learning rate"
)
parser.add_argument(
    "--weight_decay",
    "--wd",
...
parser.add_argument(
    "--margin", default=1.0, type=float, help="margin for siamese or triplet loss"
)
parser.add_argument(
    "-f", "--feat_dim", default=256, type=int, help="embedding size (default: 256)"
)
parser.add_argument(
    "--raw_size",
    nargs=2,
...
parser.add_argument(
    "--strategy",
    default="random",
    choices=["hardest", "random", "semi_hard", "hard_pair"],
    help="The number of examples from each class in each balanced batch",
)
...
def main():
    ...
    if args.loss_type.lower() == "softmax":
        train_loader = train_dataset.getSingleLoader(
            batch_size=128, shuffle=True, num_workers=args.num_workers
        )
        ...
        train_loader = train_dataset.getBalancedLoader(
            P=args.balanced_P, K=args.balanced_K, num_workers=args.num_workers
        )
        train_embd_loader = train_dataset.getSingleLoader(num_workers=args.num_workers)
        if args.val_data is not None:
            val_loader = val_dataset.getBalancedLoader(
                P=args.balanced_P,
                K=args.balanced_K,
                num_workers=args.num_workers,
                transfm="val",
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        # train for one epoch
        e.train_one_epoch(
            train_loader, epoch, True if args.loss_type.lower() == "softmax" else False
        )
        if epoch % args.plot_freq == 0 and epoch > 0:
            a, b, _ = e.predict(train_embd_loader, load_info=True, dim=args.feat_dim)
            save_embedding_plot(
                "%s_train_%d.jpg" % (args.checkpoint_prefix, epoch), a, b, {}
            )
            a, b, _ = e.predict(val_embd_loader, load_info=True, dim=args.feat_dim)
            save_embedding_plot(
                "%s_val_%d.jpg" % (args.checkpoint_prefix, epoch), a, b, {}
            )
        # evaluate on validation set
        if args.val_data is not None:
            e.validate(
                val_loader, True if args.loss_type.lower() == "softmax" else False
            )
        # save a checkpoint
        save_checkpoint(
            {
