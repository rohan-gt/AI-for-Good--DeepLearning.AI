...
def find_probablemap(true_labels, clustering_labels, K=5):
    ...
    for x in clusters:
        ...
        mapping[x] = int(
            stats.mode(np.random.choice(sub, K), axis=None)[0]
        )  # int(stats.mode(sub, axis=None)[0])
    return mapping
...
def completeLoop(X, y, base_ind):
    ...
    trainset_query = (
        Detection.select(Detection.id, Oracle.label, Detection.embedding)
        .join(Oracle)
        .where(Oracle.status == 0, Detection.kind == DetectionKind.UserDetection.value)
    )
    embedding_dataset = SQLDataLoader(
        trainset_query, "all_crops/SS_full_crops", True, num_workers=4, batch_size=64
    )
    ...
    embedding_dataset2 = SQLDataLoader(
        trainset_query, "all_crops/SS_full_crops", False, num_workers=4, batch_size=512
    )
    em = e.embedding(embedding_dataset2.getSingleLoader())
    lb = np.asarray([x[1] for x in embedding_dataset2.samples])
    pt = [
        x[0] for x in embedding_dataset2.samples
    ...
    clf_optimizer = torch.optim.Adam(
        clf_model.parameters(), lr=0.001, weight_decay=0.0005
    )
    ...
    testset_query = (
        Detection.select(Detection.id, Oracle.label, Detection.embedding)
        .join(Oracle)
        .where(Oracle.status == 0, Detection.kind == DetectionKind.ModelDetection.value)
    )
    test_dataset = SQLDataLoader(
        testset_query,
        "all_crops/SS_full_crops",
        False,
        datatype="image",
...
def selectSamples(embd, paths, n):
    ...
    while len(selected_set) < n:
        ...
        for i, x in enumerate(db.labels_):
            if (
                x == -1
                and getDistance(embd, indices, rand_ind[i]) >= 0.3
                and getDistance(embd, selected_set, rand_ind[i]) >= 0.3
            ):
                indices.add(rand_ind[i])
        moveRecords(
            DetectionKind.ModelDetection,
            DetectionKind.ActiveDetection,
            [paths[i] for i in indices.difference(selected_set)],
        )
...
def getDistance(embd, archive, sample):
    if len(archive) == 0:
        ...
        return pairwise_distances_argmin_min(
            embd[sample].reshape(1, -1), embd[np.asarray(list(archive), dtype=np.int32)]
        )[1]


def moveRecords(srcKind, destKind, rList):
    query = Detection.update(kind=destKind.value).where(
        Detection.id.in_(rList), Detection.kind == srcKind.value
    )
    # print(query.sql())
    query.execute()
