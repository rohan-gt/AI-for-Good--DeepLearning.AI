...
parser.add_argument(
    "image_dir",
    type=str,
    default="missouricameratraps/lila",
    help="Root folder of the images.",
)
parser.add_argument(
    "output_dir",
    type=str,
    default="missouricameratraps/crops",
    ...
    with tf.Session() as sess:
        ...
        if "detection_masks" in tensor_dict:
            ...
            detection_boxes = tf.slice(
                detection_boigxes, [0, 0], [real_num_detection, -1]
            )
        ...
        imgs_in_dir = glob.glob(
            os.path.join(IMAGE_DIR, "**/*.JPG"), recursive=True
        )  # All images in directory (may be a subset of the dataset)
        for cur_image in tqdm.tqdm(sorted(imgs_in_dir)):
            ...
            selected_boxes = output_dict["detection_boxes"][selection] * np.tile(
                [imsize[1], imsize[0]], (1, 2)
            )
            ...
            offsets = (
                PADDING_FACTOR * np.max(bbox_sizes, axis=1, keepdims=True) - bbox_sizes
            ) / 2
            ...
            for box_id in range(selected_boxes.shape[0]):
                ...
                crop_box_area = (crop_box[1] - crop_box[0]) * (
