...
class UI(QTabWidget):
    ...
    def active(self, event):
        ...
        embd = np.asarray(
            [np.fromstring(x[2], dtype="<f4") for x in run_dataset.samples]
        )
        # label = [ x[1] for x in run_dataset.samples]
        paths = [
            x[0] for x in run_dataset.samples
    ...
    def selectSamples(self, embd, paths, n):
        ...
        while len(selected_set) < n:
            ...
            for i, x in enumerate(db.labels_):
                if (
                    x == -1
                    and self.getDistance(embd, indices, rand_ind[i]) > 0.7
                    and self.getDistance(embd, selected_set, rand_ind[i]) > 0.7
                ):
    ...
    def getDistance(self, embd, archive, sample):
        if len(archive) == 0:
            ...
            return pairwise_distances_argmin_min(
                embd[sample].reshape(1, -1),
                embd[np.asarray(list(archive), dtype=np.int32)],
            )[1]
