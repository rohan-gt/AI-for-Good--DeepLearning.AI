...
class Tag(QWidget):
    ...
    def getFinal(self):
        ...
        if (
            abs(x - self.bbox[0]) < 0.01
            and abs(x - self.bbox[0]) < 0.01
            and abs(x - self.bbox[0]) < 0.01
            and abs(x - self.bbox[0]) < 0.01
        ):
    ...
    def speciesChanged(self, text):
        if text == "Add New":
            self.parentWidget().parentWidget().parentWidget().parentWidget().setCurrentIndex(
                2
            )
        else:
            self.label = text
    ...
    def setCursorShape(self, e_pos: QPoint):
        ...
        if (
            (
                (e_pos.y() > self.y() + self.height() - diff)  # Bottom
                and (e_pos.x() < self.x() + diff)
            )  # Left
            or
            # Right-Bottom
            (
                (e_pos.y() > self.y() + self.height() - diff)  # Bottom
                and (e_pos.x() > self.x() + self.width() - diff)
            )  # Right
            or
            # Left-Top
            (
        ...
        # check cursor horizontal position
            ...
            or (e_pos.x() > self.x() + self.width() - diff)
        ):  # Right
