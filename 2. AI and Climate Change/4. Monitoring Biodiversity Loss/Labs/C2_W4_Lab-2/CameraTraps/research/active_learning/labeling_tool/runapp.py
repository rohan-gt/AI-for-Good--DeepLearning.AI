...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run a web user interface for labeling camera trap images for classification."
    )
    parser.add_argument(
        "--host", type=str, default="0.0.0.0", help="Web server host to bind to."
    )  ## default='localhost', help='Web server host to bind to.')
    parser.add_argument(
        "--port", type=int, default=8080, help="Web server port port to listen on."
    )
    parser.add_argument(
        "--verbose", type=bool, default=True, help="Enable verbose debugging."
    )
    parser.add_argument(
        "--db_name",
        type=str,
        default="missouricameratraps",
        help="Name of Postgres DB with target dataset tables.",
    )
    parser.add_argument(
        "--db_user", type=str, default=None, help="Name of user accessing Postgres DB."
    )
    parser.add_argument(
        "--db_password",
        type=str,
        default=None,
        help="Password of user accessing Postgres DB.",
    )
    parser.add_argument(
        "--db_query_limit",
        default=3000,
        type=int,
        help="Maximum number of records to read from the Postgres DB.",
    )
    parser.add_argument(
        "--crop_dir",
        type=str,
        required=True,
        help="Path to directory containing cropped images to display.",
    )
    parser.add_argument(
        "--class_list",
        type=str,
        required=True,
        help="Path to .txt file containing classes in dataset.",
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to a specific classifier checkpoint to load initially.",
    )
    parser.add_argument(
        "--embedding_checkpoint",
        type=str,
        default="/home/lynx/pretrainedmodels/embedding_triplet_resnet50_1499/triplet_resnet50_1499.tar",
        help="Path to a specific embedding checkpoint to load initially.",
    )
    ...
    target_db = PostgresqlDatabase(
        DB_NAME, user=USER, password=PASSWORD, host="localhost"
    )
    ...
    if (
        checkpoint["loss_type"].lower() == "center"
        or checkpoint["loss_type"].lower() == "softmax"
    ):
        embedding_net = SoftmaxNet(
            checkpoint["arch"], checkpoint["feat_dim"], checkpoint["num_classes"], False
        )
    else:
        embedding_net = NormalizedEmbeddingNet(
            checkpoint["arch"], checkpoint["feat_dim"], False
        )
    ...
    grayscale_indices = list(
        itertools.compress(range(len(grayscale_values)), grayscale_values)
    )  # records with grayscale images
    color_indices = list(
        set(range(len(dataset.samples))) - set(grayscale_indices)
    )  # records with color images
    ...
    webUIapp_server_kwargs = {"server": "tornado", "host": args.host, "port": args.port}
    ...
    def refresh_images_to_display():
        ...
        indices_to_exclude.update(
            set(dataset.set_indices[DetectionKind.UserDetection.value])
        )  # never show records that have been labeled by the user
        indices_to_exclude.update(
            set(dataset.set_indices[DetectionKind.ConfirmedDetection.value])
        )  # never show records that have been confirmed by the user
        detection_conf_thresh_indices = [
            i
            for i, e in enumerate(detection_conf_values)
            if e < float(data["detection_threshold"])
        ]  # find records below the detection confidence threshold
        ...
        if data["display_class"] == "All Species":
            ...
            dataset_class_labels = [
                dataset.samples[i][1] for i in range(len(dataset.samples))
            ]
            other_classes = [
                i for i, cl in enumerate(dataset_class_labels) if cl != cat_id
            ]
    ...
    def load_images():
        ...
        data["display_images"]["image_file_names"] = [
            dataset.samples[i][5] for i in indices
        ]
        data["display_images"]["detection_kinds"] = [
            dataset.samples[i][2] for i in indices
        ]
        ...
        for i in indices:
            if str(dataset.samples[i][1]) == "None":
                ...
                existing_category_entries = {
                    cat.id: cat.name for cat in Category.select()
                }
    ...
    def assign_label():
        ...
        if label_category_name == "empty":
            ...
            moveRecords(
                dataset,
                DetectionKind.ModelDetection.value,
            ...
            matching_detection_entries = Detection.select(
                Detection.id, Detection.category_id
            ...
            for mde in matching_detection_entries:
                command = Detection.update(
                    ...
                    kind=DetectionKind.UserDetection.value,
                ).where(Detection.id == mde.id)
                command.execute()
            ...
            dataset.set_indices[4] = list(
                set(dataset.set_indices[4]).union(set(indices_to_label))
            )  # add the index to the set of labeled/confirmed indices
            # numLabeled = len(dataset.set_indices[DetectionKind.UserDetection.value])
    ...
    def confirm_predicted_label():
        ...
        if label_category_name == "empty":
            ...
            moveRecords(
                dataset,
                DetectionKind.ModelDetection.value,
                DetectionKind.ConfirmedDetection.value,
                [index_to_label],
            )
            ...
            command = Detection.update(
                category_id=label_category_id,
                category_confidence=1,
                kind=DetectionKind.ConfirmedDetection.value,
            ).where(Detection.id == mde.id)
            command.execute()
            ...
            dataset.set_indices[4] = list(
                set(dataset.set_indices[4]).union({index_to_label})
            )  # add the index to the set of labeled/confirmed indices
            # numLabeled = len(dataset.set_indices[DetectionKind.UserDetection.value])
            print([len(x) for x in dataset.set_indices])
    ...
    def train_classifier():
        ...
        y_pred = kwargs["model"].predict(X_pred)
        print(
            "Predicting on unlabeled samples took %0.2f seconds" % (time.time() - timer)
        )
        # print(y_pred)
        ...
        for pos in range(len(y_pred)):
            ...
            dataset.samples[idx] = tuple(sample_data)
        print(
            "Updating the dataset dataloader took %0.2f seconds" % (time.time() - timer)
        )

        if not classifier_trained:
            # once the classifier has been trained the first time, switch to AL sampling
            classifier_trained = True
            sampler = get_AL_sampler("confidence")(
                dataset.em, dataset.getalllabels(), 1234
            )
    ...
    def show_fullsize_image():
        ...
        matching_image_entries = Image.select(
            Image.file_name, Image.source_file_name
    ...
    def get_sequential_images():
        ...
        matching_image_entries = Image.select(
            Image.seq_id, Image.seq_num_frames, Image.frame_num
        ).where((Image.file_name == image_src))
        try:
            mie = matching_image_entries.get()
            if mie.seq_num_frames > 1:
                images_in_seq = (
                    Image.select(Image.source_file_name)
                    .where((Image.seq_id == mie.seq_id))
                    .order_by(Image.frame_num)
                )
                image_sequence = sorted(
                    list(set([i.source_file_name for i in images_in_seq]))
                )
