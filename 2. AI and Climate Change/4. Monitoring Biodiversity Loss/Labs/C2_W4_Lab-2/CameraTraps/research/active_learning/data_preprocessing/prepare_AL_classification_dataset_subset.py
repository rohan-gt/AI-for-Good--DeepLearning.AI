...
def main():
    ...
    parser.add_argument(
        ...
        default=1000,
        type=int,
        help="Number of samples to draw from the full dataset for the subset dataset.",
    )
    parser.add_argument(
        "--crop_dir",
        type=str,
        help="Path to directory with the cropped images of the full dataset.",
    )
    parser.add_argument(
        "--crop_json",
        type=str,
        help="Path to .json with information about cropped images in the full dataset.",
    )
    parser.add_argument(
        "--image_dir",
        type=str,
        help="Path to directory with the full-size source images of the full dataset.",
    )
    parser.add_argument(
        "--old_base_dir",
        type=str,
        help="Path specifying base directory for crop and image subdirs, to be stripped from filenames in crops.json.",
    )
    parser.add_argument(
        "--new_base_dir",
        type=str,
        help="Path specifying base directory for crop and image subdirs, to be added to filenames in crops.json.",
    )
    ...
    parser.add_argument(
        ...
        default=1234,
        type=int,
        help="Random seed to get same samples from dataset.",
    )
    ...
    if verbose:
        ...
        print(
            "Writing subset crops to: \t\t%s"
            % args.crop_dir.replace(args.old_base_dir, args.new_base_dir)
        )
        print(
            "Writing subset full-size images to: \t%s"
            % args.image_dir.replace(args.old_base_dir, args.new_base_dir)
        )
    ...
    for k in subset_crops:
        ...
        k_data["file_name"] = k_data["file_name"].replace(
            args.old_base_dir, args.new_base_dir
        )  # correct base directory from old (full dataset) to new (subset dataset)
        k_data["source_file_name"] = k_data["source_file_name"].replace(
            args.old_base_dir, args.new_base_dir
        )
        subset_crop_data[k] = k_data

        # Copy file for this crop to subset dataset crop dir
        sh_command = "mkdir -p %s && cp %s $_" % (
            os.path.dirname(k_data["file_name"]),
            crop_data[k]["file_name"],
        )
        os.popen(sh_command)

        # Copy file for its full-size source image to subset dataset image dir
        sh_command = "mkdir -p %s && cp %s $_" % (
            os.path.dirname(k_data["source_file_name"]),
            crop_data[k]["source_file_name"],
        )
        os.popen(sh_command)
