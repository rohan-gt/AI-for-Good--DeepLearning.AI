...
class ImageryVisualizer(object):
    ...
    default_landsat_sr_viz_params = DefaultVisParams(min=0.0, max=3000.0, gamma=0.7)

    # ee.ImageCollection('COPERNICUS/S2_SR')
    default_sentinel2_sr_viz_params = DefaultVisParams(min=0.0, max=3000.0, gamma=0.7)
    ...
    landsat8_swir = (
        ...
        5,
        1,
    )  # shortwave infrared SWIR as red, NIR as green, and deep blue as blue
    ...
    def show_single_band(
        ...
        normalizer: mcolors.Normalize = normalized_band_normalizer,
        ...
        assert len(raster.shape) == 2, (
            "Single band should be a 2D array after squeezing."
        )
    ...
    def norm_band(
        ...
        """
        assert band_max > band_min, (
            f"invalid range specified by band_min {band_min} and band_max {band_max}"
        )
    ...
    def get_landsat8_ndvi(
        tile: Union[rasterio.DatasetReader, np.ndarray],
        ...
        if isinstance(tile, rasterio.io.DatasetReader):
            ...
            band_red = tile.read(
                4, window=window, boundless=True, fill_value=0
            ).squeeze()
            band_nir = tile.read(
                5, window=window, boundless=True, fill_value=0
            ).squeeze()
    ...
    def show_patch(
        ...
        if isinstance(bands, int):
            bands = [
                bands
            ]  # otherwise rasterio read and numpy indexing will return a 2D array instead of 3D
        ...
        if isinstance(tile, rasterio.io.DatasetReader):
            ...
            bands = [
                b - 1 for b in bands
            ]  # rasterio indexes bands from 1, here we subtract 1 for numpy
        ...
        tile_bands = ImageryVisualizer.norm_band(
            tile_bands, band_min=band_min, band_max=band_max, gamma=gamma
        )
    ...
    def show_landsat8_patch(
        ...
        """
        return ImageryVisualizer.show_patch(
            tile,
            bands=bands,
            window=window,
            band_min=band_min,
    ...
    def stat_landsat_tile(tile_tif, n_bins=40):
        ...
        for b in range(1, tile_tif.count + 1):
            band = tile_tif.read(
                b
            ).flatten()  # returns a numpy.ndarray; flatten otherwise hist is "2D"
            band_stats[b] = {
                "min": band.min(),
                "max": band.max(),
                "mean": band.mean(),
