...
class RasterLabelVisualizer(object):
    ...
    def __init__(self, label_map: Union[str, dict]):
        ...
        for num, color_name in sorted(
            self.num_to_color.items(), key=lambda x: x[0]
        ...
        for num, color in num_to_color.items():
            if mcolors.is_color_like(color):
                ...
                assert len(color) == 3 or len(color) == 4, (
                    f"Color {color} is specified as a tuple or list but is not of length 3 or 4"
                )
                for c in color:
                    assert isinstance(c, int) and 0 < c < 256, (
                        f"RGB value {c} is out of range"
                    )

                new[num] = RasterLabelVisualizer.uint8_rgb_to_hex(
                    color[0], color[1], color[3]
                )  # drop any alpha values
    ...
    @staticmethod
    def matplotlib_color_to_uint8_rgb(
    ...
    @staticmethod
    def plot_colortable(
        name_to_color: dict, title: str, sort_colors: bool = False, emptycols: int = 0
    ) -> plt.Figure:
        """
        ...
        for i, name in enumerate(names):
            ...
            ax.hlines(
                y, swatch_start_x, swatch_end_x, color=name_to_color[name], linewidth=18
            )
    ...
    def show_label_raster(
        self,
        label_raster: Union[Image.Image, np.ndarray],
        ...
        _ = plt.imshow(
            label_raster, cmap=self.colormap, norm=self.normalizer, interpolation="none"
        )
