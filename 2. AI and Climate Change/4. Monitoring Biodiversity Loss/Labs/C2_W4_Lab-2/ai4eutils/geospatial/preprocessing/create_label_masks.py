...
def get_feature_info(feature):
    ...
    for feat in feature["features"]["xy"]:
        ...
        coords = list(
            mapping(feat_shape)["coordinates"][0]
...
def mask_polygons_together_with_border(size, polys, border):
    ...
    for uid, tup in polys.items():
        ...
        fillPoly(
            mask_img, [ns_poly], (damage_class_num, damage_class_num, damage_class_num)
        )
...
def mask_tiles(images_dir, label_paths, targets_dir, border_width, overwrite_target):
    for label_path in tqdm(label_paths):
        tile_id = os.path.basename(label_path).split(".json")[
            0
...
def main():
    ...
    parser.add_argument(
        "root_dir",
        help=(
            "Path to the directory that contains both the `images` and `labels` folders. "
            "The `targets_border{border_width}` folder will be created if it does not already exist."
        ),
    )
    parser.add_argument("-b", "--border_width", type=int, default=1)
    parser.add_argument(
        "-o",
        "--overwrite_target",
    ...
    li_label_paths = [
        os.path.join(labels_dir, i)
        for i in li_label_fn
        if i.startswith(DISASTERS_OF_INTEREST)
    ]

    print(
        f"{len(li_label_fn)} label jsons found in labels_dir, "
