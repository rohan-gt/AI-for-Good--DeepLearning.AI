...
def create_sample_file(input_file_path, input_file_size_gb):
    assert not os.path.exists(input_file_path), "Target file {} already exists".format(
        input_file_path
    )
    ...
    print(
        "Generating input file at {} ({} GB)".format(
            input_file_path, input_file_size_gb
        )
    )
    str_command = "dd if=/dev/urandom of={} bs={} count={}".format(
        input_file_path, block_size_bytes, count
    )
...
def get_bandwidth(
    ...
    input_file_size_gb=default_file_size_gb,
    page_blob=False,
):
    ...
    if not os.path.exists(input_file_path):
        ...
        assert os.path.isfile(input_file_path), "{} is not a valid file name".format(
            input_file_path
        )

    str_command = "azcopy copy {} {} --output-type text".format(
        input_file_path, sas_url
    )
    if page_blob:
...
def main():
    ...
    parser.add_argument("sas_url", type=str, help="SAS URL for blob container")
