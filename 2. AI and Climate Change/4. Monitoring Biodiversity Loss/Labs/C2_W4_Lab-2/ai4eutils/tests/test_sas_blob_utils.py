...
class Tests(unittest.TestCase):
    ...
    def tearDown(self):
        if self.needs_cleanup:
            ...
            with BlobClient(
                ...
                credential=PRIVATE_ACCOUNT_KEY,
            ) as bc:
                if bc.exists():
                    print("deleted blob")
            ...
            with ContainerClient.from_container_url(
                PRIVATE_CONTAINER_URI, credential=PRIVATE_ACCOUNT_KEY
            ) as cc:
                try:
    ...
    def test_get_container_from_uri(self):
        self.assertEqual(get_container_from_uri(PUBLIC_BLOB_URI), "nacti-unzipped")
    ...
    def test_get_sas_token_from_uri(self):
        ...
        self.assertEqual(
            get_sas_token_from_uri(PUBLIC_CONTAINER_URI_SAS), PUBLIC_CONTAINER_SAS
        )
    ...
    def test_check_blob_exists(self):
        ...
        self.assertTrue(
            check_blob_exists(PUBLIC_CONTAINER_URI, blob_name=PUBLIC_BLOB_NAME)
        )
    ...
    def test_list_blobs_in_container(self):
        blobs_list = list_blobs_in_container(PUBLIC_ZIPPED_CONTAINER_URI, limit=100)
        expected = sorted(
            [
                "wcs_20200403_bboxes.json.zip",
                "wcs_camera_traps.json.zip",
                "wcs_camera_traps_00.zip",
                "wcs_camera_traps_01.zip",
                "wcs_camera_traps_02.zip",
                "wcs_camera_traps_03.zip",
                "wcs_camera_traps_04.zip",
                "wcs_camera_traps_05.zip",
                "wcs_camera_traps_06.zip",
                "wcs_specieslist.csv",
                "wcs_splits.json",
            ]
        )
    ...
    def test_upload_blob(self):
        ...
        with self.assertRaises(HttpResponseError):
            upload_blob(PUBLIC_CONTAINER_URI, blob_name="failblob", data="fail")
        ...
        with self.assertRaises(HttpResponseError):
            upload_blob(PUBLIC_CONTAINER_URI_SAS, blob_name="failblob", data="fail")
        ...
        with self.assertRaises(HttpResponseError):
            upload_blob(
                PRIVATE_CONTAINER_URI, blob_name=PRIVATE_BLOB_NAME, data="success"
            )
        ...
        blob_url = upload_blob(
            private_container_uri_sas, blob_name=PRIVATE_BLOB_NAME, data="success"
        )
    ...
    def test_build_blob_uri(self):
        generated = build_blob_uri(
            container_uri=PUBLIC_CONTAINER_URI, blob_name=PUBLIC_BLOB_NAME
        )
        self.assertEqual(generated, PUBLIC_BLOB_URI)

        generated = build_blob_uri(
            container_uri=PUBLIC_CONTAINER_URI_SAS, blob_name=PUBLIC_BLOB_NAME
        )
