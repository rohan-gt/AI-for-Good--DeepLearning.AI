    ...
    for detection in megadetector_result["detections"]:
        ...
        if detection["category"] == "1":
            ...
            shape = [
                ...
                (int((bbox[0] + bbox[2]) * w), int((bbox[1] + bbox[3]) * h)),
            ]
...
def crop_image(
    image: ImageOps, megadetector_result: Dict, input_folder: str, output_subfolder: str
) -> List:
    ...
    for res in megadetector_result["detections"]:
        ...
        if res["category"] == "1":
            ...
            new_file = new_file = megadetector_result["file"].replace(
...
def plot_detections(image: ImageOps, detections: List, megadetector_result: Dict):
...
def preprocess_dataset(
    ...
    for count, im_file in enumerate(all_filenames):
        ...
        res = megadetector.generate_detections_one_image(
            image, im_file, detection_threshold=detection_threshold
        )
        print(res)
...
def load_data(
    image_dir: str, batch_size: int, image_size: Tuple[int, int], seed: int
...
def count_examples_per_class(
    ds: tf.data.Dataset, label2cat: Dict[int, str], cat2label: Dict[str, int]
) -> Dict[str, int]:
...
def plot_single_image(image: np.ndarray, title: str):
    """Plot one image.
...
def data_aug_zoom(image_single_batch: tf.Tensor):
    ...
    def _data_aug_zoom(zoom_factor):
        ...
        zoom2 = zoom_factor * 2
        x, y = int(w / 2), int(h / 2)
        img2 = img.crop((x - w / zoom2, y - h / zoom2, x + w / zoom2, y + h / zoom2))
...
def data_aug_random(image_single_batch: tf.Tensor):
    ...
    t1 = widgets.Checkbox(
        value=True, description="RandomFlip", disabled=False, indent=False
    )
    t2 = widgets.Checkbox(
        value=False, description="RandomRotation", disabled=False, indent=False
    )
    t3 = widgets.Checkbox(
        value=False, description="RandomContrast", disabled=False, indent=False
    )
    t4 = widgets.Checkbox(
        value=False, description="RandomZoom", disabled=False, indent=False
    )
...
def get_test_imgs(test_dir: str) -> pd.core.frame.DataFrame:
    ...
    for animal in os.listdir(test_dir):
        files = [
            os.path.join(test_dir, animal, filename)
            for filename in os.listdir(f"{test_dir}/{animal}")
        ]
        for t in files:
...
def pick_img_and_plot_predictions(
    ...
    def _pick_img_and_plot_predictions(animal):
        ...
        filename = (
            test_img_df[test_img_df.animal == animal].sample(1)["filename"].values[0]
        )
        fig, axes = plt.subplots(
            1, 2, figsize=(15, 6), gridspec_kw={"width_ratios": [1, 1], "wspace": 0.5}
        )
        ...
        # Title
        plt.suptitle(
            f"True: {animal}; Prediction: {predictions.loc[0, 'category']}", size=16
        )

    interact(_pick_img_and_plot_predictions, animal=choose_animal_widget)


def get_img_array(img_path: str, image_size: Tuple[int, int]) -> np.ndarray:
...
def plot_original_image(img_array: np.ndarray, ax: plt.Axes):
...
def get_probabilities_for_img_array(
    preds: np.ndarray, label2cat: Dict[int, str]
    ...
    if type(label2cat) is dict:
        for p, l in zip(preds[0], label2cat):
            predictions.append({"category": label2cat[l], "probability": p})
    else:
        preds_list = label2cat(preds, top=20)
        for prediction in preds_list[0]:
            predictions.append(
                {"category": prediction[1], "probability": prediction[2]}
            )
...
def plot_predictions(
    ...
    if topn is None:
        ...
        data = predictions.head(topn)

    palette = [
        "green" if predictions["category"].values[i] == true_animal else "red"
        for i in range(3)
    ]

    f = sns.barplot(x="probability", y="category", data=data, palette=palette, ax=ax)
        ...
        self.data_augmentation = tf.keras.Sequential(
            [
                tf.keras.layers.RandomFlip(
                    "horizontal_and_vertical", input_shape=kwargs["input_shape"]
                ),
...
def plot_cm(y_true: Iterable, y_pred: Iterable, label2cat: Dict[int, str]):
    ...
    cm_normalize_widget = widgets.RadioButtons(
        value=True, options=[True, False], disabled=False, description="Normalize?"
    )
...
def plot_training_history(filename: str):
    ...
    fig.add_trace(
        go.Scatter(
            x=epochs,
            y=history_training["val_accuracy"],
            mode="lines",
    ...
    fig.add_trace(
        go.Scatter(
            x=epochs, y=history_training["loss"], mode="lines", name="Training loss"
        ),
        row=1,
        col=2,
    )
...
def resample_data(
    ...
    label2cat = {
        i: category
        for i, category in enumerate(sorted(next(os.walk(f"{input_dir}/train"))[1]))
    }
    ...
    # First n classes
    labels = np.argsort([count_original[label] for label in cat2label.keys()])[::-1][
        :n_classes
    ]
    ...
    for label in labels:
        shutil.copytree(
            os.path.join(input_dir, "test", label2cat[label]),
            os.path.join(output_dir, "test", label2cat[label]),
        )
        shutil.copytree(
            os.path.join(input_dir, "validation", label2cat[label]),
            os.path.join(output_dir, "validation", label2cat[label]),
        )
        ...
        if count_original[label2cat[label]] >= n_samples_x_class:
            ...
            shutil.copytree(
                os.path.join(input_dir, "train", label2cat[label]),
                os.path.join(output_dir, "train", label2cat[label]),
            )
            ...
            for file in files:
                shutil.copyfile(
                    file,
                    file.replace(input_dir, output_dir).replace(
                        ".JPG", "_" + str(index) + ".JPG"
                    ),
                )
                index += 1
