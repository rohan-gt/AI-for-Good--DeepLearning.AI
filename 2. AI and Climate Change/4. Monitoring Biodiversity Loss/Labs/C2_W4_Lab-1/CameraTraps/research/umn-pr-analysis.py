...
results_file_filtered = os.path.join(
    analysis_base,
    "umn-gomez-reprise-2021-11-272021.11.24-images_detections.filtered_rde_0.60_0.85_10_0.20.json",
)
# results_file = results_file_filtered
results_file = os.path.join(
    analysis_base, "umn-gomez-reprise-2021-11-272021.11.24-images_detections.json"
)
ground_truth_file = os.path.join(
    analysis_base, "images_hv_jan2020_reviewed_force_nonblank.csv"
)
...
deployment_folders = [
    s for s in deployment_folders if os.path.isdir(os.path.join(image_base, s))
]
...
print(
    "Loaded {} MD results from a total of {} (ignored {})".format(
...
print("Loaded {} ground truth annotations".format(len(ground_truth_df)))
...
for deployment_name in tqdm(deployment_folders):
    ...
    for filename in files:
        if (
            deployment_name in detection_only_deployments
            and "detection" not in filename
        ):
        ...
        assert image_name not in file_mappings, (
            "Redundant image name {} in deployment {}".format(
...
print(
    "Processed deployments, ignored {} deployments and {} files".format(
...
for i_row, row in tqdm(ground_truth_df.iterrows(), total=len(ground_truth_df)):
    ...
    assert deployment_name in deployment_folders, (
        "Could not find deployment folder {}".format(deployment_name)
    )
    assert deployment_name in deployment_name_to_file_mappings, (
        "Could not find deployment folder {}".format(deployment_name)
    )
    ...
    assert image_name in file_mappings, (
        "No mappings for image {} in deployment {}".format(image_name, deployment_name)
    )
...
for d in tqdm(ground_truth_dicts):
    if check_blank_consistency:
        ...
        blank_is_consistent = (
            (d["is_blank"] == 1) and (d["common_name"] == "Blank")
...
print(
    "Merged ground truth and MD results, {} images didn't have ground truth".format(
...
def precision_recall_analysis(
    gt_labels, prediction_probs, name, confidence_threshold=0.8, target_recall=0.9
):
    precisions, recalls, thresholds = precision_recall_curve(
        gt_labels, prediction_probs
    )
...
if False:
    ...
    for d in tqdm(merged_images):
        if d["common_name"] == "Human":
            ...
            output_path_absolute = os.path.join(
                human_output_folder, output_relative_path
            )
    ...
    for im in tqdm(merged_images):
        ...
        if im["common_name"] == "Blank" or im["common_name"] == "Human":
            if im["confidences"]["animal"] > fp_threshold:
                ...
                output_path_absolute = os.path.join(
                    fp_output_folder, output_relative_path
                )
                if not os.path.isfile(output_path_absolute):
                    shutil.copyfile(input_path_absolute, output_path_absolute)
        ...
        # GT says this is an animal
            ...
            if im["confidences"]["animal"] < fn_threshold:
                ...
                output_path_absolute = os.path.join(
                    fn_output_folder, output_relative_path
                )
                if not os.path.isfile(output_path_absolute):
                    shutil.copyfile(input_path_absolute, output_path_absolute)
...
if False:
    ...
    convert_output_format.convert_json_to_csv(
        input_path=results_file_filtered,
