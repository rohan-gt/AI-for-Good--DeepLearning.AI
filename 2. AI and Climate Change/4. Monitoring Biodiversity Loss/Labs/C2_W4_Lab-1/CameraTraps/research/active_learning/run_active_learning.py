...
parser.add_argument(
    "--experiment_name", default="", type=str, help="prefix name for output files"
)
parser.add_argument(
    "-N", "--active_batch", default=100, type=int, help="number of queries per batch"
)
parser.add_argument(
    "-A", "--active_budget", default=30000, type=int, help="number of queries per batch"
)
parser.add_argument(
    "--finetuning_P",
    default=16,
...
parser.add_argument(
    "--finetuning_lr", default=0.0001, type=float, help="initial learning rate"
)
parser.add_argument(
...
def main():
    ...
    if args.experiment_name == "":
        args.experiment_name = "experiment_%s_%s" % (
            checkpoint["loss_type"],
    ...
    env = ActiveLearningManager(
        target_dataset, model, device, criterion, args.normalize_embedding
    )
    sampler = None
    N = args.active_batch
    # create a classifier
    classifier = MLPClassifier(
        hidden_layer_sizes=(150, 100), alpha=0.0001, max_iter=2000
    )
    ...
    while numQueries <= args.active_budget:
        ...
        if numQueries == 0:
            ...
            indices = sampler.select_batch(
                N=N, already_selected=env.active_pool, model=classifier
            )
        ...
        if numQueries % 2000 == 1000:
            ...
            sampler = get_AL_sampler(args.active_learning_strategy)(
                env.embedding, None, random.randint(0, sys.maxsize * 2 + 1)
            )
        ...
        print(
            "Number of Queries: %d,  Accuracy: %.4f"
            % (numQueries, classifier.score(X_test, y_test))
        )
        ...
        torch.save(
            ...
            "%s/%s_%04d.pth" % (args.experiment_name, "AL_snapshot", numQueries),
            pickle_protocol=4,
        )
        sys.stdout.flush()
