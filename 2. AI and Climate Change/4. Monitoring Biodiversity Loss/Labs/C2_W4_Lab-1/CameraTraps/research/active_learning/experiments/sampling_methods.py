...
def main():
    ...
    parser.add_argument(
        "--crop_dir",
        type=str,
        help="Path to directory with cropped images to get embedding features for.",
    )
    parser.add_argument(
        "--base_model", type=str, help="Path to latest embedding model checkpoint."
    )
    parser.add_argument(
        "--random_seed",
        default=1234,
    ...
    if mie.seq_num_frames > 1:
        images_in_seq = Image.select(Image.file_name).where(
            (Image.seq_id == mie.seq_id) & (Image.file_name << imagepaths)
        )
    ...
    print(
        "Finished fitting nearest neighbors for whole dataset in %0.2f seconds"
        % (float(time.time() - timer))
    )
