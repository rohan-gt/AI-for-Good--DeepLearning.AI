...
def main():
    ...
    with open(DETECTOR_OUTPUT, "r") as f:
        ...
        for row in data:
            ...
            if (
                "p158" not in imgfile
            ):  # this is for Washington Urban-Wildland Carnivore Project in emammal (Robert Long)
                continue
            # ------------------------------------------------------------------------------------------------------------#
            maxconf = float(row[1])
            ...
            if (
                len(detections) > 1
            ):  # for now, skip if there is more than one detection in the image
                continue
            ...
            imggrayscale = bool(
                np.all(abs(np.mean(img[:, :, 0]) - np.mean(img[:, :, 1])) < 1)
                & (abs(np.mean(img[:, :, 1]) - np.mean(img[:, :, 2])) < 1)
            )
            ...
            imgseqid = int(
                os.path.basename(imgfile).split(".JPG")[0].split("s")[-1].split("i")[0]
            )
            imgseqid_prefix = os.path.basename(imgfile).split(".JPG")[0].split("i")[0]
            imgseqnumframes = len(
                [
                    name
                    for name in os.listdir(os.path.dirname(imgfile))
                    if (
                        os.path.isfile(os.path.join(os.path.dirname(imgfile), name))
                        & (imgseqid_prefix in name)
                    )
                ]
            )
            # ------------------------------------------------------------------------------------------------------------#
            ...
            for box_id in range(len(detections)):
                if (
                    detections[box_id][5] != 1
                ):  # something besides an animal was detected (vehicle, person)
                    continue
                detection_box_pix = detections[box_id][0:4] * np.tile(
                    [imgheight, imgwidth], (1, 2)
                )
                detection_box_pix = detection_box_pix[0]
                detection_box_size = np.vstack(
                    [
                        detection_box_pix[2] - detection_box_pix[0],
                        detection_box_pix[3] - detection_box_pix[1],
                    ]
                ).T

                offsets = (
                    PADDING_FACTOR * np.max(detection_box_size, keepdims=True)
                    - detection_box_size
                ) / 2
                ...
                detection_padded_cropped_img = img[
                    crop_box_pix[0] : crop_box_pix[2], crop_box_pix[1] : crop_box_pix[3]
                ]
                crop_data = []
                crop_id = str(uuid.uuid4())
                crop_fn = os.path.join(CROP_DIR, crop_id + ".JPG")
                crop_rel_size = (
                    (crop_box_pix[2] - crop_box_pix[0])
                    * (crop_box_pix[3] - crop_box_pix[1])
                    / (imgwidth * imgheight)
                )
                ...
                try:
                    ...
                    crops_json[crop_id] = {
                        "id": crop_id,
                        "file_name": crop_fn,
                        "width": detection_padded_cropped_img.shape[1],
                        "height": detection_padded_cropped_img.shape[0],
                        "grayscale": imggrayscale,
                        "relative_size": crop_rel_size,
                        "source_file_name": imgfile,
                        "seq_id": imgseqid,
                        "seq_num_frames": imgseqnumframes,
                        "frame_num": imgframenum,
                        "bbox_confidence": detections[box_id][4],
                        "bbox_X1": detections[box_id][0],
                        "bbox_Y1": detections[box_id][1],
                        "bbox_X2": detections[box_id][2],
                        "bbox_Y2": detections[box_id][3],
                    }
                except ValueError:
                    continue
            ...
            if counter % 100 == 0:
                print(
                    "Processed crops for %d out of %d images in %0.2f seconds"
                    % (counter, num_images, time.time() - timer)
                )
