...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path to a directory containing full-sized images in the dataset.",
    )
    parser.add_argument(
        "--coco_json",
        required=True,
        type=str,
        help="Path to COCO JSON file for the dataset.",
    )
    parser.add_argument(
        "--crop_dir",
        required=True,
        type=str,
        help="Path to output directory for crops.",
    )
    parser.add_argument(
        "--padding_factor",
        type=float,
    ...
    bboxes_available = any(
        [("bbox" in a.keys()) for a in coco_json_data["annotations"]]
    )
    assert bboxes_available, (
        "COCO JSON does not contain bounding boxes, need to run a detector first."
    )
    ...
    for ann in coco_json_data["annotations"]:
        ...
        detection_padded_cropped_img = img[
            crop_box_pix[1] : crop_box_pix[3], crop_box_pix[0] : crop_box_pix[2]
        ]
        ...
        crops_json[crop_id] = {
            ...
            "seq_id": image_seq_id,
            "seq_num_frames": image_seq_num_frames,
            "frame_num": image_frame_num,
            "bbox_confidence": detection_conf,
            "bbox_X1": int(crop_box_pix[1]),
            "bbox_Y1": int(crop_box_pix[0]),
            "bbox_X2": int(crop_box_pix[3]),
            "bbox_Y2": int(crop_box_pix[2]),
        }

        if crop_counter % 100 == 0:
            print(
                "Produced crops for %d out of %d detections in %0.2f seconds."
                % (
                    crop_counter,
                    len(coco_json_data["annotations"]),
                    time.time() - timer,
                )
            )
