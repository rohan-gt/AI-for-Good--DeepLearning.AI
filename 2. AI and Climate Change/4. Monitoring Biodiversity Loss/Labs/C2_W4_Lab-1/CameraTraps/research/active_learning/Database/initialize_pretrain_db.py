...
parser = argparse.ArgumentParser(
    description="Initialize a PostgreSQL database for a dataset of camera trap images to use for active learning for classification."
)
parser.add_argument(
    "--db_name",
...
parser.add_argument(
    "--verbose", default=False, type=bool, help="Output print messages while running"
)
parser.add_argument(
    "--coco_json",
    metavar="DIR",
    help="Path to COCO Camera Traps json file if available",
    default=None,
)
...
pretrain_db = PostgresqlDatabase(
...
try:
    ...
    info_entry = Info.create(
        name=info_name,
        description=info_desc,
        contributor=info_contrib,
        version=info_version,
        year=info_year,
        date_created=info_date,
    )
    info_entry.save()
...
if sys.version_info >= (3, 5):
    ...
    classes = [
        d
        for d in os.listdir(args.crop_dir)
        if os.path.isdir(os.path.join(args.crop_dir, d))
    ]
classes.sort()
    ...
    for root, _, fnames in sorted(os.walk(os.path.join(args.crop_dir, cat))):
        for i, fname in enumerate(sorted(fnames)):
            if fname.endswith(".JPG"):
                ...
                if (abs(np.mean(img[:, :, 0]) - np.mean(img[:, :, 1])) < 1e-1) & (
                    abs(np.mean(img[:, :, 1]) - np.mean(img[:, :, 2])) < 1e-1
                ):
                    img_grayscale = True
                else:
                    img_grayscale = False
                if len(fname.split(".JPG")[-2].split("_")) > 1:
                    img_source_fname = os.path.join(
                        args.image_dir,
                        os.path.basename(root),
                        fname.split(".JPG")[-2].split("_")[0] + ".JPG",
                    )
                else:
                    img_source_fname = os.path.join(
                        args.image_dir, os.path.basename(root), fname
                    )

                orig_img = np.array(PIL.Image.open(img_source_fname))
                orig_img_width = orig_img.shape[1]
                orig_img_height = orig_img.shape[0]
                img_relative_size = (img_height * img_width) / float(
                ...
                existing_image_entries = Image.select().where(
                ...
                try:
                    ...
                    image_entry = Image.create(
                        id=img_uid,
                        file_name=img_fname,
                        width=img_width,
                        height=img_height,
                        grayscale=img_grayscale,
                        source_file_name=img_source_fname,
                        relative_size=img_relative_size,
                    )
                    image_entry.save()
                    ...
                    detection_entry = Detection.create(
                        id=detection_uid,
                        image=detection_img,
                        kind=detection_kind,
                        category=detection_cat,
                        category_confidence=detection_cat_conf,
                    )
                    ...
                    oracle_entry = Oracle.create(
                        detection=detection_uid, label=detection_cat
                    )
