...
class_list = ["empty"] + sorted(
    [
        d
        for d in os.listdir(args.coco_clf_crop_dir)
        if os.path.isdir(os.path.join(args.coco_clf_crop_dir, d))
    ]
)
...
with open(
    os.path.join(args.coco_clf_crop_dir, "detections_final.pkl"), "rb"
) as pklfile:
...
for class_name in sorted(
    ...
    files = [
        f.split(os.path.join(args.coco_clf_crop_dir, class_name) + "/")[1]
        for f in glob.glob(
            os.path.join(args.coco_clf_crop_dir, class_name, "**/*.JPG"), recursive=True
        )
    ]
    for f in files:
