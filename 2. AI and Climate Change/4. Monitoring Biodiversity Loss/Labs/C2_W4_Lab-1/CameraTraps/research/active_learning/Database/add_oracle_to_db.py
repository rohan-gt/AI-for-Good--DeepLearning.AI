...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to .json file with COCO formatted information about detections.",
    )
    parser.add_argument(
        "--img_base", type=str, help="Path to add as prefix to filenames in COCO json."
    )
    parser.add_argument(
        "--class_list",
        type=str,
    ...
    coco_categories = {
        cat["id"]: cat["name"].replace("_", " ") for cat in coco_json["categories"]
    }
    coco_imgid_to_fn = {
        im["id"]: os.path.join(args.img_base, im["file_name"].replace("\\", "/"))
    ...
    for crop in crop_json:
        ...
        crop_class = coco_imgfn_to_catname[
            os.path.join(args.img_base, source_image_file_name)
        ]
        ...
        if counter % 100 == 0:
            print(
                "Updated database with Oracle table entries for %d out of %d detections in %0.2f seconds"
                % (counter, len(crop_json), time.time() - timer)
            )
