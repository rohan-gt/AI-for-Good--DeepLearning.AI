...
class ActiveLearningManager(object):
    ...
    # gather test set
    def getTestSet(self):
        return self.embedding[self.default_pool], np.asarray(
            [
    ...
    def getTrainSet(self):
        return self.embedding[self.active_pool], np.asarray(
            [self.dataset.samples[self.dataset.indices[x]][1] for x in self.active_pool]
        )
    ...
    def finetune_embedding(self, epochs, P, K, lr, num_workers=10):
        train_loader = self.dataset.getBalancedLoader(
            P=P, K=K, num_workers=num_workers, sub_indices=self.active_pool
        )
    ...
    def get_pools(self):
        return {
            ...
            "active_pool": [
                self.dataset.samples[self.dataset.indices[x]] for x in self.active_pool
            ],
            "default_pool": [
                self.dataset.samples[self.dataset.indices[x]] for x in self.default_pool
            ],
        }
