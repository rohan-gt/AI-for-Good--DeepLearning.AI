...
class ExtendedImageFolder(ImageFolder):
    ...
    def get_transform(self, trns_mode):
        ...
        if trns_mode == "train":
            ...
            transform_list.append(
                RandomOrder([RandomHorizontalFlip(), ColorJitter(), RandomRotation(20)])
            )
        else:
    ...
    def getBalancedLoader(
        self, P=10, K=10, num_workers=4, sub_indices=None, transfm="train"
    ):
        ...
        if sub_indices is not None:
            ...
            return DataLoader(
                subset, batch_sampler=train_batch_sampler, num_workers=num_workers
            )
        train_batch_sampler = BalancedBatchSampler(self, n_classes=P, n_samples=K)
        return DataLoader(
            self, batch_sampler=train_batch_sampler, num_workers=num_workers
        )
...
class BalancedBatchSampler(BatchSampler):
    ...
    def __init__(self, underlying_dataset, n_classes, n_samples):
        if hasattr(underlying_dataset, "dataset"):
            self.labels = [
                underlying_dataset.dataset.samples[
                    underlying_dataset.dataset.indices[i]
                ][1]
                for i in underlying_dataset.indices
            ]
        else:
            self.labels = [
                underlying_dataset.samples[i][1] for i in underlying_dataset.indices
            ]
