...
class HierarchicalClusterAL(SamplingMethod):
    ...
    def get_node_upper_lower_bounds(self, node):
        ...
        delta = 1.0 / n_v + np.sqrt(p_v * (1 - p_v) / (1.0 * n_v))
        return (
            np.maximum(p_v - delta, np.zeros(len(p_v))),
            ...
            for v in node_list:
                ...
                if v >= self.n_leaves:  # v is not a leaf
                    if len(admissable_indices) > 0:  # There exists an admissible label
                        ...
                        admissible_ancestors = [
                            len(self.admissible[a]) > 0
                            for a in self.tree.get_ancestor(node)
                        ]
                        if all(admissible_ancestors):
                            ...
                            split_score = (
                                node_counts[1] / node_counts[0] * left_node.score
                                + node_counts[2] / node_counts[0] * right_node.score
                            )
    ...
    def select_batch_(self, N, already_selected, labeled, y, **kwargs):
        ...
        print("Actual impurity for pruning is: %.2f" % (self.get_pruning_impurity(y)))
