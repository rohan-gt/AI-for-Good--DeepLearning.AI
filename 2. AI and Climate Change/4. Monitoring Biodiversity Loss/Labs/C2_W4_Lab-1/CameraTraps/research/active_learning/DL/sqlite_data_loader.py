...
class SQLDataLoader(Dataset):
    ...
    def __init__(
        ...
        if query is None:
            self.query = (
                Detection.select(Detection.id, Oracle.label, Detection.kind)
                .join(Oracle)
                .order_by(fn.random())
                .limit(limit)
            )
        ...
        self.set_indices = [
            ...
            [],
            [],
        ]  # indices with ActiveDetection, ModelDetection, ConfirmedDetection, UserDetection, and all user labeled/confirmed detections (ConfirmedDetection U UserDetection)
    ...
    def updateEmbedding(self, model):
        ...
        self.set_kind(
            self.kind
        )  # revert current_set to only samples of specified detection kind
    ...
    def __getitem__(self, idx):
        ...
        if (
            target is None
        ):  # to get getSingleLoader to work when using Detection.category (not Oracle) as label, and there are no labels for the current sample set
            target = -1
    ...
    def getpaths(self):
        ...
        return [
            os.path.join(self.img_base, self.samples[i][0] + ".JPG")
    ...
    def getBalancedLoader(self, P=14, K=10):
        ...
        return DataLoader(
            self, batch_sampler=train_batch_sampler, num_workers=self.num_workers
        )
...
class BalancedBatchSampler(BatchSampler):
    ...
    def __init__(self, underlying_dataset, n_classes, n_samples):
        self.labels = [
            underlying_dataset.samples[i][1] for i in underlying_dataset.current_set
        ]
