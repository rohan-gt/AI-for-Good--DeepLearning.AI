...
def plot_together(embedd, labels, preds, ind, paths, info):
    ...
    colors = [
        indexcolors[int(i) % len(indexcolors)]
        for i in np.equal(labels, preds).astype(np.int32).squeeze()
    ]
    ...
    def onpick3(event):
        if event.button == 1:
            ...
            for thumb in ind["ind"]:
                ...
                ab = AnnotationBbox(
                    img,
                    (embedding[thumb, 0] + 0.2, embedding[thumb, 1] + 0.2),
                    xycoords="data",
                    frameon=False,
                )
                ax.add_artist(ab)
    ...
    for i, label in enumerate(legend_texts):
        if np.count_nonzero(np.equal(labels, i)) > 0:
            ...
            color = (
                "black"
                if 1
                - (int(r, 16) * 0.299 + int(g, 16) * 0.587 + int(b, 16) * 0.114) / 255
                < 0.5
                else "white"
            )
            plt.annotate(
                label,
                np.median(embedding[labels == i], axis=0),
...
def plot_embedding_images(embedd, labels, paths, info, savefile):
    ...
    for i, thumb in enumerate(paths):
        ...
        ab = AnnotationBbox(
            img,
            (embedding[i, 0] + 0.3, embedding[i, 1] + 0.3),
            xycoords="data",
            frameon=False,
        )
        ax.add_artist(ab)
