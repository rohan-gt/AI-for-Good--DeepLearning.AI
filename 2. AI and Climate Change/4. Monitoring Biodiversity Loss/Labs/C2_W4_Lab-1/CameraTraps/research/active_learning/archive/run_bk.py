...
def train_eval_classifier(clf_model, unlabeled_dataset, model, pth, epochs=15):
    trainset_query = (
        Detection.select(Detection.id, Oracle.label)
        .join(Oracle)
        .where(Detection.kind == DetectionKind.UserDetection.value)
    )
    train_dataset = SQLDataLoader(
        trainset_query, os.path.join(args.run_data, "crops"), is_training=True
    )
...
def main():
    ...
    unlabeled_dataset = SQLDataLoader(
    ...
    random_ids = np.random.choice(
        unlabeled_dataset.getIDs(), 5000, replace=False
    ).tolist()
    ...
    plot_embedding(
        train_dataset.em,
        np.asarray(train_dataset.getlabels()),
    ...
    unlabeledset_query = (
        Detection.select(Detection.id, Oracle.label)
        .join(Oracle)
        .where(Detection.kind == DetectionKind.ModelDetection.value)
        .order_by(fn.random())
        .limit(20000)
    )
    unlabeled_dataset = SQLDataLoader(
