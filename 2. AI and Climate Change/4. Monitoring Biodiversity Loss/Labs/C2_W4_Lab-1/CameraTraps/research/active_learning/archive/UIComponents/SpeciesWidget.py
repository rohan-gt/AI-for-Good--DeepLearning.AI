    ...
    def validate(self):
        ...
        for row in range(allRows):
            ...
            if rowname is None or rowname in name_set:
                return "There is an issue with %s at row %d" % (
                    rowname,
                    rowid,
                ), itemsList
            if (
                rowshortname is None
                or rowshortname in shortname_set
                or len(rowshortname) > 2
            ):
                return "There is an issue with %s at row %d" % (
                    rowshortname,
                    rowid,
    ...
    def syncSpecies(self, event):
        ...
        if val is None:
            ...
            Category.delete().where(
                Category.id.not_in([x[0] for x in itemsList])
            ).execute()
            msg = QMessageBox()
