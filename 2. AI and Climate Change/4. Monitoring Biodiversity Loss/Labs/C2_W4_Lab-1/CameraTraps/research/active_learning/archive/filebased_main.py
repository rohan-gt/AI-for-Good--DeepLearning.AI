...
parser.add_argument(
    "--val_data", metavar="DIR", help="path to validation dataset", default=None
)
parser.add_argument(
    "--arch",
    "-a",
...
parser.add_argument("--loss_type", default="triplet", help="Loss Type")
parser.add_argument(
    "--margin",
    default=1.0,
    type=float,
...
def main():
    ...
    if args.val_data is not None:
        val_dataset = BaseDataLoader(
            args.val_data, False, batch_size=args.batch_size, num_workers=args.workers
        )
    ...
    if args.loss_type.lower() == "center":
        ...
        train_loader = train_dataset.getBalancedLoader(
            P=args.balanced_P, K=args.balanced_K
        )
    ...
    if args.loss_type.lower() == "center":
        model = torch.nn.DataParallel(
            ClassificationNet(embedding_net, n_classes=num_classes)
        ).cuda()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        e.train_one_epoch(
            train_loader, epoch, True if args.loss_type.lower() == "center" else False
        )
        # evaluate on validation set
        if args.val_data is not None:
            e.validate(
                val_loader, True if args.loss_type.lower() == "center" else False
            )
        save_checkpoint(
            {
                "epoch": epoch + 1,
