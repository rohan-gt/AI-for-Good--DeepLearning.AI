...
def completeLoop(X, y):
    ...
    for it in range(9):
        ...
        # diff= all_indices.difference(base_ind)
        print(
            "Iteration %d, Accuracy %.3f" % (it, clf.score(X, y))
        )  # [list(diff)],y[list(diff)])))
        preds = clf.predict_proba(X)
        preds_tr = clf.predict_proba(X_train)
        # conf.update(preds_tr,y_train)
        # classes= np.apply_along_axis(conf.classScore,0,preds.argmax(axis=1))
...
def main():
    ...
    run_dataset = BaseDataLoader(
        args.run_data, False, num_workers=args.workers, batch_size=args.batch_size
    )
    ...
    if checkpoint["loss_type"].lower() == "center":
        model = torch.nn.DataParallel(
            ClassificationNet(embedding_net, n_classes=14)
        ).cuda()
    else:
    ...
    with torch.no_grad():
        ...
        for images, target, path in dataloader:
            ...
            embeddings[k : k + len(images)] = (
                embedding.data.cpu().numpy().reshape((len(images), -1))
            )
