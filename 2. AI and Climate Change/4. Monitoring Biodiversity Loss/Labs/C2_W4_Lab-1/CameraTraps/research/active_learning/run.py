...
parser = argparse.ArgumentParser(
    description="PyTorch Classifier Training with Active Learning"
)
parser.add_argument(
    "--db_name",
...
parser.add_argument(
    ...
    type=str,
    metavar="PATH",
    help="Path to latest checkpoint for embedding model (default: none).",
)
parser.add_argument(
    "--experiment_name",
    default="",
...
parser.add_argument(
    ...
    type=str,
    metavar="PATH",
    help="Path to cropped image files.",
)
parser.add_argument(
    "--db_query_limit", default=50000, type=int, help="Max number of records to read."
)
parser.add_argument(
    "--strategy", default="confidence", type=str, help="Active learning query strategy."
)
parser.add_argument(
    "--num_classes", default=50, type=int, help="Number of species in the dataset."
)
parser.add_argument(
    "-j",
    "--workers",
...
def main():
    ...
    dataset_query = (
        Detection.select(Detection.image_id, Oracle.label, Detection.kind)
        .join(Oracle)
        .order_by(fn.random())
    ...
    while numLabeled <= args.active_budget:
        ...
        if numLabeled == 0:
            indices = np.random.choice(
                dataset.current_set, kwargs["N"], replace=False
            ).tolist()
        else:
        ...
        joblib.dump(
            kwargs["model"],
            "%s/%s_%04d.skmodel" % (args.experiment_name, "classifier", numLabeled),
        )
