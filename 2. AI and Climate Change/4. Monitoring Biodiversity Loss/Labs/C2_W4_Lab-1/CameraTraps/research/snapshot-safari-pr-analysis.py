...
if True:
    results_file_all = os.path.expanduser(
        "~/postprocessing/"
...
if True:
    ...
    required_prefix = "KGA"
    ground_truth_file = os.path.expanduser(
        "~/postprocessing/snapshot-safari/"
...
if False:
    ...
    required_prefix = "KRU"
    ground_truth_file = os.path.expanduser(
        "~/postprocessing/snapshot-safari/" + "ground_truth/SnapshotKruger_S1_v1.0.json"
    )
...
if False:
    ...
    required_prefix = "KAR"
    ground_truth_file = os.path.expanduser(
        "~/postprocessing/snapshot-safari/" + "ground_truth/SnapshotKaroo_S1_v1.0.json"
    )
...
print(
    "\nMatched {} of {} MD results".format(
        len(results_images_filtered), len(results_all["images"])
    )
)
...
assert md_results["detection_categories"] == {
    "1": "animal",
    "2": "person",
    "3": "vehicle",
}
...
ground_truth_category_id_to_name = {
    c["id"]: c["name"] for c in ground_truth_all["categories"]
}
...
print(
    "\nMatched {} of {} ground truth images".format(
        len(gt_images_filtered), len(ground_truth_all["images"])
    )
)
...
for seq_id in sequence_id_to_images:
    if (
        seq_id not in sequence_id_to_confidence_values
        or sequence_id_to_confidence_values[seq_id] is None
    ):
        continue
    ...
    category_ids_this_sequence = set(
        [ann["category_id"] for ann in annotations_this_sequence]
    )
...
if False:
    ...
    print(
        "Found {} sequence-level false negatives".format(len(false_negative_sequences))
    )
