...
serialized_structures_file = os.path.join(
    taxonomy_download_dir, "serialized_taxonomies.p"
)
...
def initialize_taxonomy_lookup(force_init=False) -> None:
    ...
    * Skips all of the above if the serialized pickle file already exists
    ...
    global inat_taxonomy, gbif_taxonomy, gbif_common_mapping, inat_taxon_id_to_row, gbif_taxon_id_to_row, inat_taxon_id_to_vernacular, inat_vernacular_to_taxon_id, inat_taxon_id_to_scientific, inat_scientific_to_taxon_id, gbif_taxon_id_to_vernacular, gbif_vernacular_to_taxon_id, gbif_taxon_id_to_scientific, gbif_scientific_to_taxon_id
    ...
    # Load iNat taxonomy
    inat_taxonomy = pd.read_csv(
        os.path.join(taxonomy_download_dir, "iNaturalist", "taxa.csv")
    )
    inat_taxonomy["scientificName"] = (
        inat_taxonomy["scientificName"].fillna("").str.strip()
    )
    inat_taxonomy["vernacularName"] = (
        inat_taxonomy["vernacularName"].fillna("").str.strip()
    )

    # Load GBIF taxonomy
    gbif_taxonomy = pd.read_csv(
        os.path.join(taxonomy_download_dir, "GBIF", "Taxon.tsv"), sep="\t"
    )
    gbif_taxonomy["scientificName"] = (
        gbif_taxonomy["scientificName"].fillna("").str.strip()
    )
    gbif_taxonomy["canonicalName"] = (
        gbif_taxonomy["canonicalName"].fillna("").str.strip()
    )

    # Remove questionable rows from the GBIF taxonomy
    gbif_taxonomy = gbif_taxonomy[
        ~gbif_taxonomy["taxonomicStatus"].isin(["doubtful", "misapplied"])
    ]
    gbif_taxonomy = gbif_taxonomy.reset_index()

    # Load GBIF vernacular name mapping
    gbif_common_mapping = pd.read_csv(
        os.path.join(taxonomy_download_dir, "GBIF", "VernacularName.tsv"), sep="\t"
    )
    gbif_common_mapping["vernacularName"] = (
        gbif_common_mapping["vernacularName"].fillna("").str.strip()
    )

    # Only keep English mappings
    gbif_common_mapping = gbif_common_mapping.loc[
        gbif_common_mapping["language"] == "en"
    ]
    ...
    for i_row, row in tqdm(
        gbif_common_mapping.iterrows(), total=len(gbif_common_mapping)
    ):
        taxon_id = row["taxonID"]
...
def traverse_taxonomy(
    ...
    for i_match in matching_rownums:
        ...
        while True:
            ...
            vernacular_names = sorted(
                taxon_id_to_vernacular[taxon_id]
            )  # sort for determinism, pylint: disable=line-too-long
            match_details.append(
                (
                    taxon_id,
                    current_row["taxonRank"],
...
def get_preferred_taxonomic_match(
    query: str, taxonomy_preference="inat", retry=True
) -> TaxonomicMatch:
    """
    ...
    m, query = _get_preferred_taxonomic_match(
        query=query, taxonomy_preference=taxonomy_preference
    )
...
def _get_preferred_taxonomic_match(
    query: str, taxonomy_preference="inat"
) -> TaxonomicMatch:
    ...
    return TaxonomicMatch(
        scientific_name, common_name, taxonomic_level, source, taxonomy_string, match
    ), query
