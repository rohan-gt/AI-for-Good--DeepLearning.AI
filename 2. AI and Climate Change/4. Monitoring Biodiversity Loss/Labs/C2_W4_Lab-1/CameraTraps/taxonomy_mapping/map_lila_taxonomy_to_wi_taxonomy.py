...
for taxon in tqdm(wi_taxonomy):
    ...
    if not is_empty_wi_item(taxon["species"]):
        ...
        taxon_name = (
            (taxon["genus"].strip() + " " + taxon["species"].strip()).strip().lower()
        )
        assert (
            not is_empty_wi_item(taxon["class"])
            and not is_empty_wi_item(taxon["order"])
            and not is_empty_wi_item(taxon["family"])
        )
        ...
        assert not is_empty_wi_item(taxon["class"]) and not is_empty_wi_item(
            taxon["order"]
        )
...
taxon_name_to_preferred_taxon_id["homo sapiens"] = (
    2002045  # multiple sensible human entries
)
...
taxon_name_to_preferred_taxon_id["canis familiaris"] = (
    2021548  # "domestic dog" and "dog-on-leash"
)
taxon_name_to_preferred_taxon_id["cervus canadensis"] = (
    2021592  # "domestic elk" and "elk"
)
taxon_name_to_preferred_taxon_id["muridae"] = (
    2021642  # different interpretations of "muridae"
)
taxon_name_to_preferred_taxon_id["stagonopleura bella"] = 2021939
taxon_name_to_preferred_taxon_id["bison bison"] = (
    2021593  # "American bison" vs. "domestic bison"
)
taxon_name_to_preferred_taxon_id["meleagris gallopavo"] = (
    2021598  # "domestic turkey" vs. "wild turkey"
)
...
taxon_name_to_preferred_taxon_id["motacilla flava"] = (
    2016194  # "yellow wagtail" vs. "yellow crowned-wagtail"
)
...
for taxon_name in taxon_name_to_preferred_taxon_id.keys():
    ...
    selected_taxa = [
        ...
        if t["id"] == taxon_name_to_preferred_taxon_id[taxon_name]
    ]
...
for i_taxon, lila_taxon in enumerate(lila_taxonomy):
    ...
    if query in unknown_queries:
        ...
        if lila_common_name in wi_common_name_to_taxon:
            ...
            if False:
                print(
                    "LILA common name {} maps to WI taxon {} ({})".format(
                        lila_common_name, wi_taxon_name, wi_common_name
                    )
                )
...
print(
    "Of {} entities, there are {} mismatches ({} mapped by common name) ({} mapped by supplementary mapping file)".format(
        len(all_searches),
        len(mismatches),
        len(mismatches_with_common_mappings),
        len(supplementary_mappings),
    )
)
...
with open(wi_mapping_table_file, "w") as f:
    f.write(
        "lila_dataset_name,lila_category_name,wi_guid,wi_taxon_name,wi_common,count\n"
    )
    ...
    for dataset_name in lila_dataset_to_categories.keys():
        ...
        for category in dataset_categories:
            lila_dataset_category = (
                dataset_name + ":" + category["name"].strip().lower()
            )
            ...
            # and count
            s = (
                f"{dataset_name},{category['name']},{wi_taxon['uniqueIdentifier']},"
