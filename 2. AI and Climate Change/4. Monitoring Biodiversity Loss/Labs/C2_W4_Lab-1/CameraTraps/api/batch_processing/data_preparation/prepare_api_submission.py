...
class Task:
    ...
    def __repr__(self) -> str:
        return "Task(name={name}, id={id})".format(
            name=self.name, id=getattr(self, "id", None)
        )
    ...
    def upload_images_list(
        ...
        self.remote_images_list_url = ai4e_azure_utils.upload_file_to_blob(
            ...
            local_path=self.local_images_list_path,
            blob_name=blob_name,
            sas_token=sas_token,
            overwrite=overwrite,
        )
    ...
    def get_missing_images(self, submitted_images, verbose: bool = False) -> List[str]:
        ...
        return get_missing_images_from_json(
            submitted_images, detections, n_failed_shards, verbose
        )
...
def create_response_message(n_failed_shards, detections_url, task_id):
    ...
    """
    output_file_urls = {"detections": detections_url}
    message = {
        "num_failed_shards": n_failed_shards,
...
def get_missing_images_from_json(
    submitted_images, detections, n_failed_shards, verbose=False
):
    """
    ...
    non_image_files_submitted = [
        s for s in submitted_images if not is_image_file_or_url(s)
    ]
    if len(non_image_files_submitted) > 0:
        print(
            "Warning, {} non-image files submitted:\n".format(
                len(non_image_files_submitted)
            )
        )
    ...
    if verbose:
        ...
        print(
            f"{n_failed_shards} failed shards "
...
def divide_files_into_tasks(
    file_list_json: str, n_files_per_task: int = MAX_FILES_PER_API_TASK
) -> Tuple[List[str], List[Sequence[Any]]]:
    """
    ...
    return divide_list_into_tasks(
        file_list, save_path=file_list_json, n_files_per_task=n_files_per_task
    )


def clean_request_name(
    request_name: str,
    whitelist: str = VALID_REQUEST_NAME_CHARS,
    char_limit: int = REQUEST_NAME_CHAR_LIMIT,
) -> str:
    """
...
if False:
    ...
    request_strings, request_dicts = generate_api_queries(
        input_container_sas_url, file_list_sas_urls, request_name_base, caller
    )
