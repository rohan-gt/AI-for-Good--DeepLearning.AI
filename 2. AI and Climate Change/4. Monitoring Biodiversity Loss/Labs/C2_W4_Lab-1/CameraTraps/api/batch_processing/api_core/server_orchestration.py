...
def create_batch_job(job_id: str, body: dict):
    ...
    try:
        ...
        job_name = body.get(
            "request_name", ""
        )  # in earlier versions we used "request" to mean a "job"
        ...
        # Case 2: user supplied a list of images to process; can include metadata
            ...
            )  # could be a file hosted anywhere
            image_paths = response.json()

            log.info(
                "server_job, create_batch_job, length of image_paths provided by the user: {}".format(
                    len(image_paths)
                )
            )
            ...
            for p in image_paths:
                ...
                if image_path_prefix is not None and not locator.startswith(
                    image_path_prefix
                ):
                    continue
            ...
            log.info(
                (
                    "server_job, create_batch_job, length of image_paths provided by user, "
        ...
        if sample_n:
            ...
            if sample_n > len(image_paths):
                msg = (
                    "parameter sample_n specifies more images than "
        ...
        log.info(
            f"server_job, create_batch_job, num_images after applying all filters: {num_images}"
        )
        ...
        images_list_str_as_bytes = bytes(
            json.dumps(image_paths, ensure_ascii=False), encoding="utf-8"
        )
        ...
        with ContainerClient.from_container_url(
            container_url, credential=api_config.STORAGE_ACCOUNT_KEY
        ) as api_container_client:
        ...
        job_status = get_job_status(
            "created", f"{num_images} images listed; submitting the job..."
        )
        job_status_table.update_job_status(job_id, job_status)

    except Exception as e:
        job_status = get_job_status(
            "failed", f"Error occurred while preparing the Batch job: {e}"
        )
        job_status_table.update_job_status(job_id, job_status)
        log.error(
            f"server_job, create_batch_job, Error occurred while preparing the Batch job: {e}"
        )
    ...
    try:
        ...
        batch_job_manager.create_job(
            job_id, model_rel_path, input_container_sas, use_url
        )

        num_tasks, task_ids_failed_to_submit = batch_job_manager.submit_tasks(
            job_id, num_images
        )
        ...
        job_status = get_job_status(
            "problem",
            f"Please contact us. Error occurred while submitting the Batch job: {e}",
        )
        job_status_table.update_job_status(job_id, job_status)
        log.error(
            f"server_job, create_batch_job, Error occurred while submitting the Batch job: {e}"
        )
        return
    ...
    try:
        ...
        job_status = get_job_status(
            "problem", f"Error occurred while starting the monitoring thread: {e}"
        )
        job_status_table.update_job_status(job_id, job_status)
        log.error(
            f"server_job, create_batch_job, Error occurred while starting the monitoring thread: {e}"
        )
        return
...
def monitor_batch_job(
    ...
    try:
        ...
        while True:
            ...
            num_tasks_succeeded, num_tasks_failed = (
                batch_job_manager.get_num_completed_tasks(job_id)
            )
            job_status = get_job_status(
                "running",
                (
            ...
            if num_checks > api_config.MAX_MONITOR_CYCLES:
                ...
                job_status_table.update_job_status(job_id, job_status)
                log.warning(
                    f"server_job, create_batch_job, MAX_MONITOR_CYCLES reached, ending thread"
                )
                break  # still aggregate the Tasks' outputs

    except Exception as e:
        job_status = get_job_status(
            "problem", f"Error occurred while monitoring the Batch job: {e}"
        )
        job_status_table.update_job_status(job_id, job_status)
        log.error(
            f"server_job, create_batch_job, Error occurred while monitoring the Batch job: {e}"
        )
        return

    try:
        output_sas_url = aggregate_results(
            job_id, model_version, job_name, job_submission_timestamp
        )
        ...
        msg = {
            ...
            "output_file_urls": {"detections": output_sas_url},
        }
        ...
        job_status_table.update_job_status(job_id, job_status)
        log.error(
            f"server_job, create_batch_job, Error occurred while aggregating results: {e}"
        )
        return


def aggregate_results(
    job_id: str, model_version: str, job_name: str, job_submission_timestamp: str
) -> str:
    ...
    with ContainerClient.from_container_url(
        ...
        if blob_client.exists():
            log.warning(
                f"The output file already exists, likely because another monitoring thread already wrote it."
            )
        else:
            task_outputs_dir = (
                f"api_{api_config.API_INSTANCE_NAME}/job_{job_id}/task_outputs/"
            )
            ...
            api_output_as_bytes = bytes(
                json.dumps(api_output, ensure_ascii=False, indent=1), encoding="utf-8"
            )
            _ = container_client.upload_blob(
                name=output_file_path, data=api_output_as_bytes
            )
