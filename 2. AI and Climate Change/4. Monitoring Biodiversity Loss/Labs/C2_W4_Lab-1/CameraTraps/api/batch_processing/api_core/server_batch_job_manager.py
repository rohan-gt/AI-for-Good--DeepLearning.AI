...
class BatchJobManager:
    ...
    def create_job(
        self,
        job_id: str,
        detector_model_rel_path: str,
        input_container_sas: str,
        use_url: bool,
    ):
        ...
        job = JobAddParameter(
            ...
            common_environment_settings=[
                EnvironmentSetting(
                    name="DETECTOR_REL_PATH", value=detector_model_rel_path
                ),
                EnvironmentSetting(
                    name="API_INSTANCE_NAME", value=api_config.API_INSTANCE_NAME
                ),
                EnvironmentSetting(name="JOB_CONTAINER_SAS", value=input_container_sas),
                EnvironmentSetting(name="JOB_USE_URL", value=str(use_url)),
                EnvironmentSetting(
                    name="DETECTION_CONF_THRESHOLD",
                    value=api_config.DETECTION_CONF_THRESHOLD,
                ),
            ],
        )
    ...
    def submit_tasks(self, job_id: str, num_images: int) -> Tuple[int, list]:
        ...
        access_duration_hrs = (
            api_config.MONITOR_PERIOD_MINUTES * api_config.MAX_MONITOR_CYCLES / 60
        )
        ...
        if len(task_ids_failed_to_submit) > 0:
            ...
            task_ids_failed_to_submit = self._create_tasks(
                job_id, tasks_to_retry, api_config.NUM_TASKS_PER_RESUBMISSION, 2
            )
    ...
    def _create_tasks(self, job_id, tasks, num_tasks_per_submission, n_th_try) -> list:
        ...
        for i in range(0, len(tasks), num_tasks_per_submission):
            ...
            collection_results = self.batch_client.task.add_collection(
                job_id, tasks_to_submit, threads=10
            )
            ...
            for task_result in collection_results.value:
                if task_result.status is not TaskAddStatus.success:
                    ...
                    log.info(
                        f"task {task_result.task_id} failed to submitted after {n_th_try} try/tries, "
    ...
    def get_num_completed_tasks(self, job_id: str) -> Tuple[int, int]:
        ...
        tasks = self.batch_client.task.list(
            job_id,
    ...
    def get_num_active_jobs(self) -> int:
        jobs_generator = self.batch_client.job.list(
            job_list_options=JobListOptions(filter="state eq 'active'", select="id")
        )
