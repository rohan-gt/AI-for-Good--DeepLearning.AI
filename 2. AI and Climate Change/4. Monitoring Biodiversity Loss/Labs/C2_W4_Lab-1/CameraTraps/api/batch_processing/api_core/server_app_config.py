...
class AppConfig:
    ...
    def __init__(self):
        ...
        self.sentinel = (
            self._get_sentinel()
        )  # get initial sentinel and allowlist values
    ...
    def _get_allowlist(self):
        filtered_listed = self.client.list_configuration_settings(
            key_filter="batch_api_allow:*"
        )
        allowlist = []
    ...
    def get_allowlist(self):
        try:
            ...
            log.error(
                f"AppConfig, get_allowlist, exception so using old allowlist: {e}"
            )
            return self.allowlist
