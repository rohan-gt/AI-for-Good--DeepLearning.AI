...
def check_data_container_sas(input_container_sas: str) -> Optional[Tuple[int, str]]:
    ...
    msg = "input_container_sas provided does not have both read and list permissions."
...
def get_job_status(request_status: str, message: Any) -> dict:
    return {"request_status": request_status, "message": message}


def validate_provided_image_paths(
    ...
    if isinstance(first_item, str):
        ...
        for i in image_paths:
            if len(i) != 2:  # i should be [image_id, metadata_string]
                error = (
                    "Items in image_paths are lists, but not all lists "
