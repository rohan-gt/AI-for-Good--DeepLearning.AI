...
def remove_repeat_detections(inputFile, outputFile, filteringDir):
    ...
    if os.path.isfile(filteringDir):
        ...
        assert os.path.isdir(filteringDir), "{} is not a valid folder".format(
            filteringDir
        )
        options.filterFileToLoad = os.path.join(
            filteringDir, repeat_detections_core.DETECTION_INDEX_FILE_NAME
        )
...
def main():
    ...
    parser.add_argument(
        "inputFile", help=".json file containing the original, unfiltered API results"
    )
    parser.add_argument(
        "outputFile",
        help=".json file to which you want to write the final, "
        + "filtered API results",
    )
    parser.add_argument(
        "filteringDir",
        help="directory where you looked at lots of images and "
