...
def process_detections(im, options):
    ...
    if detections is None:
        ...
        for det in detections:
            ...
            if category_id not in options.category_id_to_category_name:
                print(
                    "Warning: unrecognized category {} in file {}".format(
        ...
        for category_name in category_names:
            ...
            max_confidence_this_category = category_name_to_max_confidence[
                category_name
            ]
        ...
        using_classification_folders = (
            options.classification_thresholds is not None
            and len(options.classification_thresholds) > 0
        )
        ...
        if len(categories_above_threshold) > 1:
            ...
            target_folder = options.category_name_to_folder[
                "_".join(categories_above_threshold)
            ]
            ...
            if ("animal" in categories_above_threshold) and (
                using_classification_folders
            ):
                ...
                category_name_to_id = {
                    v: k for k, v in options.category_id_to_category_name.items()
                }
                animal_category_id = category_name_to_id["animal"]
                valid_animal_detections = [
                    d
                    for d in detections
                    if (
                        d["category"] == animal_category_id
                        and d["conf"] >= options.category_name_to_threshold["animal"]
                    )
                ]
                ...
                for d in valid_animal_detections:
                    ...
                    for classification in d["classifications"]:
                        ...
                        classification_category_name = (
                            options.classification_category_id_to_name[
                                classification_category_id
                            ]
                        )
                        if (
                            classification_category_name
                            in options.classification_thresholds
                        ) and (
                            classification_confidence
                            > options.classification_thresholds[
                                classification_category_name
                            ]
                        ):
                            classification_categories_above_threshold.add(
                                classification_category_name
                            )
                ...
                if len(classification_categories_above_threshold) == 0:
                    ...
                    classification_folder_name = list(
                        classification_categories_above_threshold
                    )[0]
    ...
    if (
        (categories_above_threshold is None) or (len(categories_above_threshold) == 0)
    ) and options.skip_empty_images:
        return
    ...
    if (
        (not options.render_boxes)
        or (categories_above_threshold is None)
        or (len(categories_above_threshold) == 0)
    ):
        ...
        for category_name in categories_above_threshold:
            ...
            category_detections = [
                d for d in detections if d["category"] == category_id
            ]
...
def separate_detections_into_folders(options):
    ...
    if (os.path.isdir(options.base_output_folder)) and (
        ...
        if options.allow_existing_directory:
            print(
                "Warning: target folder exists and is not empty... did you mean to delete an old version?"
            )
        else:
    ...
    if default_threshold is None:
        if (
            "detector_metadata" in results["info"]
            and "typical_detection_threshold" in results["info"]["detector_metadata"]
        ):
            default_threshold = results["info"]["detector_metadata"][
                "typical_detection_threshold"
            ]
        elif ("detector" not in results["info"]) or (
            results["info"]["detector"] is None
        ):
            print(
                "Warning: detector version not available in results file, using MDv5 defaults"
            )
            ...
            print(
                "Warning: detector metadata not available in results file, inferring from MD version"
            )
    ...
    options.category_name_to_folder["empty"] = os.path.join(
        options.base_output_folder, "empty"
    )
    options.category_name_to_folder["failure"] = os.path.join(
        options.base_output_folder, "processing_failure"
    )
    ...
    for category_name in category_names:
        ...
        category_threshold = options.category_name_to_threshold[category_name]
        print(
            "Processing category {} at threshold {}".format(
                category_name, category_threshold
            )
        )
    ...
    for combination_length in range(2, len(category_names) + 1):
        combined_category_names = list(
            itertools.combinations(category_names, combination_length)
        )
    ...
    for category_name in target_category_names:
        ...
        options.category_name_to_folder[category_name] = os.path.join(
            options.base_output_folder, folder_name
        )
    ...
    if options.classification_thresholds is not None:
        assert (
            "classification_categories" in results
            and results["classification_categories"] is not None
        ), (
        ...
        classification_category_name_to_id = {
            v: k for k, v in classification_categories.items()
        }
        classification_category_id_to_name = {
            k: v for k, v in classification_categories.items()
        }
    ...
    if options.n_threads <= 1 or options.debug_max_images is not None:
        ...
        for i_image, im in enumerate(tqdm(images)):
            if (
                options.debug_max_images is not None
                and i_image > options.debug_max_images
            ):
                break
        ...
        results = list(
            tqdm(pool.imap(process_detections_with_options, images), total=len(images))
        )
...
def main():
    ...
    def validate_threshold(v, name):
        # print('{} {}'.format(v,name))
        if v is not None:
            assert v >= 0.0 and v <= 1.0, "Illegal {} threshold {}".format(name, v)
    ...
    if args.threshold is not None:
        if (
            args.animal_threshold is not None
            and args.human_threshold is not None
            and args.vehicle_threshold is not None
        ):
            raise ValueError(
                "Default threshold specified, but all category thresholds also specified... not exactly wrong, but it's likely that you meant something else."
            )
