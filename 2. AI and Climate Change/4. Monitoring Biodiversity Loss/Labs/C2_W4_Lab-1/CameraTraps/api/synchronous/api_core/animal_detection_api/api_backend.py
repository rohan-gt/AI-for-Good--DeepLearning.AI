    ...
    while True:
        ...
        if serialized_entry:
            ...
            try:
                ...
                assert os.path.isdir(temp_direc), (
                    "Could not find temporary folder {}".format(temp_direc)
                )
                ...
                for filename in os.listdir(temp_direc):
                    ...
                    result = detector.generate_detections_one_image(
                        image,
                        filename,
                        detection_threshold=config.DEFAULT_CONFIDENCE_THRESHOLD,
                    )
                ...
                db.set(
                    entry["id"],
                    json.dumps({"status": 500, "error": "Detection error: " + str(e)}),
                )
            ...
            try:
                ...
                db.set(
                    ...
                    json.dumps(
                        {
                            "status": 500,
                            "error": "Error consolidating the detection boxes:"
                            + str(e),
                        }
                    ),
                )
