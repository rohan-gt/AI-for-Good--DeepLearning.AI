...
def plot_confusion_matrix(
    ...
    if normalize:
        matrix = matrix.astype(np.float64) / (matrix.sum(axis=1, keepdims=True) + 1e-7)
    ...
    if use_colorbar:
        cbar = fig.colorbar(
            im, fraction=0.046, pad=0.04, ticks=[0.0, 0.25, 0.5, 0.75, 1.0]
        )
...
def calibration_ece(
    true_scores: Sequence[int], pred_scores: Sequence[float], num_bins: int
    ...
    bins = np.linspace(0.0, 1.0 + 1e-8, num=num_bins + 1)
