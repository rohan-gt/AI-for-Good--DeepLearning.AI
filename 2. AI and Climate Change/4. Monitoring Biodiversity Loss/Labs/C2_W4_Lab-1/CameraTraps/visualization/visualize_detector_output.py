...
def visualize_detector_output(
    ...
    for entry in tqdm(images):
        ...
        if is_azure:
            ...
            if not sas_blob_utils.check_blob_exists(blob_uri):
                ...
                print(
                    f"Image {image_id} not found in blob container "
                    f"{container}; skipped."
                )
                continue
            # BytesIO object
    ...
    print(f"Rendered detection results on {num_saved} images, saved to {out_dir}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--is-azure",
        action="store_true",
        help="Flag that indidcates images_dir is an Azure blob storage container URL.",
    )
    parser.add_argument(
        "-n",
        "--sample",
