...
from write_html_image_list import (
...
def visualize_incoming_annotations(args):
    ...
    if args.num_to_visualize != -1 and args.num_to_visualize <= len(
        ...
        incoming_id_to_anno = sample(
            list(incoming.image_id_to_annotations.items()), args.num_to_visualize
        )
    else:
    ...
    for image_id, annotations in tqdm(incoming_id_to_anno):
        ...
        im_rel_path = get_image_rel_path(
            dataset_seq_images, dataset_name, seq_id, frame_num
        )
        if im_rel_path is None:
            print(
                f"Not found in megadb entries: dataset {dataset_name},"
                f" seq_id {seq_id}, frame_num {frame_num}"
            )
            continue

        im_full_path = megadb_utils.get_full_path(
            datasets_table, dataset_name, im_rel_path
        )
        ...
        vis_utils.render_iMerit_boxes(
            boxes, classes, image, label_map=incoming.cat_id_to_name
        )
