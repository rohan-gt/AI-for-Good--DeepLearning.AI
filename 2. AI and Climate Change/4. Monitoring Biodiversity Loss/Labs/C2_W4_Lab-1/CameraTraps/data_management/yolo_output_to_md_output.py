...
def yolo_output_to_md_output(
    input_results_folder, input_image_folder, output_file, detector_tag=None
):
    ...
    image_files_relative_no_extension = [
        os.path.splitext(f)[0] for f in image_files_relative
    ]
    ...
    for image_fn in image_files_relative:
        ...
        if not os.path.exists(label_path):
            ...
            with open(label_path, newline="") as f:
                ...
                for row in reader:
                    ...
                    api_box = ct_utils.convert_yolo_to_xywh(
        ...
        images_entries.append(
            {"file": image_fn, "max_detection_conf": max_conf, "detections": detections}
        )
...
if False:
    ...
    input_results_folder = os.path.expanduser(
        "~/tmp/model-version-experiments/pt-test-kru/exp/labels"
    )
