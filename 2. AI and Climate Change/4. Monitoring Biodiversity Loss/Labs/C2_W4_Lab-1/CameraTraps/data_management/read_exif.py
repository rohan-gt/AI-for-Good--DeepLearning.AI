...
def read_exif_tags_for_image(file_path):
    ...
    if (
        (exif_lines is None)
        or (len(exif_lines) == 0)
        or not any([s.lower().startswith("[exif]") for s in exif_lines])
    ):
...
def populate_exif_for_images(input_folder, images):
    ...
    if n_workers == 1:
        ...
        results = list(
            pool.map(partial(populate_exif_data, input_folder=input_folder), images)
        )
...
def read_exif_from_folder(input_folder, output_file):
    ...
    assert output_file.lower().endswith(".json") or output_file.lower().endswith(
        ".csv"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Read EXIF information from all images in"
