...
image_db_has_dims = (
    True  # does the image DB image entries have the height and width of the images
)
...
old_emammal_pattern = re.compile(
    "^datasetemammal\.project(.+?)\.deployment(.+?)\.seq(.+?)[-_]frame(.+?)\.img(.+?)\."
)
...
annotation_image_file_name_to_db_image_id = (
    caltech_annotation_image_file_name_to_db_image_id
)
...
def idfg_add_image_entry(file_name):
    ...
    if file_name.startswith("CrowCreek"):
        ...
        raise RuntimeError(
            "image not in image DB and also not in the two specified folders:",
            file_name,
        )
...
original_rspb_db = json.load(
    open(
        "/Users/siyuyang/Source/temp_data/CameraTrap/databases_2018/rspb/rspb_gola.json"
    )
)
    ...
    for i_row, row in tqdm(enumerate(content)):
        ...
        for bbox_entry in annotations_entry:
            ...
            if image_db_has_dims:
                im_width, im_height = (
                    all_images[img_id]["width"],
                    all_images[img_id]["height"],
                )
            else:
            ...
            db_annotations.append(
                {
                    "id": bbox_entry["id"],
                    "image_id": img_id,
                    "category_id": bbox_cat_map[
                        anno_bbox_cat_map[int(bbox_entry["category_id"])]
                    ],
...
print(
    "Number of annotation entries in the bbox DB (should be same as last line): {}".format(
        len(db_annotations)
    )
)
...
if len(image_ids_not_found):
    print(
        "{} image_ids with annotations are not found in the images DB.".format(
            len(image_ids_not_found)
        )
    )
