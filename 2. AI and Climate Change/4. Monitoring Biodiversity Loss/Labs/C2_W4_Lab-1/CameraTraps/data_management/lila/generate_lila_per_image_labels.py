...
for ds_name in metadata_table.keys():
    metadata_table[ds_name]["json_filename"] = get_json_file_for_dataset(
        ds_name=ds_name, metadata_dir=metadata_dir, metadata_table=metadata_table
    )
...
with open(output_file, "w") as f:
    ...
    for ds_name in metadata_table.keys():
        ...
        for i_image, im in enumerate(images):
            if (
                (debug_max_images_per_dataset is not None)
                and (debug_max_images_per_dataset > 0)
            ...
            for k in im.keys():
                if ("date" in k or "time" in k) and (
                    k not in ["datetime", "date_captured"]
                ):
            ...
            if has_valid_datetime(im):
                ...
                if dt is None or dt.year < 1990 or dt.year > 2025:
                    ...
                    print(
                        "Suspicious date parsing result for image {}: {}".format(
                            im["id"], im["datetime"]
                        )
                    )
                else:
        ...
        if len(unannotated_images) > 0:
            print(
                "Warning: {} of {} images are un-annotated\n".format(
                    len(unannotated_images), len(images)
                )
            )
...
for ds_name in metadata_table.keys():
    ...
    if len(non_empty_rows) == 0:
        ...
        non_empty_rows_to_download = non_empty_rows.sample(
            n=n_non_empty_images_per_dataset
        )
...
for i_image, image in tqdm(
    enumerate(images_to_download), total=len(images_to_download)
):
    url = image["url"]
    ext = os.path.splitext(url)[1]
    output_file = os.path.join(
        preview_folder, "image_{}".format(str(i_image).zfill(4)) + ext
    )
