...
for ds_name in datasets_of_interest:
    ...
    for species_name in species_of_interest:
        matching_categories = list(
            filter(lambda x: x["name"] == species_name, categories)
        )
    ...
    print(
        "Found {} matching categories for data set {}:".format(
            len(category_ids), ds_name
        )
    )
    ...
    image_ids_of_interest = set(
        [ann["image_id"] for ann in annotations if ann["category_id"] in category_ids]
    )

    print(
        "Selected {} of {} images for dataset {}".format(
            len(image_ids_of_interest), len(images), ds_name
        )
    )
    ...
    if max_images_per_dataset is not None and len(filenames) > max_images_per_dataset:
        print(
            "Taking the first {} of {} images for {}".format(
...
if use_azcopy_for_download:
    ...
    for ds_name in downloads_by_dataset:
        ...
        #
        az_filename = os.path.join(
            output_dir, "filenames_{}.txt".format(ds_name.lower().replace(" ", "_"))
        )
    ...
    if n_download_threads <= 1:
        ...
        tqdm(
            pool.imap(
                lambda s: download_relative_filename(s, output_dir, verbose=False),
                urls_to_download,
            ),
