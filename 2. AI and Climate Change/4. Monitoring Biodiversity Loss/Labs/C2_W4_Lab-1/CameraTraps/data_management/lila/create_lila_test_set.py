...
for ds_name in metadata_table.keys():
    ...
    if "empty" not in category_name_to_id:
        ...
        empty_annotations = [
            ann for ann in d["annotations"] if ann["category_id"] == empty_category_id
        ]
        try:
            empty_annotations_to_download = random.sample(
                empty_annotations, n_empty_images_per_dataset
            )
        except ValueError:
    ...
    try:
        non_empty_annotations_to_download = random.sample(
            non_empty_annotations, n_non_empty_images_per_dataset
        )
    except ValueError:
    ...
    annotations_to_download = (
        empty_annotations_to_download + non_empty_annotations_to_download
    )
...
for ds_name in metadata_table.keys():
    ...
    filenames = [
        im["file_name"] for im in metadata_table[ds_name]["images_to_download"]
    ]
...
for ds_name in metadata_table.keys():
    ...
    for url in urls_to_download:
        ...
        output_file_relative = (
            ...
            + "_"
            + url.replace(base_url, "").replace("/", "_").replace("\\", "_")
        )
        output_file_absolute = os.path.join(output_dir, output_file_relative)
        try:
            download_url(
                url,
                destination_filename=output_file_absolute,
