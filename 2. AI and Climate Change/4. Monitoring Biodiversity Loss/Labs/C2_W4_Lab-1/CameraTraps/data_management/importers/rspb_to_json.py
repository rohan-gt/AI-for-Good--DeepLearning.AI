...
metadataFile = os.path.join(
    baseDir, "gola_camtrapr_master_renaming_table_2019-01-31.csv"
)
...
print(
    "Read {} columns and {} rows from metadata file".format(
        len(metadataTable.columns), len(metadataTable)
    )
)
...
print(
    "Read {} unique species in {} rows".format(len(uniqueSpecies), len(metadataTable))
)
...
print(
    "Finished checking file existence, extracting metadata in {}, couldn"
    "t find {} images".format(
...
print(
    "Finished matching {} images, failed to match {}".format(
...
htmlOutputFile = visualize_db.process_images(
    db_path, output_dir, image_base_dir, options
)
