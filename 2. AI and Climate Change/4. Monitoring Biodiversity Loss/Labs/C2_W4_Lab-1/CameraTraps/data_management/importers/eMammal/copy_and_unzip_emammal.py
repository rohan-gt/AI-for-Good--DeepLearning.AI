...
def _copy_unzip(source_path, dest_folder):
    try:
        dest_subfolder = os.path.join(
            dest_folder, os.path.basename(source_path).split(".zip")[0]
        )
        ...
        try:
            ...
            print(
                "WARNING {} did not get copied. Exception: {}".format(
                    source_path, str(e)
                )
            )
            return source_path
...
def copy_from_mounted_container(source_folder, dest_folder):
    ...
    with open(
        os.path.join(log_folder, "copy_unzip_results_{}.json".format(cur_time)), "w"
    ) as f:
        json.dump(results, f)


def _download_unzip(
    blob_service: BlobServiceClient, container: str, blob_path: str, dest_path: str
) -> Optional[str]:
    try:
        ...
        print(
            "ERROR while downloading or unzipping {}. Exception: {}".format(
                blob_path, str(e)
            )
        )
        return blob_path
...
def download_from_container(
    ...
    desired_blob_prefix: str = "",
) -> None:
    generator = blob_service.get_containre_client(container).list_blobs()
    desired_blobs = [
        blob.name for blob in generator if blob.name.startswith(desired_blob_prefix)
    ]
    ...
    cur_time = datetime.now().strftime("%Y%m%d-%H%M%S")
    with open(
        os.path.join(log_folder, "download_unzip_results_{}.json".format(cur_time)), "w"
    ) as f:
        json.dump(results, f)
...
if __name__ == "__main__":
    ...
    if origin == "mounted":
        ...
        source_folder = (
            "/home/yasiyu/mnt/wildlifeblobssc/emammal"  # blob container mounted
        )
