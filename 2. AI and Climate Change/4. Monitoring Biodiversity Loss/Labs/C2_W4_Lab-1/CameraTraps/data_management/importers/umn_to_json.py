...
ground_truth_file = os.path.join(
    input_base, "images_hv_jan2020_reviewed_force_nonblank.csv"
)
...
for i_location, location in tqdm(enumerate(locations)):
    ...
    sorted_images_this_location = sorted(
        images_this_location, key=lambda im: im["datetime"]
    )
...
categories_to_counts_sorted = {
    k: v
    for k, v in sorted(
        categories_to_counts.items(), key=lambda item: item[1], reverse=True
    )
}
...
filenames_with_multiple_annotations = [
    fn for fn in filename_to_images.keys() if len(filename_to_images[fn]) > 1
]

print(
    "Found {} filenames with multiple annotations".format(
        len(filenames_with_multiple_annotations)
    )
)
...
for input_im in tqdm(images):
    ...
    if im["id"] in image_id_to_image:
        print(
            "Warning: image ID {} ({}) has multiple annotations".format(
                im["id"], im["id"].replace("_", "/")
            )
        )
    else:
...
def copy_image(im):
    ...
    if ("human" in category_names_this_image) or (
        "missing" in category_names_this_image
    ):
        prefix = "private"
    else:
        prefix = "public"
