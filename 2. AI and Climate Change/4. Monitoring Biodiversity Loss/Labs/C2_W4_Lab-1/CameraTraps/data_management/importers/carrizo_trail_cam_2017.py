...
category_replacements = {
    "unidnetifiable": "unidentifiable",
    "unidentifiable animal": "unidentifiable",
}

annotation_fields_to_copy = [
    "rep",
    "photo rep",
    "timeblock",
    "night.day",
...
for inp_file in input_metadata_files:
    ...
    input_metadata["file"] = input_metadata[["file", "rep", "week", "microsite"]].apply(
        lambda x: "{3}/{4}{1}-week{0}-carrizo-2017/IMG_{2}.JPG".format(
            int(x[2]),
            int(x[1]),
...
print(
    "Finished verifying image existence in {}, found {} missing files (of {})".format(
        humanfriendly.format_timespan(elapsed), len(missing_files), len(image_filenames)
    )
)
...
for iImage, image_path in tqdm(
    enumerate(image_full_paths), total=len(image_full_paths)
):
...
print(
    "{} of {} files are not in metadata".format(
        len(images_missing_from_metadata), len(image_full_paths)
    )
)
...
elapsed = time.time() - start_time
print(
    "Finished creating CCT dictionaries in {}".format(
...
html_output_file, image_db = visualize_db.process_images(
    ...
    output_dir=os.path.join(output_base, "carrizo trail cam 2017/preview"),
