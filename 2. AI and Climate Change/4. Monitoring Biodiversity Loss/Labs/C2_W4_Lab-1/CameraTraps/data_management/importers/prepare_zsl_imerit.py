...
image_json = (
    r"d:\wildlife_data\zsl_borneo\201906cameratraps\0.5\zsl_camera_traps_201906.json"
)
...
html_output_file, image_db = visualize_db.process_images(
    indexedData.db, os.path.join(output_base, "preview"), image_base, viz_options
)
...
print(
    "Found {} of {} images ({} humans)".format(
        len(images_to_annotate), len(annotation_list), n_humans
    )
)
...
for im in tqdm(images_to_annotate):
    ...
    output_filename = (
        "datasetzslborneo.seq"
        + "{0:0>8d}".format(seq_id)
        + ".frame"
        + "{0:0>4d}".format(frame_num)
        + ".img"
        + id
        + extension
    )
