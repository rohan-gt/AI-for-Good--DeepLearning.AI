...
for i_folder, folder in enumerate(folders):
    ...
    if folder.startswith("SC_"):
        ...
        input_metadata["image_relative_path"] = input_metadata[
            ["camera_name", "filename"]
        ].apply(lambda x: os.path.join(folder, x[0], x[1]), axis=1)
    ...
    print(
        "Finished verifying image existence for {} files in {} rows\nFound {} filenames with multiple labels, {} missing images".format(
            len(input_metadata),
            len(filenames_to_rows),
    ...
    image_relative_paths_this_folder = [
        s for s in image_relative_paths if s.startswith(folder)
    ]
    ...
    unannotated_images = [
        s
        for s in image_relative_paths_this_folder
        if s not in annotated_files_this_folder_set
    ]

    print(
        "Found {} unannotated images (of {}) in this folder".format(
    ...
    for image_relative_path in list(filenames_to_rows.keys()):
        ...
        img_id = (
            image_relative_path.replace("\\", "/").replace("/", "_").replace(" ", "_")
        )
        ...
        for i_row in row_indices:
            ...
            if (
                category_name == "passerine_species"
                and common_name != ""
                and common_name != "passerine_species"
            ):
            ...
            if category_name in category_name_to_category:
                ...
                if category["common_name"] == "":
                    ...
                    if common_name != category["common_name"]:
                        print(
                            "Warning: common name {} used for species {}, previously {}".format(
...
print(
    "Finished creating CCT dictionaries, loaded {} images of {} total on disk ({} invalid)".format(
