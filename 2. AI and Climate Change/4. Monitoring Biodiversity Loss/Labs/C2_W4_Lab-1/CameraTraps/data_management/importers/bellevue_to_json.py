...
output_filename = os.path.join(
    base_dir, "bellevue_camera_traps.{}.json".format(str(datetime.date.today()))
)
...
for i_camera, camera_path in enumerate(camera_folders):
    ...
    sorted_images_this_camera = sorted(
        images_this_camera, key=lambda im: im["datetime"]
    )
...
html_output_file, image_db = visualize_db.process_images(
    output_filename, os.path.join(output_base, "preview"), base_dir, viz_options
)
