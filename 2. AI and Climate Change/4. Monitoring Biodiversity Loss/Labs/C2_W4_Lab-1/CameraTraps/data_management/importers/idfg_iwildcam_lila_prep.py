...
print(
    "Read {} annotations for {} images".format(
        len(private_csv_lines), len(input_data["images"])
    )
)
...
info["contributor"] = (
    "Images acquired by the Idaho Department of Fish and Game, dataset curated by Sara Beery"
)
...
for ann in annotations:
    ...
    category_names_to_counts[category_name] = (
        category_names_to_counts[category_name] + 1
    )
...
options.baseDir = os.path.join(base_folder, "images")
assert os.path.isdir(options.baseDir)
...
print(
    "Read {} annotations for {} images".format(
        len(input_data["annotations"]), len(input_data["images"])
    )
)
