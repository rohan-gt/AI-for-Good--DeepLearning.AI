...
output_json_original_strings = os.path.join(
    output_base, "idaho-camera-traps-original-strings.json"
)

# The output file, using obfuscated strings for everything but filenamed
output_json_remapped_ids = os.path.join(
    output_base, "idaho-camera-traps-remapped-ids.json"
)
...
megadetector_results_file = (
    r"H:\idaho-camera-traps\idfg-2021-07-26idaho-camera-traps_detections.json"
)
...
opstate_mappings = {
    "snow on lense": "snow on lens",
    "poop/slobber": "lens obscured",
    "maintenance": "human",
}
...
def get_files():
    ...
    csv_files = [
        ...
        if (
            ...
            and ("Backups" not in s)
            and ("Metadata.csv" not in s)
            and ("ExportedDataFiles" not in s)
            and ("CSV Files" not in s)
        )
    ]
...
def csv_to_sequences(csv_file):
    ...
    for sequence_id in location_sequences:
        ...
        survey_species = [
            s.replace("present", "")
            for s in presence_columns_marked
            if s != "otherpresent"
        ]
        ...
        if len(presence_columns_marked) == 0:
            ...
            for count_column in count_columns:
                ...
                if not (len(values) == 1 and values[0] == 0):
                    print(
                        "Warning: presence and counts are inconsistent for {}".format(
                            sequence_id
                        )
                    )
                    ...
                    for presence_column in presence_to_count_columns.keys():
                        count_columns_this_species = presence_to_count_columns[
                            presence_column
                        ]
        ...
        for i_row, row in sequence_df.iterrows():
            ...
            if (
                ("RelativePath" in row)
                and (isinstance(row["RelativePath"], str))
...
if __name__ == "__main__":
    ...
    for i_sequences, sequences in enumerate(tqdm(sequences_by_file)):
        ...
        for i_sequence, sequence in enumerate(sequences):
            ...
            if sequence_id in sequence_ids:
                print(
                    "Warning: duplicate sequence for {}, creating new sequence".format(
                        sequence_id
                    )
                )
            ...
            for im in images:
                ...
                image_file_container_relative = os.path.relpath(
                    image_file_absolute, input_base
                )
    ...
    print(
        "{} of {} images missing ({} on disk)".format(
            len(missing_images), n_images_in_sequences, len(image_files)
        )
    )
    ...
    with open(human_review_list, "r") as f:
        ...
        human_flagged_images = [
            s.strip().replace("/", "\\") for s in human_flagged_images
        ]
    ...
    for sequences in tqdm(sequences_by_file):
        ...
        for sequence in sequences:
            species_present = sequence["species_present"]
            species_present = [
                s.lower().strip().replace(",", ";") for s in species_present
            ]
            ...
            if len(species_present) == 0:
                ...
                assert (
                    category_name_to_category["empty"]["id"]
                    == list(sequence_category_ids)[0]
                )
                ...
                if "unknown" in species_present and len(species_present) > 1:
                    assert all(
                        [
                            (
                                (s in category_mappings)
                                or (s in valid_opstates)
                                or (s in opstate_mappings.values())
                            )
                ...
                for category_name_string in species_present:
                    ...
                    if category_name_string not in category_mappings:
                        assert (
                            category_name_string in valid_opstates
                            or category_name_string in opstate_mappings.values()
                        )
                    else:
            ...
            for i_image, im in enumerate(sequence_images):
                ...
                output_im["file_name"] = os.path.join(
                    csv_folder_relative, im["file_name"]
                )
    ...
    print(
        "Loaded {} annotations in {} categories for {} images".format(
    ...
    for ann in tqdm(annotations):
        ...
        categories_to_counts[ann["category_id"]] = (
            categories_to_counts[ann["category_id"]] + 1
        )
    ...
    _, _, _ = sanity_check_json_db.sanity_check_json_db(
        output_json_original_strings, options
    )
    ...
    html_output_file, _ = visualize_db.process_images(
        db_path=output_json_original_strings,
    ...
    if False:
        ...
        human_md_categories = [
            ...
            if (
                (md_results["detection_categories"][category_id] == "person")
                or (md_results["detection_categories"][category_id] == "vehicle")
            )
        ]
        ...
        print(
            "MD found {} potential human images (of {})".format(
        ...
        print(
            "{} images identified as human in source metadata".format(
                len(manual_human_images)
            )
        )
        ...
        for i_image, fn in enumerate(tqdm(missing_human_images)):
            ...
            output_path = os.path.join(
                human_review_folder, str(i_image).zfill(4) + "_" + fn.replace("/", "~")
            )
    ...
    if (not force_generate_mappings) and (os.path.isfile(id_mapping_file)):
        ...
        for im in tqdm(images):
            ...
            if im["seq_id"] in sequence_id_mappings:
                ...
                # Otherwise assign a string-formatted int as the ID
                n_sequences_this_location = location_id_string_to_n_sequences[
                    location_id
                ]
                ...
                location_id_string_to_n_sequences[location_id] = (
                    n_sequences_this_location
                )
    ...
    _, _, _ = sanity_check_json_db.sanity_check_json_db(
        output_json_remapped_ids, options
    )
    ...
    html_output_file, _ = visualize_db.process_images(
        db_path=output_json_remapped_ids,
        output_dir=os.path.join(output_base, "preview"),
        image_base_dir=input_base,
        options=viz_options,
    )
    ...
    print(
        "Moving {} of {} images to the private folder".format(
            len(private_image_ids), len(images)
        )
    )
    ...
    def process_image(im):
        ...
        if private_image:
            output_absolute_path = os.path.join(
                output_image_base_private, output_relative_path
            )
        else:
            output_absolute_path = os.path.join(
                output_image_base_public, output_relative_path
            )
    ...
    print(
        "Enumerated {} output files (of {} images)".format(
            len(all_output_files), len(images)
        )
    )
    ...
    html_output_file, _ = visualize_db.process_images(
        db_path=output_json,
        output_dir=os.path.join(output_base, "final-preview-01"),
    ...
    all_public_output_files_list = os.path.join(
        output_base, "all_public_output_files.json"
    )
    ...
    all_public_output_sizes_list = os.path.join(
        output_base, "all_public_output_sizes.json"
    )
    ...
    print(
        "{} in {} files".format(
            humanfriendly.format_size(total_size), len(all_public_output_files)
        )
    )
    ...
    def create_zipfile(i_file_list):
        ...
        zipfile_name = os.path.join(
            "k:\\idaho-camera-traps-images.part_{}.zip".format(i_file_list)
        )
