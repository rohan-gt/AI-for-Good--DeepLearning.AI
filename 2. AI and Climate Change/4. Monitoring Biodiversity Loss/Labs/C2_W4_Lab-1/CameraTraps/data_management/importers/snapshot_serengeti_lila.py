...
resized_images_dictionary_cache_file = os.path.join(
    temp_base, "resized_image_dictionaries.p"
)
revised_annotations_dictionary_cache_file = os.path.join(
    temp_base, "revised_annotations.p"
)
...
for iSeason in tqdm(range(1, nSeasons + 1)):
    image_table_fn = os.path.join(
        metadata_base, "SER_S" + str(iSeason) + "_report_lila_image_inventory.csv"
    )
    annotation_table_fn = os.path.join(
        metadata_base, "SER_S" + str(iSeason) + "_report_lila.csv"
    )
...
print(
    "Finished concatenating {} images and {} annotations".format(
        len(image_table), len(annotation_table)
    )
)
...
print(
    "Done converting tables to dictionaries in {}".format(
...
if False:
    ...
    print(
        "Loaded metadata about {} images and {} sequences".format(
            len(images), len(seq_id_to_annotations)
        )
    )
...
def process_image(im):
    ...
    assert all(ann["seq_id"] == im["seq_id"] for ann in seq_annotations), (
        "Error on image {}".format(im["id"])
    )
    ...
    assert all(seqim["seq_id"] == im["seq_id"] for seqim in images_in_sequence), (
        "Error on image {}".format(im["id"])
    )
    ...
    assert all(ann["site"] == im["location"] for ann in seq_annotations), (
        "Error on image {}".format(im["id"])
    )
    ...
    assert all(ann["datetime"] == im["datetime"] for ann in seq_annotations), (
        "Error on image {}".format(im["id"])
    )
...
print(
    "Saving size-checked dictionaries to {}".format(
        resized_images_dictionary_cache_file
    )
)
cct_cache = [
    im_id_to_image,
...
if False:
    ...
    print(
        "Loading dictionaries with size information from {}".format(
            resized_images_dictionary_cache_file
        )
    )
...
print(
    "\nOf {} images: {} missing, {} corrupt, {} no annotation".format(
        len(images), n_missing, n_corrupt, n_no_annotation
    )
)
...
print(
    "\nMean/min/max sequence length is {}/{}/{}".format(
        np.mean(sequence_lengths), min(sequence_lengths), max(sequence_lengths)
    )
)
...
print(
    "\nCreated {} replicated annotations from {} original annotations".format(
        len(annotations_replicated), len(annotations)
    )
)
...
elapsed = time.time() - start_time
print(
    "Finished listing {} files in {}".format(
        len(all_files), humanfriendly.format_timespan(elapsed)
    )
)
...
print(
    "{} files not in the database (of {})".format(len(files_not_in_db), len(all_files))
)
...
for old_im in tqdm(cct_old["images"]):
    ...
    if old_im["id"] not in im_id_to_image:
        ...
        if (old_im["width"] != new_im["width"]) or (
            old_im["height"] != new_im["height"]
        ):
...
print(
    "Saving revised-annotation dictionaries to {}".format(
        revised_annotations_dictionary_cache_file
    )
)
cct_cache = [
    im_id_to_image,
...
if False:
    # %%
    print(
        "Loading dictionaries from {}".format(revised_annotations_dictionary_cache_file)
    )
...
for season in seasons:
    season_mismatches = [
        x for x in size_mismatches if x["id"].startswith("S" + str(season))
    ]
    mismatches_by_season.append(season_mismatches)

for iSeason, season_mismatches in enumerate(mismatches_by_season):
    print(
        "Size mismatches in season {}: {}".format(
            iSeason + 1, len(mismatches_by_season[iSeason])
        )
    )
...
for season in range(1, nSeasons + 1):
    i_season = season - 1
    print(
        "Season {}: {} images, {} annotations".format(
            season,
            len(images_by_season[i_season]),
            len(annotations_by_season[i_season]),
        )
    )

print(
    "Public total: {} images, {} annotations".format(
        len(images_public), len(annotations_public)
    )
)
...
for season in range(1, nSeasons + 3):
    ...
    if i_season == nSeasons + 1:
        ...
        data["info"]["description"] = data["info"][
            "description"
...
for id in categories_by_season[target_season_idx]:
    ...
    if not b_found:
        print(
            "Category {} ({}) only in S{}".format(
                id, cat_id_to_cat[id]["name"], target_season_idx + 1
            )
        )
...
for cat in categories:
    if cat["id"] not in categories_by_season[target_season_idx]:
        print(
            "Category {} ({}) not in S{}".format(
                cat["id"], cat["name"], target_season_idx + 1
            )
        )
...
fn_annotation_csv_public = os.path.join(
    output_base, "SnapshotSerengeti_v2_0_annotations.csv"
)
fn_image_csv_all = os.path.join(output_base, "SnapshotSerengeti_v2_1_images.csv")
fn_annotation_csv_all = os.path.join(
    output_base, "SnapshotSerengeti_v2_1_annotations.csv"
)
...
def create_season_archive(i_season):
    ...
    zipfilename = os.path.join(
        output_zip_base, "SnapshotSerengeti_S{:>02d}_v2_0.zip".format(season)
    )
    ...
    for iImage, im in enumerate(images):
        ...
        if n_images_added >= max_files_per_archive:
            ...
            print(
                "Starting new archive for season {}: {}".format(
                    i_season + 1, zipfilename
                )
            )
            zip = ZipFile(zipfilename, "w")
            n_images_added = 0
...
for jsonFn in files_to_check:
    ...
    for im in tqdm(data_public["images"]):
        ...
        assert (not im["file_name"].startswith("S11")) and (
            not im["file_name"].startswith("SER11")
        )
    ...
    for ann in tqdm(data_public["annotations"]):
        assert (not ann["image_id"].startswith("S11")) and (
            not ann["image_id"].startswith("SER11")
        )
...
print(
    "{} missing images in {} bounding boxes ({} found)".format(
...
html_output_file, bbox_db = visualize_db.process_images(
    bbox_json_fn, os.path.join(output_base, "preview2"), image_base, viz_options
)
...
print(
    "Found {} images ({} empty, {}%) in {} sequences, in {} categories".format(
