...
output_file = (
    "/datadrive/snapshotserengeti/databases/SnapshotSerengeti_multiple_classes.json"
)
...
for event in data[1:]:
    if event[0] not in data_dicts:
        data_dicts[event[0]] = []
    data_dicts[event[0]].append(
        {data_fields[i]: event[i] for i in range(len(data_fields))}
    )
