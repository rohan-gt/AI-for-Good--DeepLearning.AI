...
project_name = "SER"
season_name = "S1-11"
project_friendly_name = "Snapshot Serengeti"
...
image_inventory_file = os.path.join(
    project_base, project_season_name + "_report_lila_image_inventory.csv"
)

# Total number of each answer to each question, e.g. total number of times each species was identified
#
# Not used here
response_overview_file = os.path.join(
    project_base, project_season_name + "_report_lila_overview.csv"
)
...
species_list_filename = os.path.join(
    output_base,
    project_friendly_name.replace(" ", "")
    + "_"
    + season_name
...
summary_info_filename = os.path.join(
    output_base,
    project_friendly_name.replace(" ", "")
    + "_"
    + season_name
...
print(
    "Finished loading {} image mappings and {} annotations".format(
...
for iRow, row in tqdm(annotation_table.iterrows(), total=len(annotation_table)):
    ...
    ann["site"] = row["site"]
    if is_float_and_nan(row["capture_date_local"]) or is_float_and_nan(
        row["capture_time_local"]
    ):
        ann["datetime"] = ""
        n_invalid_dates += 1
    else:
...
print(
    "Converted {} annotations, {} images, {} categories ({} invalid dates)".format(
...
def process_image(im):
    ...
    assert all(str(ann["site"]) == im["location"] for ann in seq_annotations), (
        "Error on image {}".format(im["id"])
    )
...
print(
    "Finished listing {} files in {}".format(
        len(image_files), humanfriendly.format_timespan(elapsed)
    )
)
...
print(
    "{} files not in the database (of {})".format(
        len(files_not_in_db), len(image_files)
    )
)
del fn
...
info["description"] = "Camera trap data from the {} program".format(
...
sortedCategories, data, errorInfo = sanity_check_json_db.sanity_check_json_db(
...
zip_files_to_flat(
    [annotation_file, image_inventory_file],
    os.path.join(output_base, project_season_name + ".csv.zip"),
)
...
if False:
    ...
    with open(
        os.path.join(
            output_base, project_name + "_" + season_name + "_missing_images.txt"
        ),
        "w",
    ) as f:
