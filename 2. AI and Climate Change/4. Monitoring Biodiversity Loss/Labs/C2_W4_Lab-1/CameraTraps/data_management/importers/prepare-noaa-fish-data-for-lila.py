...
json_files = [
    ...
    if (fn.endswith(".json") and (fn != os.path.basename(output_json_fn)))
]
...
categories = [
    {"id": CATEGORY_ID_EMPTY, "name": "empty"},
    {"id": CATEGORY_ID_FISH, "name": "animal"},
]
        ...
        for line in lines:
            ...
            if id_string in id_string_to_site:
                ...
                if site_id in site_to_location_id:
                    ...
                    print(
                        "Adding new location ID {} for site {}".format(
                            location_id, site_id
                        )
                    )
            ...
            assert (
                len(input_ann) == 1
                and len(input_ann.keys()) == 1
                and "object" in input_ann
            )
            ...
            if len(formatted_data) == 0:
                ...
                for box in formatted_data:
                    ...
                    assert (
                        len(attributes) == 2
                        and "occluded" in attributes
                        and "truncated" in attributes
                    )
...
print(
    "Found annotations for {} images (of {})".format(
        len(fn_to_image), len(jpeg_image_folder_files_set)
    )
)


print(
    "Matched locations for {} images (failed to match {})".format(
