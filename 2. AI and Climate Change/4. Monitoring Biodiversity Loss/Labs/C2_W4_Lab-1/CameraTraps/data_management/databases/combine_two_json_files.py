...
desc_2 = " ".join(
    [info_2["description"].split()[0].lower()] + info_2["description"].split()[1:]
)
...
### categories to merge
categories_to_merge = [
    ["elk aka red deer", "elk_red deer"],
    ["other bird species", "unknown bird"],
    ["blue eared pheasant", "blue eared-pheasant"],
    ["reeves' muntjac", "reeve's muntjac"],
...
new_category_names = ["empty"] + sorted(
    list(
        set([c["name"] for c in categories_1] + [c["name"] for c in categories_2])
        - {"empty"}
        - set(category_remapping.keys())
    )
)
categories_new = []
