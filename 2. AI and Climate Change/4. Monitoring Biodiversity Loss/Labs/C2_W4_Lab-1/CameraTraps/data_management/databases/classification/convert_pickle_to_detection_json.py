...
parser = argparse.ArgumentParser(
    "Converts pickle files with detection created by ./make_classification_dataset.py "
...
parser.add_argument(
    "dataset_json",
    type=str,
    help="Path to the dataset description in COCO json format.",
)
parser.add_argument(
    "output_json",
    type=str,
    help="Path to the output file that will contain the detection in API JSON format.",
)
args = parser.parse_args()

# Parameter check
assert os.path.isfile(args.input_pkl), "ERROR: The input file could not be found!"
assert os.path.isfile(args.dataset_json), (
    "ERROR: The dataset json file could not be found!"
)
...
for im_key in tqdm.tqdm(list(detections.keys())):
    ...
    for det_id in range(len(det_boxes)):
        if det_conf[det_id] > DETECTION_CONF_THRESHOLD:
            det_list.append(
                dict(
                    category=str(det_classes[det_id]),
    ...
    if len(im_dict["detections"]) > 0:
        ...
        im_dict["max_detection_conf"] = 0.0
