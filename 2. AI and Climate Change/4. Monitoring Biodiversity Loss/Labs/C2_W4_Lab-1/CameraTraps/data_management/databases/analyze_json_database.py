...
sortedCats = sorted(
    zip([len(cat_to_ims[cat]) for cat in cat_to_ims], [cat for cat in cat_to_ims]),
    key=lambda t: t[0],
    reverse=True,
)
...
plt.xticks(
    range(len(sortedCats)), [cat_id_to_name[cat[1]] for cat in sortedCats], rotation=90
)
plt.tight_layout()
...
sortedLocs = sorted(
    zip([len(loc_to_ims[loc]) for loc in loc_to_ims], [loc for loc in loc_to_ims]),
    key=lambda t: t[0],
    reverse=True,
)
...
plt.xticks(
    range(len(loc_to_ims)), [loc[1] for loc in sortedLocs], rotation=90, fontsize=2
)
plt.tight_layout()
plt.savefig(plot_directory + "/ims_per_loc.jpg", dpi=500)
plt.clf()

cat_count_per_location = {
    loc[1]: {cat["id"]: 0 for cat in categories} for loc in sortedLocs
}
for im in images:
...
plt.xlabel("Location")
plt.legend(
    catNames, loc="center left", bbox_to_anchor=(1.005, 0.5), edgecolor=None, fontsize=4
)
...
sortedLocs = list(
    zip(
        [len(season_to_ims[loc]) for loc in season_to_ims],
        [loc for loc in season_to_ims],
    )
)
...
loc_to_lion_ims = {
    loc: [i for i in loc_to_ims[loc] if im_id_to_cat[i] in lion_ids]
    for loc in loc_to_ims
}
loc_to_lion_ims = {
    loc: loc_to_lion_ims[loc]
    for loc in loc_to_lion_ims
    if len(loc_to_lion_ims[loc]) > 0
}
sortedLocs = sorted(
    zip(
        [len(loc_to_lion_ims[loc]) for loc in loc_to_lion_ims],
        [loc for loc in loc_to_lion_ims],
    ),
    key=lambda t: t[0],
    reverse=True,
)
...
plt.ylabel("Locations")
plt.xticks(
    range(len(loc_to_lion_ims)), [loc[1] for loc in sortedLocs], rotation=90, fontsize=6
)
plt.tight_layout()
...
loc_to_elephant_ims = {
    loc: [i for i in loc_to_ims[loc] if im_id_to_cat[i] in elephant_ids]
    for loc in loc_to_ims
}
loc_to_elephant_ims = {
    loc: loc_to_elephant_ims[loc]
    for loc in loc_to_elephant_ims
    if len(loc_to_elephant_ims[loc]) > 0
}
sortedLocs = sorted(
    zip(
        [len(loc_to_elephant_ims[loc]) for loc in loc_to_elephant_ims],
        [loc for loc in loc_to_elephant_ims],
    ),
    key=lambda t: t[0],
    reverse=True,
)
...
plt.ylabel("Locations")
plt.xticks(
    range(len(loc_to_elephant_ims)),
    [loc[1] for loc in sortedLocs],
    rotation=90,
    fontsize=6,
)
plt.tight_layout()
...
lions_and_elephants = list(
    set(loc_to_elephant_ims.keys()).intersection(set(loc_to_lion_ims.keys()))
)
total = []
...
sorted_by_total = sorted(
    zip(lions_and_elephants, total), reverse=True, key=lambda t: t[1]
)
