...
def order_seq_properties(seq_item):
    ordered = OrderedDict(
        [("dataset", seq_item["dataset"]), ("seq_id", seq_item["seq_id"])]
    )
    ...
    if len(seq["images"]) > 1:
        ...
        for i in seq["images"]:
            if "frame_num" not in i:
                assert False, (
                    "sequence {} has more than one image but not all images have frame_num".format(
                        seq["seq_id"]
                    )
                )
        ...
        assert len(set(frame_num_set)) == len(seq["images"]), (
            "sequence {} has frame_num that are not unique".format(seq["seq_id"])
        )
...
def check_class_on_seq_or_image(seq):
    ...
    assert class_on_seq or class_on_all_img, (
        "sequence {} does not have the class property on either sequence or image level".format(
            seq["seq_id"]
        )
    )
...
def main():
    ...
    parser.add_argument(
        "items_json", action="store", type=str, help=".json file to ingest into MegaDB"
    )
