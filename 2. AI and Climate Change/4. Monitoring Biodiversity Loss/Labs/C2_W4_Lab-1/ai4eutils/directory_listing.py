...
def is_image_file(path):
    ...
    return os.path.splitext(path)[1].lower()[1:] in [
        "jpg",
        "jpeg",
    ]  # , 'gif', 'tiff', 'tif', 'png']
...
def traverse_and_create_index(
    ...
    if sas_url:
        ...
        blob_service = BlobServiceClient(
            account_url=f"{account_name}.blob.core.windows.net", credential=sas_token
        )
...
def parse_args(argv=sys.argv[1:]):
    ...
    parser.add_argument(
        "directory", type=str, help="Path to directory which should be traversed."
    )
    parser.add_argument(
        "--basepath",
        type=str,
...
if __name__ == "__main__":
    ...
    assert os.path.isdir(args.directory), "{} is not a valid directory".format(
        args.directory
    )
    assert re.match("https?://[^\.]+\.blob\.core\.windows\.net/.+", args.sas_url), (
        "--sas_url does not "
        + "match the format https://accname.blob.core.windows.net/bname/path/to/folder?..."
    )

    traverse_and_create_index(
        args.directory,
