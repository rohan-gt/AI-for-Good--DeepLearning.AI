...
def get_used_avg_blob_capacity(credentials, subscription_id):
    ...
    for group in resource_groups:
        ...
        for storage_account in storage_accounts:
            ...
            blob_size = get_metric_data_capacity(
                group.name,
    ...
    cols = [
        ...
        "File capacity",
        "Total capacity",
        "Total capacity (friendly)",
    ]
    df = pd.DataFrame(lst, columns=cols)

    file_name = (
        "metrics_" + datetime.datetime.now().strftime("%m-%d-%y-%H%M%S") + ".csv"
    )
...
def get_metric_data_capacity(
    resource_group_name, storage_account_name, subscription_id, type
):
    ...
    if type == Metric_type.blob_capacity:
        resource_id = resource_id.format(
            subscription_id,
            resource_group_name,
