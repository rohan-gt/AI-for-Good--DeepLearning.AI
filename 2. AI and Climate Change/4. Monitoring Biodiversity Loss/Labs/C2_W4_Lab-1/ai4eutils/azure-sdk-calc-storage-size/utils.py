...
def authenticate_client_key(authenticationInfo):
    ...
    mgmt_token = context.acquire_token_with_client_credentials(
        resource_uri, app_id, app_secret
    )
...
def authenticate_device_code(authenticationInfo):
    ...
    print(
        "To authenticate, open a browser to this URL:\n{}".format(
            device_login_url + "?input=" + code["user_code"]
        )
    )

    mgmt_token = context.acquire_token_with_device_code(
        resource_uri, code, authenticationInfo.client_id
    )
...
def get_subscription_id(credentials):
    ...
    while subscription_id is None:
        if first_run:
            ...
            subscription_id = find_subscription_by_name(name.strip(), credentials)
