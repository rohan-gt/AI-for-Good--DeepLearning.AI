...
def list_containers(account_name, sas_token, required_string=None):
    ...
    blob_service_client = BlobServiceClient(
        account_url=storage_account_url_blob, credential=sas_token
    )
...
def list_blobs_in_container(
    container_name, account_name, sas_token, output_folder, prefix=None
):
    if not sas_token.startswith("?"):
        sas_token = "?" + sas_token
    ...
    print(
        "Finished enumerating {} blobs for container {}".format(i_blob, container_name)
    )
...
def list_blobs_threads(account_name, sas_token, containers, output_folder):
    ...
    for container_name in containers:
        ...
        t = Thread(
            name=container_name,
            target=list_blobs_in_container,
            args=(
                container_name,
                account_name,
                sas_token,
...
if __name__ == "__main__":
    ...
    parser.add_argument("account_name", help="Name of the target storage account")
    parser.add_argument(
        "sas_token",
