...
assert (not downloadImages) or (not len(imageOutputDir) == 0), (
    "Can't have empty output dir if you're downloading images"
)
...
def PrepareFolderDownload(folderID, folderTargetDir, dataEnumerator=None):
    ...
    try:
        ...
        errorString = str(ex)
        print(
            "Error listing directory {}:{}:{}".format(
                folderTargetDir, folderID, errorString
            )
        )
    ...
    for f in fileList:
        ...
        if title in titles:
            ...
            if bMergeDuplicateFolders:
                print(
                    "Warning: folder conflict at {}/{}".format(folderTargetDir, title)
                )
            else:
                ...
                print(
                    "Renaming {} to {} in [{}]".format(title, newTitle, folderTargetDir)
                )
                title = newTitle
                f["title"] = title
        else:
...
if (enumerationMode == "ifnecessary") and (os.path.exists(downloadListFile)):
    ...
    # Either we're in 'all' mode or we're in 'ifnecessary' mode and enumeration is necessary
        ...
        print(
            "Finished enumeration in {}".format(
    ...
    with open(errorListFile, "w+") as f:
        for e in dataEnumerator.errors:
            f.write(",".join(e) + "\n")
    print(
        "Finished writing error list ({} errors) to {}".format(
            len(dataEnumerator.errors), errorListFile
        )
    )
...
print(
    "Total download size is {} in {} files".format(
