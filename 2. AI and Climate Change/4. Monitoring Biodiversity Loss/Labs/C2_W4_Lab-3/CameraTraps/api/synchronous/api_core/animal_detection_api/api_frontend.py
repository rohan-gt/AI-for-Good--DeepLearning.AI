...
def check_posted_data(request):
    ...
    if not content_length:
        return _make_error_object(
            411, "No image(s) were sent, or content length cannot be determined."
        )
    if content_length > config.MAX_CONTENT_LENGTH_IN_MB * 1024 * 1024:
        return _make_error_object(
            ...
            (
                ...
                "Please upload fewer or more compressed images."
            ).format(content_length / (1024 * 1024), config.MAX_CONTENT_LENGTH_IN_MB),
        )
    ...
    if "min_confidence" in params:
        ...
        if return_confidence_threshold < 0.0 or return_confidence_threshold > 1.0:
            return _make_error_object(
                400,
                "Detection confidence threshold {} is invalid, should be between 0.0 and 1.0.".format(
                    return_confidence_threshold
                ),
            )
    else:
    ...
    if "min_rendering_confidence" in params:
        ...
        print(
            "runserver, post_detect_sync, user specified rendering confidence: ",
            rendering_confidence_threshold,
        )
        if rendering_confidence_threshold < 0.0 or rendering_confidence_threshold > 1.0:
            return _make_error_object(
                400,
                "Rendering confidence threshold {} is invalid, should be between 0.0 and 1.0.".format(
                    rendering_confidence_threshold
                ),
            )
    else:
    ...
    num_images = sum(
        [
            1 if file.content_type in config.IMAGE_CONTENT_TYPES else 0
    ...
    if num_images > config.MAX_IMAGES_ACCEPTED:
        return _make_error_object(
            413,
            "Too many images. Maximum number of images that can be processed in one call is {}.".format(
                config.MAX_IMAGES_ACCEPTED
            ),
        )
    elif num_images == 0:
        return _make_error_object(
...
def detect_sync():
    ...
    if post_data.get("error_code", None) is not None:
        return _make_error_response(
            post_data.get("error_code"), post_data.get("error_message")
        )
        ...
        try:
            ...
            for name, file in request.files.items():
                if file.content_type in config.IMAGE_CONTENT_TYPES:
                    ...
                    assert os.path.isfile(image_path), "Error creating file {}".format(
                        image_path
                    )
            ...
            if result:
                ...
                try:
                    ...
                    if render_boxes and result["status"] == 200:
                        ...
                        for image_name, detections in detections.items():
                            ...
                            for d in detections:
                                ...
                                _detections.append(
                                    {"bbox": bbox, "conf": d[4], "category": d[5]}
                                )
                    ...
                    try:
                        ...
                        print(
                            "Error removing temporary folder {}: {}".format(
                                temp_direc, str(e)
                            )
                        )
