...
class JobStatusTable:
    ...
    def __init__(self, api_instance=None):
        self.api_instance = (
            api_instance if api_instance is not None else API_INSTANCE_NAME
        )
    ...
    def create_job_status(
        self, job_id: str, status: Union[dict, str], call_params: dict
    ) -> dict:
    ...
    def read_job_status(self, job_id) -> Optional[dict]:
        ...
        try:
            ...
            assert read_item["api_instance"] == self.api_instance, (
                "Job does not belong to this API instance"
            )
...
class TestJobStatusTable(unittest.TestCase):
    ...
    def test_insert(self):
        ...
        status = {"request_status": "running", "message": "this is a test"}
        job_id = uuid.uuid4().hex
        item = table.create_job_status(
            job_id, status, {"container_sas": "random_string"}
        )
        self.assertTrue(job_id == item["id"], "Expect job_id to be the id of the item")
        self.assertTrue(
            item["status"]["request_status"] == "running",
            "Expect fields to be inserted correctly",
        )
    ...
    def test_update_and_read(self):
        ...
        status = {"request_status": "running", "message": "this is a test"}
        job_id = uuid.uuid4().hex
        res = table.create_job_status(
            job_id, status, {"container_sas": "random_string"}
        )

        status = {"request_status": "completed", "message": "this is a test again"}
        res = table.update_job_status(job_id, status)
        item_read = table.read_job_status(job_id)
        self.assertTrue(
            item_read["status"]["request_status"] == "completed",
            "Expect field to have updated",
        )
