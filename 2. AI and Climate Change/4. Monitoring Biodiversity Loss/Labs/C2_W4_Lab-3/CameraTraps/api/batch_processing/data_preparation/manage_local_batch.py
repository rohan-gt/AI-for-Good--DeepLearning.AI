...
if False:
    ...
    print(
        "Loaded {} image files from chunks in {}".format(len(all_images), filename_base)
    )
...
print(
    "Expected time per chunk: {}".format(
        humanfriendly.format_timespan(seconds_per_chunk)
    )
)
...
for i_task, task in enumerate(task_info):
    ...
    cmd_file = os.path.join(
        filename_base,
        "run_chunk_{}_gpu_{}.sh".format(str(i_task).zfill(2), str(gpu_number).zfill(2)),
    )
    ...
    resume_cmd_file = os.path.join(
        filename_base,
        "resume_chunk_{}_gpu_{}.sh".format(
            str(i_task).zfill(2), str(gpu_number).zfill(2)
        ),
    )
...
if False:
    ...
    for i_task, task in enumerate(task_info):
        ...
        print(
            "Task {}: finished inference for {} images in {}".format(
...
assert n_total_failures < max_tolerable_failed_images, (
    "{} failures (max tolerable set to {})".format(
        n_total_failures, max_tolerable_failed_images
    )
)

print(
    "Processed all {} images with {} failures".format(len(all_images), n_total_failures)
)
...
for i_task, task in enumerate(task_info):
    ...
    assert (
        task_results["info"]["format_version"]
        == combined_results["info"]["format_version"]
    )
    assert (
        task_results["detection_categories"] == combined_results["detection_categories"]
    )
...
combined_api_output_file = os.path.join(
    combined_api_output_folder, "{}_detections.json".format(base_task_name)
)
...
rde_string = "rde_{:.2f}_{:.2f}_{}_{:.2f}".format(
    ...
    options.occurrenceThreshold,
    options.maxSuspiciousDetectionSize,
)
options.outputBase = os.path.join(
    filename_base, rde_string + "_task_{}".format(task_index)
)
...
output_file = os.path.join(
    filename_base, "run_{}_".format(classifier_name_short) + job_name + ".sh"
)
...
target_mapping_path = os.path.join(
    classifier_base, "idfg_to_megaclassifier_labels.json"
)
...
for fn in input_files:
    input_file_path = fn
    classifier_output_path = crop_path + classifier_output_suffix
    classifier_output_path_remapped = classifier_output_path.replace(
        ".csv.gz", "_remapped.csv.gz"
    )
...
for fn in input_files:
    ...
    final_output_path = os.path.join(
        output_base, os.path.basename(classifier_output_path)
    ).replace(classifier_output_suffix, final_output_suffix)
    ...
    merge_cmd += (
        ...
        + " \\\n"
        + '--threshold "'
        + classification_threshold_str
        + '"'
        + " \\\n"
        + '--typical-confidence-threshold "'
        + typical_classification_threshold_str
        + '"'
        + " \\\n"
        + "\n"
    )
...
classifier_base = os.path.expanduser(
    "~/models/camera_traps/idfg_classifier/idfg_classifier_20200905_042558"
)
assert os.path.isdir(classifier_base)
...
classification_detection_files = [final_output_path_mc, final_output_path_ic]
...
for final_output_path in classification_detection_files:
    ...
    category_name_to_id = {
        d["classification_categories"][k]: k for k in d["classification_categories"]
    }
    other_category_ids = []
    for s in other_category_names:
        if s in category_name_to_id:
            ...
            print(
                'Warning: "other" category {} not present in file {}'.format(
    ...
    for im in tqdm(d["images"]):
        if (
            "detections" not in im
            or im["detections"] is None
            or len(im["detections"]) == 0
        ):
            continue
        ...
        for det in detections:
            if ("classifications" in det) and (
                det["conf"] >= detection_confidence_threshold
            ):
        ...
        category_to_count = {
            k: v
            for k, v in sorted(
                category_to_count.items(), key=lambda item: item[1], reverse=True
            )
        }
        ...
        if (
            (len(keys) > 1)
            and (keys[0] in other_category_ids)
            and (keys[1] not in other_category_ids)
            and (category_to_count[keys[0]] == category_to_count[keys[1]])
        ):
        ...
        if (
            max_count >= min_detections_to_overwrite_other
            and most_common_category not in other_category_ids
        ):
            other_change_made = False

            for det in detections:
                if ("classifications" in det) and (
                    det["conf"] >= detection_overwrite_threshold
                ):
                    for c in det["classifications"]:
                        if (
                            c[1] >= classification_overwrite_threshold
                            and c[0] in other_category_ids
                        ):
    ...
    print(
        "Classification smoothing: changed {} detections on {} images".format(
            n_detections_flipped, n_images_changed
        )
    )

    print(
        '"Other" smoothing: changed {} detections on {} images'.format(
...
for classification_detection_file in classification_detection_files:
    ...
    folder_token = classification_detection_file.split("/")[-1].replace(
        "classifier.json", ""
    )

    output_base = os.path.join(
        postprocessing_output_folder,
        folder_token + base_task_name + "_{:.3f}".format(options.confidence_threshold),
    )
    os.makedirs(output_base, exist_ok=True)
...
size_separated_file = input_file.replace(
    ".json", "-size-separated-{}.json".format(size_options.threshold)
)
d = categorize_detections_by_size.categorize_detections_by_size(
    input_file, size_separated_file, size_options
)


# %% Preview large boxes

output_base_large_boxes = os.path.join(
    postprocessing_output_folder,
    base_task_name
    + "_{}_{:.3f}_large_boxes".format(rde_string, options.confidence_threshold),
)
...
from api.batch_processing.postprocessing.subset_json_detector_output import (
    ...
    SubsetJsonDetectorOutputOptions,
)
...
for i_folder, folder_name in enumerate(folders):
    ...
    print(
        "Processing folder {} of {} ({}) to {}".format(
            i_folder, len(folders), folder_name, output_filename
        )
    )
    ...
    subset_data = subset_json_detector_output(
        input_filename, output_filename, options, data
    )
...
from api.batch_processing.postprocessing.separate_detections_into_folders import (
    ...
    SeparateDetectionsIntoFoldersOptions,
)

default_threshold = 0.2
base_output_folder = os.path.expanduser(
    "~/data/{}-{}-separated".format(base_task_name, default_threshold)
)
...
for i_task in task_set:
    ...
    if checkpoint_frequency is not None and checkpoint_frequency > 0:
        checkpoint_frequency_string = f"--checkpoint_frequency {checkpoint_frequency}"
        checkpoint_path_string = "--checkpoint_path {}".format(
            chunk_file.replace(".json", "_checkpoint.json")
        )
...
cmd_file = os.path.join(
    filename_base,
    "run_chunk_{}_gpu_{}.sh".format(task_set_string, str(gpu_number).zfill(2)),
)
...
input_py_file = os.path.expanduser(
    "~/git/CameraTraps/api/batch_processing/data_preparation/manage_local_batch.py"
)
