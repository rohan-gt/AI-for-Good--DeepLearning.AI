...
if input_file_lists is not None:
    ...
    for folder_name in folder_names:
        ...
        if (
            len(container_prefix) > 0
            and len(folder_name) > 0
            and container_prefix[-1] != "/"
        ):
            print(
                "Warning: no trailing slash in container prefix {} used with folder name {}".format(
        ...
        print(
            "Enumerated {} images for folder {}".format(len(folder_images), folder_name)
        )
...
for i, taskgroup_json_paths in enumerate(folder_chunks):
    ...
    for j, task_json_path in enumerate(taskgroup_json_paths):
        ...
        if len(task_name) > max_task_name_length:
            ...
            print(
                "Warning: task name {} too long, shortened to {}".format(
                    long_task_name, task_name
                )
            )
        ...
        task = prepare_api_submission.Task(
            name=task_name, images_list_path=task_json_path, api_url=endpoint_base
        )

        blob_name = "api_inputs/{}/{}".format(base_task_name, task_json_filename)
        print(
            "Task {}: uploading {} to {}".format(task_name, task_json_path, blob_name)
        )
...
if False:
    for i_taskgroup, taskgroup in enumerate(taskgroups):
        for i_task, task in enumerate(taskgroup):
            ...
            if not (
                (i_taskgroup == len(taskgroups) - 1) and (i_task == len(taskgroup) - 1)
            ):
...
if False:
    ...
    print(
        "Combined {} taskgroups into one, totalling {} tasks".format(
            in_length, len(taskgroups[0])
        )
    )
...
for i_taskgroup, taskgroup in enumerate(taskgroups):
    ...
    for i_task, task in enumerate(tasks):
        ...
        if n_failed_shards != 0:
            print(
                "Warning: {} failed shards for task {}".format(n_failed_shards, task.id)
            )
        ...
        assert (folder_name in detections_fn) or (
            clean_folder_name in detections_fn.replace("_", "")
        )
        ...
        missing_images = task.get_missing_images(
            submitted_images=submitted_images, verbose=True
        )
        missing_images_by_task.append(missing_images)
        ai4e_azure_utils.write_list_to_file(missing_images_fn, missing_images)

        missing_images_file = os.path.join(
            filename_base, "missing_images_{}.json".format(task.id)
        )
        ...
        if num_missing_images > max_tolerable_missing_images:
            print(
                "Warning: {} images missing from task {}".format(
                    num_missing_images, task.id
                )
            )
        ...
        failed_images_file = os.path.join(
            filename_base, "failed_images_{}.json".format(task.id)
        )
        ...
        failed_images_list_file = os.path.join(
            filename_base, "failed_images_list_{}.json".format(task.id)
        )
...
for i_taskgroup, taskgroup in enumerate(taskgroups):
    for task in taskgroup:
        ...
        assert (folder_name in fn) or (
            clean_folder_name in fn.replace(".", "").replace("_", "")
        )
...
for i_taskgroup, taskgroup in enumerate(taskgroups):
        ...
        with open(list_file, "r") as f:
            ...
            for s in requested_images_set_this_task:
                if s in requested_images_set:
                    assert any([s in x for x in missing_images_by_task_sets]) or any(
                        [s in x for x in failed_images_by_task_sets]
                    )

            requested_images_set = requested_images_set.union(
                requested_images_set_this_task
            )
    ...
    if len(missing_images) > 0:
        print(
            "Warning: {} missing images for folder {}".format(
                len(missing_images), folder_name
            )
        )
...
for i_folder, folder_name_raw in enumerate(folder_names):
    ...
    if folder_name in folder_name_to_filtered_output_filename:
        ...
        output_base = os.path.join(
            ...
            folder_token
            + base_task_name
            + "_{}_{:.3f}".format(rde_string, options.confidence_threshold),
        )
    else:
        ...
        output_base = os.path.join(
            ...
            folder_token
            + base_task_name
            + "_{:.3f}".format(options.confidence_threshold),
        )
...
size_separated_file = input_file.replace(
    ".json", "-size-separated-{}.json".format(options.threshold)
)
d = categorize_detections_by_size.categorize_detections_by_size(
    input_file, size_separated_file, options
)
