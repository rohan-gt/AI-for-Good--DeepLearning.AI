...
def api_results_groupby(
    api_output_path, gt_db_indexed, file_to_image_id, field="seq_id"
):
    """
...
def load_api_results(
    ...
    print(
        "Finished loading and de-serializing API results for {} images from {}".format(
...
def write_api_results(detection_results_table, other_fields, out_path):
    ...
    images = detection_results_table.to_json(orient="records", double_precision=3)
...
def load_api_results_csv(
    filename, normalize_paths=True, filename_replacements={}, nrows=None
):
    """
    DEPRECATED
    ...
    # Normalize paths to simplify comparisons later
    if normalize_paths:
        detection_results["image_path"] = detection_results["image_path"].apply(
            os.path.normpath
        )
