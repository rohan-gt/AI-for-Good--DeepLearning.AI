...
from api.batch_processing.postprocessing.load_api_results import (
    load_api_results,
    write_api_results,
)
...
class IndexedDetection:
    ...
    def __init__(
        self, iDetection=-1, filename="", bbox=[], confidence=-1, category="unknown"
    ):
        """
        Args:
...
class DetectionLocation:
    ...
    def to_api_detection(self):
        ...
        detection = {
            "conf": self.instances[0].confidence,
            "bbox": self.bbox,
            "category": self.instances[0].category,
        }
        return detection
...
def render_bounding_box(
    detection, inputFileName, outputFileName, lineWidth=5, expansion=0
):
    im = open_image(inputFileName)
    d = detection.to_api_detection()
    render_detection_bounding_boxes(
        [d], im, thickness=lineWidth, expansion=expansion, confidence_threshold=-10
    )
...
def sort_detections_for_directory(candidateDetections, options):
    ...
    if options.smartSort == "xsort":
        candidateDetectionsSorted = sorted(
            candidateDetections, key=lambda x: ((x.bbox[0]) + (x.bbox[2] / 2.0))
        )
        ...
        for det in candidateDetections:
            ...
            # Center
            points.append(
                [det.bbox[0] + det.bbox[2] / 2.0, det.bbox[1] + det.bbox[3] / 2.0]
            )
        X = np.array(points)
        ...
        for label in unique_labels:
            detections_this_label = [
                d for d in candidateDetections if (d.clusterLabel == label)
            ]
            ...
            centroid = [
                sum(x) / len(points_this_label),
                sum(y) / len(points_this_label),
            ]
        ...
        for old_cluster_label in unique_labels:
            ...
            old_cluster_label_to_new_cluster_label[old_cluster_label] = np.where(
                new_cluster_labels == old_cluster_label
            )[0][0]
        ...
        candidateDetectionsSorted = sorted(
            candidateDetections, key=lambda x: (x.clusterLabel, x.id)
        )

        return candidateDetectionsSorted

    else:
        raise ValueError("Unrecognized sort method {}".format(options.smartSort))
...
def find_matches_in_directory(dirName, options, rowsByDirectory):
    ...
    if (
        options.maxImagesPerFolder is not None
        and len(rows) > options.maxImagesPerFolder
    ):
        print(
            "Ignoring directory {} because it has {} images (limit set to {})".format(
    ...
    if options.includeFolders is not None:
        assert options.excludeFolders is None, (
            "Cannot specify include and exclude folder lists"
        )
    ...
    if options.excludeFolders is not None:
        assert options.includeFolders is None, (
            "Cannot specify include and exclude folder lists"
        )
    ...
    for iDirectoryRow, row in rows.iterrows():
        ...
        if (
            "max_detection_conf" not in row
            or "detections" not in row
            or row["detections"] is None
        ):
        ...
        for iDetection, detection in enumerate(detections):
            ...
            assert (
                "category" in detection and "conf" in detection and "bbox" in detection
            )
            ...
            overlappingCandidateDetections = candidateDetectionsIndex.intersect(
                rtree_rect
            )

            overlappingCandidateDetections.sort(key=lambda x: x.id, reverse=False)

            # For each detection in our candidate list
            for iCandidate, candidate in enumerate(overlappingCandidateDetections):
                ...
                try:
                    ...
                    print(
                        "Warning: IOU computation error on boxes ({},{},{},{}),({},{},{},{}): {}".format(
                            bbox[0],
                            bbox[1],
                            bbox[2],
                            bbox[3],
...
def render_images_for_directory(
    ...
    for iDetection, detection in enumerate(suspiciousDetectionsThisDir):
        if (
            options.debugMaxRenderDetection > 0
            and iDetection > options.debugMaxRenderDetection
        ):
            break

        nInstances = len(detection.instances)
        print(
            "Processing detection {} of {} ({} instances)".format(
        ...
        for iInstance, instance in enumerate(detection.instances):
            if (
                options.debugMaxRenderInstance >= 0
                and iInstance >= options.debugMaxRenderInstance
            ):
                break

            imageRelativeFilename = "image{:0>4d}.jpg".format(iInstance)
            imageOutputFilename = os.path.join(detectionBaseDir, imageRelativeFilename)
            thisImageInfo = {}
                ...
                if not os.path.isfile(inputFileName):
                    if options.bPrintMissingImageWarnings:
                        if (options.missingImageWarningType == "all") or (
                            ...
                            print(
                                "Warning: could not find file {}".format(inputFileName)
                            )
        ...
        write_html_image_list.write_html_image_list(
            detectionHtmlFile, imageInfo, htmlOptions
        )
    ...
    htmlOptions["defaultImageStyle"] = "max-width:650px;"
    write_html_image_list.write_html_image_list(
        directoryHtmlFile, directoryDetectionImageInfo, htmlOptions
    )
...
def update_detection_table(RepeatDetectionResults, options, outputFilename=None):
    ...
    for iRow, row in detectionResults.iterrows():
        ...
        if abs(maxP - maxPOriginal) > 1e-3:
            ...
            if (maxPOriginal >= options.confidenceMin) and (
                maxP < options.confidenceMin
            ):
    ...
    if outputFilename is not None and len(outputFilename) > 0:
        write_api_results(
            detectionResults, RepeatDetectionResults.otherFields, outputFilename
        )
...
def find_repeat_detections(inputFilename, outputFilename=None, options=None):
    ...
    if options.filterFileToLoad is not None and len(options.filterFileToLoad) > 0:
        print(
            "Bypassing detection-finding, loading from {}".format(
    ...
    if options.bWriteFilteringFolder or options.bRenderHtml:
        if not is_sas_url(options.imageBase):
            ...
            assert os.path.isfile(absolutePath), "Could not find file {}".format(
                absolutePath
            )
    ...
    for iRow, row in detectionResults.iterrows():
        ...
        if len(dirName) == 0:
            assert options.nDirLevelsFromLeaf == 0, (
                "Cant use the dirLevelsFromLeaf option with flat filenames"
            )
        else:
            if options.nDirLevelsFromLeaf > 0:
                iLevel = 0
    ...
    if options.customDirNameFunction is not None:
        print(
            "Custom dir name function made {} replacements (of {} images)".format(
    ...
    print(
        "Finished separating {} files into {} directories".format(
    ...
    if len(options.filterFileToLoad) == 0:
        ...
        if not options.bParallelizeComparisons:
            ...
            allCandidateDetections = Parallel(
                n_jobs=options.nWorkers, prefer="threads"
            )(
        ...
        for iDir in range(len(dirsToSearch)):
            ...
            print(
                "Found {} suspicious detections in directory {} ({})".format(
    ...
    # If we're just loading detections from a file...
        ...
        if options.filteredFileListToLoad is not None:
            ...
            print(
                "Loaded false positive list from file "
        ...
        for iDir, detections in enumerate(suspiciousDetections):
            ...
            if nRemovedThisDir > 0:
                print(
                    "Removed {} of {} detections from directory {}".format(
        ...
        print(
            "Removed {} of {} total detections via manual filtering".format(
    ...
    if options.bRenderHtml:
        ...
        if options.bParallelizeRendering:
            ...
            directoryHtmlFiles = Parallel(n_jobs=options.nWorkers, prefer="threads")(
                delayed(render_images_for_directory)(
                    iDir, directoryHtmlFiles, suspiciousDetections, options
                )
        ...
        for iDir, suspiciousDetectionsThisDir in enumerate(tqdm(suspiciousDetections)):
            ...
            for iDetection, detection in enumerate(suspiciousDetectionsThisDir):
                ...
                if is_sas_url(options.imageBase):
                    ...
                    assert os.path.isfile(inputFullPath), "Not a file: {}".format(
                        inputFullPath
                    )
                ...
                try:
                    ...
                    print(
                        "Warning: error rendering bounding box from {} to {}: {}".format(
