...
def process_file(relative_filename, options, execute=True):
    assert (
        ("/" in relative_filename)
        and ("\\" not in relative_filename)
        and (not path_is_abs(relative_filename))
    )
...
def top_folders_to_bottom(options):
    ...
    absolute_output_files = [
        process_file(s, options, execute=False) for s in relative_files
    ]
    ...
    if options.n_threads <= 1:
        ...
        _ = list(
            tqdm(
                pool.imap(process_file_with_options, relative_files),
...
if False:
    ...
    options = TopFoldersToBottomOptions(
        input_folder, output_folder, copy=True, n_threads=10
    )
...
def main():
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="Allow image overwrite (default=False)"
    )
    parser.add_argument(
        "--n_threads",
        type=int,
