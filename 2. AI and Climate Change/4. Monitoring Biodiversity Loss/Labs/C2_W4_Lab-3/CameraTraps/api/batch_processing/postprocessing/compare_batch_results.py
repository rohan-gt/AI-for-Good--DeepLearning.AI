...
def _compare_batch_results(options, output_index, pairwise_options):
    ...
    max_classification_threshold_a = max(
        list(pairwise_options.detection_thresholds_a.values())
    )
    max_classification_threshold_b = max(
        list(pairwise_options.detection_thresholds_b.values())
    )

    if (
        pairwise_options.rendering_confidence_threshold_a
        > max_classification_threshold_a
    ):
        print(
            "*** Warning: rendering threshold A ({}) is higher than max confidence threshold A ({}) ***".format(
                pairwise_options.rendering_confidence_threshold_a,
                max_classification_threshold_a,
            )
        )
    ...
    if (
        pairwise_options.rendering_confidence_threshold_b
        > max_classification_threshold_b
    ):
        print(
            "*** Warning: rendering threshold B ({}) is higher than max confidence threshold B ({}) ***".format(
                pairwise_options.rendering_confidence_threshold_b,
                max_classification_threshold_b,
            )
        )
    ...
    for fn in tqdm(filenames_a):
        ...
        for det in im_a["detections"]:
            ...
            if category_id not in detection_categories:
                print(
                    "Warning: unexpected category {} for model A on file {}".format(
                        category_id, fn
                    )
                )
            ...
            if (
                ...
                >= pairwise_options.detection_thresholds_a[
                    detection_categories[category_id]
                ]
            ):
        ...
        for det in im_b["detections"]:
            ...
            if category_id not in detection_categories:
                print(
                    "Warning: unexpected category {} for model B on file {}".format(
                        category_id, fn
                    )
                )
                invalid_category_error = True
    ...
    print(
        "Of {} files:\n{} common detections\n{} common non-detections\n{} A only\n{} B only\n{} class transitions".format(
            len(filenames_a),
            len(common_detections),
            len(common_non_detections),
            len(detections_a_only),
            len(detections_b_only),
            len(class_transitions),
        )
    )
    ...
    local_output_folder = os.path.join(
        options.output_folder, "cmp_" + str(output_index).zfill(3)
    )
    ...
    def render_detection_comparisons(category, image_pairs, image_filenames):
        ...
        def render_image_pair(fn):
            ...
            assert os.path.isfile(input_image_path), "Image {} does not exist".format(
                input_image_path
            )
            ...
            visualization_utils.render_detection_bounding_boxes(
                detections_a,
                im,
        ...
        if options.n_rendering_threads <= 1:
            ...
            output_image_paths = list(
                tqdm(
                    pool.imap(render_image_pair, image_filenames),
    ...
    for category in categories_to_image_pairs.keys():
        ...
        if len(image_filenames) > options.max_images_per_category:
            print(
                "Sampling {} of {} image pairs for category {}".format(
                    options.max_images_per_category, len(image_filenames), category
                )
            )
            image_filenames = random.sample(
                image_filenames, options.max_images_per_category
            )
        assert len(image_filenames) <= options.max_images_per_category

        input_image_absolute_paths = [
            os.path.join(options.image_folder, fn) for fn in image_filenames
        ]

        category_image_output_paths = render_detection_comparisons(
            category, image_pairs, image_filenames
        )

        category_html_filename = os.path.join(local_output_folder, category + ".html")
        category_image_output_paths_relative = [
            os.path.relpath(s, local_output_folder) for s in category_image_output_paths
        ]

        image_info = []

        assert len(category_image_output_paths_relative) == len(
            input_image_absolute_paths
        )
        ...
        for i_fn, fn in enumerate(category_image_output_paths_relative):
            ...
            title = input_path_relative + " (max conf {:.2f},{:.2f})".format(
                max_conf_a, max_conf_b
            )
            info = {
                "filename": fn,
                "title": title,
        ...
        category_page_header_string = "<h1>{}</h1>".format(
            categories_to_page_titles[category]
        )
        ...
        write_html_image_list(
            ...
            images=image_info,
            options={"headerHtml": category_page_header_string},
        )
    ...
    html_output_string += (
        "<p>Comparing <b>{}</b> (A, red) to <b>{}</b> (B, blue)</p>".format(
            pairwise_options.results_description_a,
            pairwise_options.results_description_b,
        )
    )
    ...
    comparison_path_relative = os.path.relpath(
        local_output_folder, options.output_folder
    )
    for category in categories_to_image_pairs.keys():
        category_html_filename = os.path.join(
            comparison_path_relative, category + ".html"
        )
...
if False:
    ...
    pairwise_options.results_filename_a = os.path.expanduser(
        "~/postprocessing/snapshot-safari/snapshot-safari-2022-04-07/combined_api_outputs/snapshot-safari-2022-04-07_detections.kru-only.json"
    )
    pairwise_options.results_filename_b = os.path.expanduser(
        "~/postprocessing/snapshot-safari/snapshot-safari-mdv5_torchscript_camonly-2022-04-14/combined_api_outputs/snapshot-safari-mdv5_torchscript_camonly-2022-04-14_detections.json"
    )
