...
from analyze_image_detection_using_confidence import (
    compute_precision_recall_with_images,
)
from analyze_sequence_detection_using_confidence import (
    compute_precision_recall_with_sequences,
)
...
if __name__ == "__main__":
    ...
    prec, recall, ap = compute_precision_recall(
        detection_file, detection_results=detection_results
    )
    im_prec, im_recall, im_ap, im_scores = compute_precision_recall_with_images(
        detection_file, detection_results=detection_results
    )
    seq_prec, seq_recall, seq_ap, seq_scores = compute_precision_recall_with_sequences(
        detection_file, db_file, detection_results=detection_results
    )
    ...
    print(
        "Prec. at ", im_recall[recall_idx], " recall with images: ", im_prec[recall_idx]
    )
    recall_idx = np.argmin([np.abs(x - recall_thresh) for x in seq_recall])
    print(
        "Prec. at ",
        seq_recall[recall_idx],
        " recall with sequences: ",
        seq_prec[recall_idx],
    )
    recall_thresh = 0.97
    recall_idx = np.argmin([np.abs(x - recall_thresh) for x in recall])
    ...
    plt.legend()
    plt.title(
        "%0.2f mAP per object, %0.2f mAP per image, %0.2f mAP per sequence"
        % (ap, im_ap, seq_ap)
    )
    plt.savefig(det_folder + exp_name + "_PR_obj_im_seq_w_conf.jpg")

    np.savez(
        det_folder + exp_name + "_obj_im_seq_prec_recall_data_with_conf.npz",
        prec=prec,
        recall=recall,
        ap=ap,
        im_prec=im_prec,
