...
def compute_precision_recall_with_sequences(
    ...
    use_im = get_images_to_consider(
        detection_results, images_to_consider, get_night_day
    )
        ...
        if best_score > 0:
            ...
            for i in range(len(seqs[seq])):
                ...
                for j in range(min(seq_num_gts[i], len(temp_labels))):
                    temp_labels[j] = (
                        True  # TODO: this currently only works for oneclass?
                    )
    ...
    print(len(seqs.keys()))
    print(
        sum([1 for i in range(len(detection_labels)) if detection_labels[i] == True]),
        num_total_gts,
    )
    precision, recall = metrics.compute_precision_recall(scores, labels, num_total_gts)
...
if __name__ == "__main__":
    inter_prec, inter_recall, inter_ap = compute_precision_recall_with_sequences(
        det_folder + "cis_test.p", db_file
    )
    loc_prec, loc_recall, loc_ap = compute_precision_recall_with_sequences(
        det_folder + "trans_test.p", db_file
    )
    print("Cis mAP: ", inter_ap, ", Trans mAP: ", loc_ap)
    recall_thresh = 0.9
    recall_idx = np.argmin([np.abs(x - recall_thresh) for x in inter_recall])
    print(
        "Cis prec. at ", inter_recall[recall_idx], " recall: ", inter_prec[recall_idx]
    )
    ...
    np.savez(
        det_folder + "seq_cis_v_trans_prec_recall_data.npz",
        cis_prec=inter_prec,
        cis_recall=inter_recall,
        cis_ap=inter_ap,
        trans_prec=loc_prec,
        trans_recall=loc_recall,
        trans_ap=loc_ap,
    )
