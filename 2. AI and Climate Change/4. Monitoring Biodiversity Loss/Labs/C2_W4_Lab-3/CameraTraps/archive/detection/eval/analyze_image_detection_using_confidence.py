...
def compute_precision_recall_with_images(
    detection_file, detection_results=None, images_to_consider="all", get_night_day=None
):
    ...
    im_id_to_box_scores = {
        im: per_image_detections[im]["scores"] for im in per_image_detections
    }
    ...
    average_precision = metrics.compute_average_precision(
        np.asarray(prec), np.asarray(rec)
    )
...
if __name__ == "__main__":
    inter_prec, inter_recall, inter_ap, _ = compute_precision_recall_with_images(
        det_folder + "cis_test.p"
    )
    loc_prec, loc_recall, loc_ap, _ = compute_precision_recall_with_images(
        det_folder + "trans_test.p"
    )
    ...
    np.savez(
        det_folder + "im_cis_v_trans_prec_recall_data_with_conf_thresh.npz",
        cis_prec=inter_prec,
