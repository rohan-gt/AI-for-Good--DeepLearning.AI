    ...
    for cat, images in cat_to_ims.iteritems():
        for image_id in images:
            ...
            scores, tp_fp_labels = get_results_per_image(
                per_image_detections[image_id], per_image_gts[image_id], per_image_eval
            )
...
if __name__ == "__main__":
    prec, recall, ap, cat_id_to_cat = compute_precision_recall_per_cat(
        detection_file, db_file
    )
    for cat in ap:
    ...
    np.savez(
        det_folder + exp_name + "_per_cat_prec_recall_data.npz",
        prec=prec,
        recall=recall,
        ap=ap,
        cat_id_to_cat=cat_id_to_cat,
    )
