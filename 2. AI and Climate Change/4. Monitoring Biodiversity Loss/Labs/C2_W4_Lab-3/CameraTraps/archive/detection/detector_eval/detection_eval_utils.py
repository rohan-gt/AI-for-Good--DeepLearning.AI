...
def get_non_person_images(results):
    ...
    print(
        "Number of bbox of person excluded: {}, images excluded: {}".format(
            num_boxes_excluded, num_images_excluded
        )
    )
    return use_im
...
def group_detections_by_image(results, use_im=None):
    ...
    per_image_detections = {
        results["images"][idx]: {
            "boxes": results["detection_boxes"][idx],
            "scores": results["detection_scores"][idx],
            "labels": results["detection_labels"][idx],
        }
        for idx in range(len(results["images"]))
        if use_im[results["images"][idx]]
    }

    # group the ground truth annotations by image id
    per_image_gts = {
        results["images"][idx]: {
            "gt_boxes": results["gt_boxes"][idx],
            "gt_labels": results["gt_labels"][idx],
        }
        for idx in range(len(results["images"]))
        if use_im[results["images"][idx]]
    }
