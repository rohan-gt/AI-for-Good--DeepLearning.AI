...
def prepare_api_output_for_timelapse(inputFilename, outputFilename, options):
    ...
    tqdm.pandas()
    detectionResults = detectionResults.progress_apply(
        lambda x: process_row(x, options), axis=1
    )

    print("Finished main loop, post-processing output")

    # Trim to matching rows
    detectionResults = detectionResults.loc[
        detectionResults[options.temporaryMatchColumn]
    ]
    print(
        "Trimmed to {} matching rows (from {})".format(
            len(detectionResults), nRowsLoaded
        )
    )
...
if False:
    ...
    detectionResults = prepare_api_output_for_timelapse(
...
def main():
    ...
    prepare_api_output_for_timelapse(
        args.inputFile, args.outputFile, args.query, options
    )
