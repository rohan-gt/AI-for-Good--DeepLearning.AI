...
def add_classification_categories(json_object, classes_file):
    ...
    if ("classification_categories" not in json_object.keys()) or (
        len(json_object["classification_categories"]) == 0
    ):
        ...
        print(
            "WARNING: The input json already contains the list of classification categories."
        )
        ...
        with tf.Session(graph=classification_graph) as sess:
            ...
            for iImage in tqdm.tqdm(list(range(0, nImages))):
                ...
                for iBox in range(nDetections):
                    ...
                    if (
                        "classifications" in cur_detection.keys()
                        and len(cur_detection["classifications"]) > 0
                    ):
                        continue
                    ...
                    box_coords_abs = box_coords * np.tile(
                    ...
                    predictions = sess.run(
                        predictions_tensor, feed_dict={image_tensor: cropped_img}
                    )
...
def load_and_run_classifier(
    ...
    print(
        "Done running detector and classifier in {}".format(
...
def main():
    parser = argparse.ArgumentParser(
        description="Applies a classifier to all detected boxes of the detection API output (JSON format)."
    )
    parser.add_argument(
        "classifier_file",
        type=str,
    ...
    parser.add_argument(
        "detector_json_file",
        type=str,
        help="JSON file that was produced by the detection API.",
    )
    parser.add_argument(
        "output_json_file",
        type=str,
        help="Path to output file, will be in JSON format.",
    )
    parser.add_argument(
        "--image_dir",
        action="store",
        type=str,
        default="",
        help='Base directory of the images. Default: ""',
    )
    parser.add_argument(
        "--threshold",
        action="store",
        ...
        help="Confidence threshold, don't classify detections below this confidence. Default: %.2f"
    ...
    parser.add_argument(
        ...
        help="Enlargement factor for bounding boxes before they are passed to the classifier. Default: %.2f"
        % PADDING_FACTOR,
    )
    parser.add_argument(
        "--num_annotated_classes",
        ...
        help="Number of top-scoring classes to add to the output for each bounding box, default: %d"
        % NUM_ANNOTATED_CLASSES,
    )
    parser.add_argument(
        "--detection_category_whitelist",
        type=str,
        nargs="+",
        default=DETECTION_CATEGORY_WHITELIST,
        help="We will run the detector on all detections with these detection categories, default: "
        + " ".join(DETECTION_CATEGORY_WHITELIST),
    )
    args = parser.parse_args()

    load_and_run_classifier(
        classifier_file=args.classifier_file,
        classes_file=args.classes_file,
        image_dir=args.image_dir,
        detector_json_file=args.detector_json_file,
        output_json_file=args.output_json_file,
        confidence_threshold=args.threshold,
        padding_factor=args.padding_factor,
        num_annotated_classes=args.num_annotated_classes,
        detection_category_whitelist=args.detection_category_whitelist,
    )
