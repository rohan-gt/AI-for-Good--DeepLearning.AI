...
def generate_detections(detection_graph, images):
    ...
    for iBox, box in enumerate(boxes):
        ...
        assert nDetections == -1 or nDetectionsThisBox == nDetections, (
    ...
    with classification_graph.as_default():
        with tf.Session(graph=classification_graph) as sess:
            ...
            for iImage in range(0, nImages):
                ...
                selected_boxes = boxes[iImage][selection] * np.tile(
                ...
                for iBox in range(len(boxes[iImage])):
                    ...
                    if selection[iBox]:
                        ...
                        predictions = sess.run(
                            predictions_tensor, feed_dict={image_tensor: input_image}
                        )
    ...
    for iImage in range(0, nImages):
        ...
        for iBox, box in enumerate(boxes[iImage]):
            ...
            for species_id in (-box_species_scores).argsort()[:num_annotated_classes]:
                box_text.append(
                    "{} {:.1%}".format(
                        class_names[species_id], box_species_scores[species_id]
                    )
                )
            box_text = "\n".join(box_text)
            # Choose color based on class
            edge_color = plt.get_cmap("rainbow")(
                box_species_scores.argmax() / box_species_scores.size
            )
            ...
            best_corner = corners[best_corner_idx] * np.array(
            ...
            ax.annotate(
                box_text,
                xy=best_corner,
                bbox=dict(boxstyle="round", color=edge_color, ec="0.5", alpha=1),
                **best_alignment,
            )
...
def load_and_run_detector(
    ...
    startTime = time.time()
    boxes, scores, detection_classes, images = generate_detections(
        detection_graph, image_file_names
    )
    species_scores = classify_boxes(
        classification_graph,
        boxes,
        scores,
        detection_classes,
    ...
    print(
        "Done running detector and classifier on {} files in {}".format(
            len(images), humanfriendly.format_timespan(elapsed)
        )
    )
...
if False:
    ...
    image_file_names = [
        r"./sample_output/mongoose___S1___R11___R11_R1___S1_R11_R1_PICT0101_1.JPG"
    ]

    detection_graph, classification_graph = load_and_run_detector(
        detection_file,
        classification_file,
        classes_file,
        image_file_names,
        DEFAULT_CONFIDENCE_THRESHOLD,
        NUM_ANNOTATED_CLASSES,
        detection_graph,
        classification_graph,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Runs both a detector and a classifier on a given image."
    )
    parser.add_argument(
        "detector_file",
        type=str,
        help="Generic detector for animals.",
        metavar="PATH_TO_DETECTOR",
    )
    parser.add_argument(
        "classifier_file",
        type=str,
        help="Frozen graph for classification including pre-processing. The graphs "
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="File with the class names. Each line should contain "
    ...
    parser.add_argument(
        ...
        help="Confidence threshold, don't render boxes below this confidence. Default: %.2f"
        % DEFAULT_CONFIDENCE_THRESHOLD,
    )
    parser.add_argument(
        "--num_annotated_classes",
        action="store",
        type=int,
        default=NUM_ANNOTATED_CLASSES,
        help="Number of classes to annotated for each bounding box, default: %d"
        % NUM_ANNOTATED_CLASSES,
    )
    parser.add_argument(
        "--recursive",
    ...
    image_file_names = [
        x for x in image_file_names if DETECTION_FILENAME_INSERT not in x
    ]
    ...
    load_and_run_detector(
        ...
        num_annotated_classes=args.num_annotated_classes,
    )
