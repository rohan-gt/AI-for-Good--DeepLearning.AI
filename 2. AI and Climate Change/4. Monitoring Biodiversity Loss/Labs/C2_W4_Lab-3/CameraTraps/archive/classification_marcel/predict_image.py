...
parser = argparse.ArgumentParser(
    description="Given a pointer to a frozen detection graph, runs inference on a single image and prints results."
)
parser.add_argument(
    "--frozen_graph",
    type=str,
        ...
        for class_id in np.argsort(-predictions)[:5]:
            print(
                '    "{}" with confidence {:.2f}%'.format(
                    classlist[class_id], predictions[class_id] * 100
                )
            )
