...
from archive.data_management.tfrecords.create_classification_tfrecords_format import (
    create_classification_tfrecords_from_json,
)
...
database_file = (
    datafolder
    + "databases/snapshotserengeti/oneclass/SnapshotSerengeti_Seasons_1_to_4_tfrecord_format_valid_files.json"
)
...
if "tfrecord_format" in database_file:
    ...
    if experiment_type == "classification":
        ...
        json.dump(
            data,
            open(
                "/ai4efs/databases/snapshotserengeti/oneclass/SnapshotSerengeti_Seasons_1_to_4_tfrecord_format.json",
                "w",
            ),
        )
    else:
...
print(data[0])
data_split = json.load(
    open(
        datafolder
        + "databases/snapshotserengeti/oneclass/SnapshotSerengeti_Seasons_1_to_4_classification_train_test_split.json"
    )
)
...
json.dump(
    valid_dataset,
    open(
        "/ai4efs/databases/snapshotserengeti/oneclass/SnapshotSerengeti_Seasons_1_to_4_tfrecord_format_valid_ims_val.json",
        "w",
    ),
)
