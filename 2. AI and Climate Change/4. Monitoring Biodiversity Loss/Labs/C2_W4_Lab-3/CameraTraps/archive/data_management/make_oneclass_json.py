...
def make_binary_json(data, experiment_type="detection", ignore_humans=False):
    ...
    for ann in data["annotations"]:
        if experiment_type == "classification":
            ...
            if ("bbox" in ann) and (
                cat_id_to_name[ann["category_id"]] not in ["empty"]
            ):
                ann["category_id"] = 1
                new_anns.append(ann)
            else:
...
if False:
    ...
    input_file = os.path.join(
        base_dir, "imerit_batch7_renamed_uncorrupted_filtered.json"
    )
    output_file = os.path.join(
        base_dir, "imerit_batch7_renamed_uncorrupted_filtered_oneclass.json"
    )
    ignore_humans = True
    ...
    print(
        "Wrote {} annotations (of {} original annotations) to {}".format(
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert a multiclass .json to a oneclass .json"
    )
