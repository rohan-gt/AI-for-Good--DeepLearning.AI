...
output_file = (
    "/datadrive/snapshotserengeti/databases/imerit_annotation_images_ss_2.json"
)
...
cats_per_location = {
    loc: {cat: list(set(cats_per_location[loc][cat])) for cat in cats_per_location[loc]}
...
for loc in loc_to_seqs:
    lion_seqs = (
        cats_per_location[loc][cat_to_id["lionMale"]]
        + cats_per_location[loc][cat_to_id["lionFemale"]]
    )
    ...
    lion_seqs_to_annotate.extend(
        random.sample(lion_seqs, min(len(lion_seqs), num_lions))
    )
...
for loc in loc_to_seqs:
    ...
    elephant_seqs = [
        seq for seq in elephant_seqs if seq_to_season[seq] in seasons_to_keep
    ]
    elephant_seqs_to_annotate.extend(
        random.sample(elephant_seqs, min(len(elephant_seqs), num_elephants))
    )
