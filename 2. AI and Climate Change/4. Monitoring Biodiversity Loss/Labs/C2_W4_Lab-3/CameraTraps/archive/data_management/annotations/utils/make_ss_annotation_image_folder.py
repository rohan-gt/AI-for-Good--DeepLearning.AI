...
image_input_folder = os.path.join(
    BASE_DIR, "imerit_batch7_snapshotserengeti_2018.10.26", "to_label"
)
...
print(
    "Finished reading {} annotations and {} images from input file {}".format(
...
for im in data["images"]:
    ...
    assert len(tokens) == 4
    new_filename = (
        tokens[0] + "/" + tokens[1] + "/" + tokens[1] + "_" + tokens[2] + "/" + m[0]
    )
...
print(
    "Done processing {} files in {}".format(
        len(data["images"]), humanfriendly.format_timespan(elapsed)
    )
)
