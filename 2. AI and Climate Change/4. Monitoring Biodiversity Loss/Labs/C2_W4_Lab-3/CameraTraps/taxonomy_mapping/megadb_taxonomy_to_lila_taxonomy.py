...
input_taxonomy_file = (
    r"G:\git\agentmorrisprivate\lila-taxonomy\lila-taxonomy-mapping-input.csv"
)

# Created by get_lila_category_list.py
input_lila_category_list_file = (
    r"G:\temp\lila\lila_categories_list\lila_dataset_to_categories.json"
)

output_taxonomy_file = (
    r"G:\git\agentmorrisprivate\lila-taxonomy\lila-taxonomy-mapping.csv"
)
        ...
        if taxonomy_string != previous_taxonomy_string:
            ...
            if source == previous_source:
                if len(taxonomy_string) > len(previous_taxonomy_string):
                    print(
                        "For mapping {}, replacing {}\n{}\n\nwith\n\n{}\n{}\n".format(
                            mapping_string,
                            previous_source,
                            previous_taxonomy_string,
                            source,
                            taxonomy_string,
                        )
                    )
...
assert not os.path.isfile(output_taxonomy_file), (
    "You don't really want to overwrite the output file"
)
output_entries = []
