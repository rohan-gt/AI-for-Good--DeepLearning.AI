...
if False:
    ...
    for source_mapping in source_mappings:
        assert source_mapping != target_mapping and source_mapping in valid_mappings
    ...
    for i_row, row in df.iterrows():
        ...
        if m is None or m.source != target_mapping:
            print(
                "No mapping for {} ({}) ({})".format(
                    scientific_name, row["query"], old_common
                )
            )
            continue
        ...
        if row["taxonomy_string"] != m.taxonomy_string:
            print(
                "Remapping {} ({} to {})".format(
                    scientific_name, old_common, new_common
                )
            )
            n_remappings += 1
    ...
    df.to_csv(
        lila_taxonomy_file.replace(".csv", "_remapped.csv"), header=True, index=False
    )
...
# i_row = 0; row = df.iloc[i_row]
for i_row, row in df.iterrows():
    if (not isinstance(row["taxonomy_string"], str)) or (
        len(row["taxonomy_string"]) == 0
    ):
...
for i_row, row in df.iterrows():
    ...
    if (isinstance(ts, str)) and (len(ts) >= 0):
        if (not isinstance(cn, str)) or (len(cn) == 0):
            print(
                "No mapping for {}:{}:{}".format(
                    row["dataset_name"], row["query"], row["scientific_name"]
                )
            )
    ...
    for previous_i_row in query_to_rows[query]:
        ...
        if not query_match:
            ...
            for x in suppress_multiple_matches:
                if x[0] == query and (
                    (
                        x[1] == row["dataset_name"]
                        and x[2] == previous_row["dataset_name"]
                    )
                    or (
                        x[2] == row["dataset_name"]
                        and x[1] == previous_row["dataset_name"]
                    )
                ):
                    suppress = True
                    n_suppressed += 1
            ...
            if not suppress:
                print(
                    "Query {} in {} and {}:\n\n{}\n\n{}\n".format(
                        query,
                        row["dataset_name"],
                        previous_row["dataset_name"],
                        taxonomy_string,
                        previous_row["taxonomy_string"],
                    )
                )
...
print(
    "Found {} queries with multiple mappings ({} occurrences suppressed)".format(
...
for i_row, row in df.iterrows():
    ...
    if (
        "unidentified" in query
        or ("unk" in query and ("skunk" not in query and "chipmunk" not in query))
    ) and ("species" in level):
        if query not in allowable_unknown_species:
            print(
                "Warning: query {}:{} maps to {} {}".format(
                    row["dataset_name"],
                    row["query"],
                    row["taxonomy_level"],
                    row["scientific_name"],
...
for i_row, row in df.iterrows():
    ...
    if (
        (" sp" not in query)
        and ("unknown" not in query)
        and (level != "species")
        and (level != "subspecies")
    ):
        print(
            "WCS query {} ({}) remapped to {} {} ({})".format(
...
for i_row, row in df.iterrows():
    ...
    paths = retrieve_sample_image.download_images(
        query=query, output_directory=image_base, limit=images_per_query, verbose=True
    )
...
for s in list(df.scientific_name):
    ...
    paths_by_size_non_dup = [
        i for (i, v) in zip(paths_by_size, b_duplicate_sizes) if not v
    ]
...
print(
    "{} of {} files unused (diff {})".format(
        len(unused_images), len(all_images), len(all_images) - len(unused_images)
    )
)
...
with open(html_output_file, "w") as f:
    ...
    f.write(
        '<p class="speciesinfo_p" style="font-weight:bold;font-size:130%">'
