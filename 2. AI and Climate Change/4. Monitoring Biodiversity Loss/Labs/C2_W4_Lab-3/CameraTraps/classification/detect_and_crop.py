...
from api.batch_processing.data_preparation.prepare_api_submission import (
    BatchAPIResponseError,
    Task,
    TaskStatus,
    divide_list_into_tasks,
)
    ...
    if run_detector:
        ...
        if os.path.exists(resume_file_path):
            tasks_by_dataset = resume_tasks(
                resume_file_path, batch_detection_api_url=batch_detection_api_url
            )
        else:
        ...
        wait_for_tasks(
            tasks_by_dataset, detector_output_cache_dir, output_dir=output_dir
        )
...
def load_detection_cache(
    detector_output_cache_dir: str, datasets: Collection[str]
...
def filter_detected_images(
    potential_images_to_detect: Iterable[str], detector_output_cache_dir: str
    ...
    """
    datasets = set(
        img_path[: img_path.find("/")] for img_path in potential_images_to_detect
    )
...
def split_images_list_by_dataset(
    images_to_detect: Iterable[str],
) -> dict[str, list[str]]:
    """
    Args:
...
def submit_batch_detection_api(
    images_to_detect: Iterable[str],
    task_lists_dir: str,
    detector_version: str,
    account: str,
...
def submit_batch_detection_api_by_dataset(
    ...
    for i, task_list_path in enumerate(task_list_paths):
        task = Task(
            ...
            images_list_path=task_list_path,
...
def resume_tasks(
    resume_file_path: str, batch_detection_api_url: str
) -> dict[str, list[Task]]:
    """
    Args:
...
def wait_for_tasks(
    tasks_by_dataset: Mapping[str, Iterable[Task]],
    ...
    remaining_tasks: list[tuple[str, Task]] = [
        (dataset, task) for dataset, tasks in tasks_by_dataset.items() for task in tasks
    ]
    ...
    while True:
        ...
        for dataset, task in remaining_tasks:
            try:
                ...
                tqdm.write(
                    f"Error in checking status of task {task.id}: "
                    f"({exception_type}) {e}"
                )
            ...
            if num_failed_shards != 0:
                tqdm.write(
                    f"Task {task.id} completed with {num_failed_shards} failed shards."
                )
...
def download_and_crop(
    ...
    for img_path in valid_img_paths:
        ...
        if "bbox" in info_dict:  # ground-truth bounding boxes
            ...
            raise ValueError(
                f"{img_path} has no ground truth bounding boxes "
