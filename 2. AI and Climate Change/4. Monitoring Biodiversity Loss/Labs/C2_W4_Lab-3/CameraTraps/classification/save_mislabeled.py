...
def update_mislabeled_images(container_path: str, input_csv_path: str) -> None:
    ...
    for ds, ds_df in df.groupby("dataset"):
        ...
        if os.path.exists(sr_path):
            ...
            old_sr = pd.Series(
                index=pd.Index([], name="file"), dtype="str", name="correct_class"
            )
        ...
        new_sr = pd.concat([old_sr, ds_sr.loc[new_indices]], verify_integrity=True)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "container_path", help="path to locally-mounted classifier-training container"
    )
    parser.add_argument("input_csv", help="path to CSV file output by Timelapse")
    return parser.parse_args()


if __name__ == "__main__":
    args = _parse_args()
    update_mislabeled_images(
        container_path=args.container_path, input_csv_path=args.input_csv
    )
