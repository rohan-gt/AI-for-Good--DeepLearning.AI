...
def main(
    logdir: str, splits: Iterable[str], margin: float, include_dataset_in_filename: bool
) -> None:
    # load files
    ...
    for split in splits:
        ...
        for ds in sorted(candidate_image_files.keys()):
            ...
            save_path = os.path.join(logdir, f"mislabeled_candidates_{split}_{ds}.txt")
...
def get_candidates_df(
    outputs_csv_path: str, label_names: Sequence[str], margin: float
) -> pd.DataFrame:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "logdir", help="folder inside <base_logdir> containing `outputs_<split>.csv.gz`"
    )
    parser.add_argument(
        "--margin",
        type=float,
