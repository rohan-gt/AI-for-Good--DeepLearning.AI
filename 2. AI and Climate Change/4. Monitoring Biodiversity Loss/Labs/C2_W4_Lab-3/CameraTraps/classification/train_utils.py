...
def fig_to_img(fig: matplotlib.figure.Figure) -> np.ndarray:
    ...
    with io.BytesIO() as b:
        fig.savefig(
            b, transparent=False, bbox_inches="tight", pad_inches=0, format="png"
        )
        b.seek(0)
...
def imgs_with_confidences(
    imgs_list: list[tuple[Any, ...]],
...
def plot_img_grid(
    ...
    fig = matplotlib.figure.Figure(
        figsize=(ncols * col_w, nrows * row_h), tight_layout=True
    )
...
def load_dataset_csv(
    ...
    label_weighted: bool,
) -> tuple[pd.DataFrame, list[str], dict[str, set[tuple[str, str]]]]:
    """
    Args:
