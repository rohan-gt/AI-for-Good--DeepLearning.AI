...
def main(
    classifier_results_csv_path: str,
    target_mapping_json_path: str,
    output_csv_path: str,
    output_label_index_json_path: str,
) -> None:
    """Main function.
    ...
    for i, chunk_df in tqdm(enumerate(chunked_df_iterator)):
        ...
        agg_df = pd.DataFrame(data=0.0, index=chunk_df.index, columns=target_names)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "classifier_results_csv", help="path to CSV with classifier probabilities"
    )
    parser.add_argument(
        "-t",
        "--target-mapping",
...
if __name__ == "__main__":
    args = _parse_args()
    main(
        classifier_results_csv_path=args.classifier_results_csv,
