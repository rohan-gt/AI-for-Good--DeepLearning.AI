...
def main(
    ...
    """
    # error checking
    assert 0 <= confidence_threshold <= 1, "Invalid confidence threshold {}".format(
    ...
    for img_path in list(detections.keys()):
        ...
        for d in info_dict["detections"]:
            if d["category"] not in detection_categories:
                print(
                    "Warning: ignoring detection with category {} for image {}".format(
                        d["category"], img_path
                    )
                )
...
def download_and_crop(
    ...
    crop_path_template = {
        ...
        False: os.path.join(
            cropped_images_dir, "{img_path}___crop{n:>02d}_" + f"{detector_version}.jpg"
        ),
    }
    ...
    for img_path in tqdm(sorted(detections.keys())):
        ...
        future = pool.submit(
            ...
            bbox_dicts,
            confidence_threshold,
            crop_path_template[is_ground_truth],
            save_full_images,
            square_crops,
            check_crops_valid,
        )
...
def save_crop(
    img: Image.Image, bbox_norm: Sequence[float], square_crop: bool, save: str
) -> bool:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("detections_json", help="path to detections JSON file")
    parser.add_argument(
        "cropped_images_dir",
    ...
    parser.add_argument(
        "--square-crops", action="store_true", help="crop bounding boxes as squares"
    )
    parser.add_argument(
        "--check-crops-valid",
        action="store_true",
    ...
    parser.add_argument(
        "--logdir", default=".", help="path to directory to save log file"
    )
