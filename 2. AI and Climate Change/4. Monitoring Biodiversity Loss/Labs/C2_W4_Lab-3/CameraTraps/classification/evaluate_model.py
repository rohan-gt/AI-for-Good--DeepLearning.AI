...
def check_override(params: Mapping[str, Any], key: str, override: Optional[Any]) -> Any:
...
def trace_model(
    model_name: str, ckpt_path: str, num_classes: int, img_size: int
) -> str:
    ...
    model = train_classifier.build_model(
        model_name, num_classes=num_classes, pretrained=ckpt_path, finetune=False
    )
...
def main(
    ...
    output_dir: str,
    splits: Sequence[str],
    target_mapping_json_path: Optional[str] = None,
    ...
    **kwargs: Any,
    ...
    try:
        ...
        print(
            "Warning: could not start accimage backend (ignore this if you're not using Linux)"
        )
    ...
    for key, new in kwargs.items():
        ...
        if key in params:
            saved = params[key]
            print(f"Overriding saved {key}. Saved: {saved}. Override with: {new}.")
        else:
    ...
    for split in splits:
        ...
        assert np.isclose(
            metrics["acc_top1"] / 100.0, sum(preds == df["label"]) / len(df)
        )
        assert np.isclose(
            metrics["acc_weighted_top1"] / 100.0,
        ...
        for batch in tqdm_loader:
            ...
            if target_mapping is not None:
                outputs_mapped = torch.zeros(
                    [batch_size, num_labels], dtype=outputs.dtype, device=outputs.device
                )
                for target, cols in target_mapping.items():
                    outputs_mapped[:, target] = torch.logsumexp(outputs[:, cols], dim=1)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("params_json", help="path to params.json")
    parser.add_argument(
        "ckpt_path", help="path to checkpoint file (normal or TorchScript-compiled)"
    )
    parser.add_argument(
        "-o", "--output-dir", required=True, help="(required) path to output directory"
    )
    parser.add_argument(
        "--splits",
        nargs="*",
    ...
    other_classifier.add_argument(
        "--label-index", help="path to label index JSON file for classifier"
    )
    ...
    override_group.add_argument(
        "--model-name", help="which EfficientNet or Resnet model"
    )
    override_group.add_argument(
        "--batch-size", type=int, help="batch size for evaluating model"
    )
    override_group.add_argument(
        "--num-workers", type=int, help="number of workers for data loading"
    )
    override_group.add_argument(
        "--dataset-dir",
