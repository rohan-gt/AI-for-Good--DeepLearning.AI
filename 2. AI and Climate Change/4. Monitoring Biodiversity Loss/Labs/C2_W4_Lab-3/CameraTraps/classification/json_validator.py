...
from taxonomy_mapping.taxonomy_graph import build_taxonomy_graph, dag_to_tree, TaxonNode


def main(
    label_spec_json_path: str,
...
def parse_spec(
    spec_dict: Mapping[str, Any], taxonomy_dict: dict[tuple[str, str], TaxonNode]
) -> set[tuple[str, str]]:
...
def validate_json(
    input_js: dict[str, dict[str, Any]],
    ...
    for ds in tqdm(sorted(ds_to_labels.keys())):  # sort for determinism
        ...
        if mislabeled_images_dir is not None:
            ...
            if os.path.exists(csv_path):
                mislabeled_images = pd.read_csv(
                    csv_path, index_col="file", squeeze=True
                )
...
def remove_non_images(
    js: MutableMapping[str, dict[str, Any]], log: MutableMapping[str, Any]
) -> None:
...
def remove_nonexistent_images(
    js: MutableMapping[str, dict[str, Any]],
    ...
    def check_local_then_azure(local_path: str, blob_url: str) -> bool:
        return os.path.exists(local_path) or sas_blob_utils.check_blob_exists(blob_url)
...
def remove_images_insufficient_locs(
    js: MutableMapping[str, dict[str, Any]],
    log: MutableMapping[str, Any],
    min_locs: int,
) -> None:
    ...
    bad_labels = set(
        label for label, locs in label_to_locs.items() if len(locs) < min_locs
    )
...
def filter_images(
    output_js: Mapping[str, Mapping[str, Any]],
    label: str,
    datasets: Container[str] | None = None,
) -> set[str]:
...
def sample_with_priority(
    input_js: Mapping[str, Mapping[str, Any]], output_js: Mapping[str, dict[str, Any]]
) -> dict[str, dict[str, Any]]:
    ...
    output_js = {img_file: output_js[img_file] for img_file in sorted(filtered_imgs)}
    return output_js
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "label_spec_json", help="path to JSON file containing label specification"
    )
    parser.add_argument("taxonomy_csv", help="path to taxonomy CSV file")
    parser.add_argument(
        "--allow-multilabel",
        action="store_true",
...
if __name__ == "__main__":
    args = _parse_args()
    main(
        label_spec_json_path=args.label_spec_json,
