...
def row_to_classification_list(
    row: Mapping[str, Any],
    ...
    if contains_label and label_pos is not None:
        ...
        item = (str(label_id + 1_000_000), 1.0)
        if label_pos == "first":
            result = [item] + result
        else:
            result.append(item)
...
def main(
    ...
    label_pos: str | None,
    relative_conf: bool,
    typical_confidence_threshold: float,
) -> None:
    """Main function."""
    # input validation
    ...
    df = pd.read_csv(classification_csv_path, float_precision="high", index_col="path")
    ...
    classification_js = combine_classification_with_detection(
        detection_js=detection_js,
        df=df,
        idx_to_label=idx_to_label,
        label_names=label_names,
        classifier_name=classifier_name,
        classifier_timestamp=classifier_timestamp,
        threshold=threshold,
        label_pos=label_pos,
    ...
    print(
        "Wrote merged classification/detection results to {}".format(output_json_path)
    )
...
def process_queried_images(
    ...
    for ds in datasets:
        ...
        with open(os.path.join(detection_cache_dir, f"{ds}.json"), "r") as f:
            detection_js = json.load(f)
        img_file_to_index = {
            im["file"]: idx for idx, im in enumerate(detection_js["images"])
        }
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Merges classification results with Batch Detection API outputs.",
    )
    parser.add_argument("classification_csv", help="path to classification CSV")
    parser.add_argument(
        "label_names_json", help="path to JSON file mapping label index to label name"
    )
    parser.add_argument(
        "-o",
        "--output-json",
    ...
    parser.add_argument(
        "-n", "--classifier-name", required=True, help="(required) name of classifier"
    )
    parser.add_argument(
        "-t",
        "--threshold",
    ...
    detection_json_group.add_argument(
        "-j", "--detection-json", help="path to detections JSON file"
    )
    ...
    queried_images_group.add_argument(
        "-q", "--queried-images-json", help="path to queried images JSON file"
    )
    queried_images_group.add_argument(
        "-c",
