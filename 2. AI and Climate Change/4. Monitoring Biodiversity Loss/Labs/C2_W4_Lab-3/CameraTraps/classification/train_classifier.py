...
def create_dataloaders(
    ...
    train_transform = tv.transforms.Compose(
        [
            ...
            tv.transforms.ColorJitter(brightness=0.25, contrast=0.25, saturation=0.25),
...
def set_finetune(model: torch.nn.Module, model_name: str, finetune: bool) -> None:
...
def build_model(
    model_name: str, num_classes: int, pretrained: bool | str, finetune: bool
) -> torch.nn.Module:
...
def prep_device(
    model: torch.nn.Module, device_id: int | None = None
    ...
    if torch.cuda.is_available():
        ...
        if device_id is not None:
            ...
            if len(device_ids) > 1:
                print(
                    f"Found multiple devices, enabling data parallelism ({device_ids})"
                )
...
def main(
    ...
    if "efficientnet" in model_name:
        optimizer = torch.optim.RMSprop(
            model.parameters(), lr, alpha=0.9, momentum=0.9, weight_decay=weight_decay
        )
        ...
        optimizer = torch.optim.SGD(
            model.parameters(), lr, momentum=0.9, weight_decay=weight_decay
        )
        lr_scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer=optimizer, step_size=8, gamma=0.1
        )  # lower every 8 epochs
...
def log_run(
    ...
    cm_fig = plot_utils.plot_confusion_matrix(cm, classes=label_names, normalize=True)
    cm_fig_img = fig_to_img(cm_fig)
    writer.add_image(
        tag=f"confusion_matrix/{split}",
        img_tensor=cm_fig_img,
        global_step=epoch,
        dataformats="HWC",
    )
    ...
    if heaps is not None:
        for heap_type, heap_dict in heaps.items():
            log_images_with_confidence(
                writer, heap_dict, label_names, epoch=epoch, tag=f"{split}/{heap_type}"
            )
    writer.flush()
...
def log_images_with_confidence(
    ...
    for label_id, heap in heap_dict.items():
        ...
        writer.add_image(
            f"{label_name}/{tag}", fig_to_img(fig), global_step=epoch, dataformats="HWC"
        )
        writer.add_text(
            f"{label_name}/{tag}_files", "\n\n".join(img_files), global_step=epoch
        )


def track_extreme_examples(
    tp_heaps: dict[int, list[HeapItem]],
...
def run_epoch(
    ...
    k_extreme: int = 0,
) -> tuple[dict[str, float], dict[str, dict[int, list[HeapItem]]] | None, np.ndarray]:
    """Runs for 1 epoch.
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "cropped_images_dir", help="path to local directory where image crops are saved"
    )
    parser.add_argument(
        "--multilabel",
        action="store_true",
    ...
    parser.add_argument(
        "--num-workers", type=int, default=8, help="# of workers for data loading"
    )
    parser.add_argument(
        "--logdir",
        default=".",
