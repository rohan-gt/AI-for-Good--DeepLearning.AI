    ...
    transform = tv.transforms.Compose(
        [
            ...
            tv.transforms.Normalize(
                mean=train_classifier.MEANS, std=train_classifier.STDS, inplace=True
            ),
        ]
    )

    dataset = SimpleDataset(
        img_files=crop_files, images_dir=cropped_images_dir, transform=transform
    )
...
def test_epoch(
    ...
    with torch.no_grad():
        for inputs, img_files in tqdm(loader):
            ...
            df = pd.DataFrame(
                data=probs, columns=label_names, index=pd.Index(img_files, name="path")
            )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("model", help="path to TorchScript compiled model")
    parser.add_argument("crops_dir", help="path to directory containing cropped images")
    parser.add_argument(
        "output",
    ...
    parser.add_argument(
        "-d",
        "--detections-json",
        help="path to detections JSON file, used to filter paths within crops_dir",
    )
    parser.add_argument(
        "-c",
    ...
    parser.add_argument(
        "--batch-size", type=int, default=1, help="batch size for evaluating model"
    )
    parser.add_argument(
        "--device", type=int, default=None, help="preferred CUDA device"
    )
    parser.add_argument(
