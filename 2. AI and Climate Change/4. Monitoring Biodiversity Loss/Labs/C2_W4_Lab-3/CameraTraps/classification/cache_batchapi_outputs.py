...
def cache_detections(
    detections: Mapping[str, Any], dataset: str, detector_output_cache_dir: str
) -> str:
    """
    Args:
    ...
    dataset_cache_path = os.path.join(detector_output_cache_dir, f"{dataset}.json")
    ...
    if os.path.exists(dataset_cache_path):
        ...
        msg = (
            "No cached detection outputs found. Saving detection output to "
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "json_file", help="path to JSON file containing response of Batch Detection API"
    )
    parser.add_argument(
        "-f",
        "--format",
        choices=["response", "detections"],
        required=True,
