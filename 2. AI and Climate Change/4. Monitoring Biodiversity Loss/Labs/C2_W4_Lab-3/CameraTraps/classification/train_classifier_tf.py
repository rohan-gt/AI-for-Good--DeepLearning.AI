...
def create_dataset(
    ...
    img_ds = img_ds.map(
        lambda p: tf.io.read_file(img_base_dir + os.sep + p),
...
def create_dataloaders(
    ...
    def train_transform(img: tf.Tensor) -> tf.Tensor:
        ...
        img = tf.image.resize_with_pad(
            img, img_size, img_size, method=tf.image.ResizeMethod.BICUBIC
        )
...
def build_model(
    model_name: str, num_classes: int, img_size: int, pretrained: bool, finetune: bool
) -> tf.keras.Model:
...
def main(
    ...
    optimizer = tf.keras.optimizers.RMSprop(learning_rate=lr, rho=0.9, momentum=0.9)
...
def log_run(
    ...
    heaps: Mapping[str, Mapping[int, list[HeapItem]]],
    cm: np.ndarray,
) -> None:
    ...
    for heap_type, heap_dict in heaps.items():
        log_images_with_confidence(
            heap_dict, label_names, epoch=epoch, tag=f"{split}/{heap_type}"
        )
    writer.flush()
...
def run_epoch(
...
) -> tuple[dict[str, float], dict[str, dict[int, list[HeapItem]]], np.ndarray]:
    """Runs for 1 epoch.
    ...
    if optimizer is not None:
        ...
        if not finetune:
            ...
            reg_vars = [v for v in model.trainable_variables if "kernel" in v.name]
    ...
    for batch in tqdm_loader:
        ...
        if return_extreme_images:
            track_extreme_examples(
                tp_heaps, fp_heaps, fn_heaps, inputs, labels, img_files, outputs
            )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pretrained", action="store_true", help="start with pretrained model"
    )
    parser.add_argument(
        "--finetune",
        type=int,
