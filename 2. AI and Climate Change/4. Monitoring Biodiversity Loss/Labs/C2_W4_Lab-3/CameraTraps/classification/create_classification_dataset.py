...
def main(
    ...
    if match_test is not None:
        ...
        match_df = pd.read_csv(
            match_test_csv_path, index_col=False, float_precision="high"
        )
...
def create_classification_csv(
    ...
    columns = ["path", "dataset", "location", "dataset_class", "confidence", "label"]
    ...
    if min_locs is not None:
        ...
        print(
            f"Excluding {orig - len(df)} crops from {len(invalid_labels)} labels:",
            invalid_labels.tolist(),
        )
...
def create_splits_random(
    ...
    loc_label_counts = (
        df.groupby(["label", "dataset_location"]).size().unstack("label", fill_value=0)
    )
    ...
    for _ in tqdm(range(10_000)):
        ...
        num_train = int(num_locs * (train_frac + np.random.uniform(-0.03, 0.03)))
        if test_frac > 0:
            num_val = int(num_locs * (val_frac + np.random.uniform(-0.03, 0.03)))
        else:
            num_val = num_locs - num_train
        permuted_locs = loc_label_counts.index[np.random.permutation(num_locs)]
        split_to_locs = {
            "train": permuted_locs[:num_train],
...
def create_splits_smallest_label_first(
    ...
    seen_locs = set()
    split_to_locs: dict[str, list[tuple[str, str]]] = dict(train=[], val=[], test=[])
    label_sizes_by_split = {
        label: dict(train=0, val=0, test=0) for label in df["label"].unique()
    }
    if test_split is not None:
        assert test_frac == 0
...
def sort_locs_by_size(
    loc_to_size: MutableMapping[tuple[str, str], int],
    ...
    if prioritize is not None:
        ...
        prioritized_loc_to_size = {
            loc: loc_to_size.pop(loc)
            for loc in list(loc_to_size.keys())
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--match-test",
        nargs=2,
        metavar=("CLASSIFICATION_CSV", "SPLITS_JSON"),
    ...
    csv_group = parser.add_argument_group("arguments for creating classification CSV")
