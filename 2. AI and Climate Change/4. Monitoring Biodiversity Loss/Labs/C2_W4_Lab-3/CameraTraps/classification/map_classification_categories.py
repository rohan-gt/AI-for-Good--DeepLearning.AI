...
def main(
    desired_label_spec_json_path: str,
    classifier_label_spec_json_path: str,
    taxonomy_csv_path: str,
    output_json_path: str,
    classifier_label_index_path: Optional[str],
) -> None:
    """Main function."""
    ...
    if classifier_label_index_path is not None:
        ...
        if len(classifier_labels) < len(classifier_spec):
            classifier_spec = {
                k: v for k, v in classifier_spec.items() if k in classifier_labels
            }
    ...
    for target, target_nodes in tqdm(target_label_to_nodes.items()):
        for label, classifier_nodes in classifier_label_to_nodes.items():
            ...
            if len(overlap) == len(classifier_nodes):
                ...
                raise ValueError(
                    "Only partial overlap between target "
...
def label_spec_to_nodes(
    label_spec_js: dict[str, dict[str, Any]],
    taxon_to_node: dict[tuple[str, str], TaxonNode],
    label_to_node: dict[tuple[str, str], TaxonNode],
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Create mapping from target categories to classifier labels.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="path to JSON file containing label specification of a trained classifier",
    )
...
if __name__ == "__main__":
    args = _parse_args()
    main(
        desired_label_spec_json_path=args.desired_label_spec_json,
