...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "input_csv_file", help="path to CSV file containing label specification"
    )
    parser.add_argument("output_json_path", help="path to save converted JSON file")
    parser.add_argument(
        "--json-indent",
        type=int,
...
def parse_csv_row(obj: dict[str, Any], rowtype: str, content: str) -> None:
    ...
    if rowtype == "row":
        ...
        obj["taxa"].append({"level": taxon_level, "name": taxon_name, "datasets": [ds]})
        ...
        taxon_level = rowtype
        taxon_name = content
        obj["taxa"].append({"level": taxon_level, "name": taxon_name})
