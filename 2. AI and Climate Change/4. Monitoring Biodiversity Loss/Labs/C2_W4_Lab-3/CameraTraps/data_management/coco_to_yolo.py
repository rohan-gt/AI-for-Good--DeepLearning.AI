...
def coco_to_yolo(input_image_folder, output_folder, input_file):
    ...
    for i_ann, ann in enumerate(data["annotations"]):
        ...
        if (
            ("segmentation" in ann.keys())
            and (ann["segmentation"] is not None)
            and (len(ann["segmentation"]) > 0)
        ) and (
            ("bbox" not in ann.keys())
            or (ann["bbox"] is None)
            or (len(ann["bbox"]) == 0)
        ):
            raise ValueError(
                "Oops: segmentation data present without bbox information, this script isn't ready for this dataset"
            )
    ...
    for i_image, im in tqdm(enumerate(data["images"]), total=len(data["images"])):
        ...
        image_name = (
            tokens[0].replace("\\", "/").replace("/", "_") + "_" + str(i_image).zfill(6)
        )
        assert image_name not in image_names, "Image name collision for {}".format(
            image_name
        )
        ...
        if image_id in image_id_to_annotations:
            for ann in image_id_to_annotations[image_id]:
                ...
                x_center_absolute = (
                    x_min_absolute + (x_min_absolute + box_w_absolute)
                ) / 2
                y_center_absolute = (
                    y_min_absolute + (y_min_absolute + box_h_absolute)
                ) / 2
