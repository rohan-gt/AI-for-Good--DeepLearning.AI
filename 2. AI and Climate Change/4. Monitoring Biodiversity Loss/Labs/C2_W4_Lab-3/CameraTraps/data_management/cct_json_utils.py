...
class CameraTrapJsonUtils:
    ...
    def annotations_to_string(annotations, cat_id_to_name):
        ...
        """
        class_names = CameraTrapJsonUtils.annotationsToClassnames(
    ...
    @staticmethod
    def get_entries_from_locations(
        db: JSONObject, locations: Iterable[str]
    ) -> Dict[str, Any]:
        """
        ...
        print(
            "Original DB has {} image and {} annotation entries.".format(
                len(db["images"]), len(db["annotations"])
            )
        )
        new_db: Dict[str, Any] = {"images": [], "annotations": []}
        new_images = set()
        for i in db["images"]:
...
class IndexedJsonDb:
    ...
    def __init__(
        self,
        json_filename: Union[str, JSONObject],
