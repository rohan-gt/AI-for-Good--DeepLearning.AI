...
print(
    "\nLoaded {} image records from {} .json files".format(
        len(input_images), len(json_files)
    )
)
    ...
    if url in urls:
        for existing_im in input_images:
            ...
            if url == existing_im["url"]:
                ...
                # Ellie_2016-2017-SC12.json
                assert im["json_filename"].replace("-", " ") == existing_im[
                    "json_filename"
                ].replace("-", " ")
                ...
                if im["Output"] is None:
                    print(
                        "Warning: duplicate URL {}, keeping existing output".format(url)
                    )
                    break
                ...
                if (existing_im["Output"] is None) and (im["Output"] is not None):
                    ...
                    obj2 = existing_im["Output"][0]["tags"]["Object"].replace(
                        "_partial", ""
                    )
                    if obj1 != obj2:
                        print(
                            "Warning: image {} tagged with {} and {}".format(
                                url, obj1, obj2
                            )
                        )
...
def process_exif(file_path):
    ...
    proc = subprocess.Popen(
        [exiftool_command_name, "-G", file_path],
        stdout=subprocess.PIPE,
        encoding="utf8",
    )
    exif_lines = proc.stdout.readlines()
    exif_lines = [s.strip() for s in exif_lines]
    assert exif_lines is not None and len(exif_lines) > 0, (
        "Failed to read EXIF data from {}".format(file_path)
    )
    ...
    for line_raw in exif_lines:
        ...
        if (
            (
                ("datetime original" in line)
                or ("create date" in line)
                or ("date/time created" in line)
                or ("date/time original" in line)
            )
            and ("[file]" not in line)
            and ("[composite]" not in line)
        ):
    ...
    if not found_makernotes:
        ...
        if not "datetime" in exif_tags:
            print(
                "Warning: could not find datetime information in {}".format(file_path)
            )
        ...
        assert "datetime" in exif_tags, (
            "Could not find datetime information in {}".format(file_path)
        )
        for field_name in makernotes_fields:
            assert field_name in exif_tags, "Could not find {} in {}".format(
...
for i_location, location in enumerate(locations):
    images_this_location = [
        im for im in images if im["exif_tags"]["location"] == location
    ]
    sorted_images_this_location = sorted(
        images_this_location, key=lambda im: im["exif_tags"]["datetime"]
    )
...
for i_location, location in enumerate(locations):
    ...
    for i_image, im in enumerate(sorted_images_this_location):
        ...
        output_relative_path = (
            ...
            + location
            + "/"
            + "{0:03d}".format(folder_number)
            + "/"
            + "{0:03d}".format(image_number)
            + ".jpg"
        )
...
if n_copy_threads == 1:
    ...
    copy_image_results = list(
        tqdm(pool.imap(copy_image_to_output, images), total=len(images))
    )
...
for i_image, input_im in tqdm(enumerate(all_image_info), total=len(all_image_info)):
    ...
    if (
        "location" not in input_im["exif_tags"]
        or input_im["exif_tags"]["location"] == "unknown"
    ):
        print("Warning: no location for image {}, skipping".format(input_im["url"]))
        continue
    ...
    output_im["temperature"] = (
        exif_tags["temperature"] if "temperature" in exif_tags else None
    )
    ...
    output_image_full_path = os.path.join(
        output_image_folder, input_im["output_relative_path"]
    )
    ...
    for i_ann, input_annotation in enumerate(input_annotations):
        ...
        if input_annotation["tags"]["Object"] is None:
            ...
            assert (
                len(input_annotation["tags"]) == 1
                and "Object" in input_annotation["tags"]
            )

            # Some annotators (but not all) included "_partial" when animals were partially obscured
            category_name = (
                input_annotation["tags"]["Object"]
                .replace("_partial", "")
                .lower()
                .strip()
            )
...
manual_human_ids = [
    "a07fc88a-6dd8-4d66-b552-d21d50fa39d0",
    "285363f9-d76d-4727-b530-a6bd401bb4c7",
]
...
for ann in tqdm(annotations):
    if ann["image_id"] in manual_human_ids:
        ...
        print(
            "Changing annotation for image {} from {} to {}".format(
