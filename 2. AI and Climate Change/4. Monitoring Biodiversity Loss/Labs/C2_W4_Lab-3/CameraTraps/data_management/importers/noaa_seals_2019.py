...
for i_row, row in tqdm(df.iterrows(), total=len(df)):
    ...
    rgb_image_path = "Images/{}/{}/{}".format(
        row["flight"], camera_view_to_path[row["camera_view"]], row["rgb_image_name"]
    )
    ...
    ir_image_path = "Images/{}/{}/{}".format(
        row["flight"], camera_view_to_path[row["camera_view"]], row["ir_image_name"]
    )
...
i_image = random.randint(0, len(df))
# i_image = 2004
row = df.iloc[i_image]
...
print(
    "Found {} RGB, {} IR annotations for this image".format(
...
for b in rgb_boxes:
    ...
    visualization_utils.draw_bounding_box_on_image(
        img_rgb, ymin, xmin, ymax, xmax, use_normalized_coordinates=False, thickness=3
    )
...
for b in ir_boxes:
    ...
    visualization_utils.draw_bounding_box_on_image(
        img_ir, ymin, xmin, ymax, xmax, use_normalized_coordinates=False, thickness=3
    )
