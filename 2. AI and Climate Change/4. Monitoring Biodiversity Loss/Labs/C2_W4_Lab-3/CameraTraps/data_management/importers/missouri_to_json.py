...
set1RedundantBoxListFilename = os.path.join(
    baseDir, "mct_images_with_redundant_boxes.txt"
)
...
for root, subdirs, files in os.walk(baseDir):
    ...
    for fname in files:
        ...
        if "Set1" in relativePath:
            ...
            raise Exception("Oops, cant assign this image to a set")
...
print("Finished file enumeration in {}".format(humanfriendly.format_timespan(elapsed)))
...
print(
    "Corrected {} paths, missing {} images of {}".format(
        len(correctedFiles), len(missingFilesSet1), len(metadataSet1Lines)
    )
)
...
if process_set2:
    ...
    for iLine, line in enumerate(metadataSet2Lines):
        ...
        assert (nTokens == 3 and tokens[1] == "0" and tokens[2] == "0") or (
            ((nTokens - 2) % 5) == 0
        )
        relPath = tokens[0].replace("/", os.sep).replace("\\", os.sep)
    ...
    print(
        "Missing {} of {} files in set 2".format(
            len(missingFilesSet2), len(metadataSet2Lines)
        )
    )
...
if process_set2:
    ...
    if True:
        ...
        print(
            "{} of {} files in set 2 lack labels".format(
                len(set2UnlabeledImageIndices), len(set2ImageIDs)
            )
        )

        set2ImageIDsLabeled = [
            x for i, x in enumerate(set2ImageIDs) if i not in set2UnlabeledImageIndices
        ]

        assert len(set2ImageIDsLabeled) + len(set2ImageIDsUnlabeled) == len(
            set2ImageIDs
        )
        ...
        if nBoxes == 0:
            ...
            for iBox in range(0, nBoxes):
                ...
                for iBoxComparison in range(0, iBox):
                    ...
                    boxCoordsComparison = imageMetadata[
                        2 + (iBoxComparison * 4) : 6 + (iBoxComparison * 4)
                    ]
...
assert (
    len(annotationsSet1) == nSequenceLevelAnnotations + nTotalBoxes + nImageLevelEmpties
)
...
if process_set2:
    ...
    sequenceClassCategoryMappingsSet2 = {
        "human": humanCat,
        "animal": animalCat,
        "mixed": unknownCat,
        "empty": emptyCat,
    ...
    print(
        "Finished processing set 2, found metadata for {} of {} images, created {} annotations and {} boxes in {} categories".format(
            nFoundMetadata,
            len(set2ImageIDs),
            len(annotationsSet2),
            nTotalBoxes,
    ...
    assert (
        len(annotationsSet2)
        == nSequenceLevelAnnotations + nTotalBoxes + nImageLevelEmpties
    )
...
sortedCategories, data, _ = sanity_check_json_db.sanity_check_json_db(
...
htmlOutputFile, _ = visualize_db.process_images(
    outputJsonFilenameSet1, output_dir, baseDir, options
)
