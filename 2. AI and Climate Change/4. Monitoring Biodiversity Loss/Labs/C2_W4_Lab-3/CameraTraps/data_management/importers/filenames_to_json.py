...
if bUseExternalRemappingTable:
    ...
    if os.path.isfile(rawClassListFilename):
        ...
        with io.open(
            rawClassListFilename, "w", encoding=outputEncoding
        ) as classListFileHandle:
            for cat in categories:
                catId = cat["id"]
                categoryName = cat["name"]
                categoryCount = cat["count"]
                classListFileHandle.write(
                    str(catId) + ',"' + categoryName + '",' + str(categoryCount) + "\n"
                )
...
print(
    "Mapped {} original classes to {} new classes".format(
        len(originalCategories), len(categories)
    )
)
