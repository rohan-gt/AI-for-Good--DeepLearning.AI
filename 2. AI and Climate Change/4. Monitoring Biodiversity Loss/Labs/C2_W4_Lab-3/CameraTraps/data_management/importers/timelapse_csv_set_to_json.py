...
print(
    "Found {} image files and {} .csv files ({} non-matching files)".format(
    ...
    try:
        ...
        if "invalid start byte" in str(e):
            try:
                ...
                print("Cant read file {}: {}".format(csv_filename, str(e)))
                bad_csv_files.append(csv_filename)
                continue

    if not (
        len(df.columns) == len(expected_columns)
        and (df.columns == expected_columns).all()
    ):
        ...
        if not (len(missing_fields) == 0 and len(extra_fields) == 0):
            print(
                "In file {}, extra fields {}, missing fields {}".format(
                    csv_filename, extra_fields, missing_fields
                )
            )
...
print(
    "Concatenated all .csv files into a dataframe with {} rows".format(
        len(input_metadata)
    )
)
...
for fn_original in valid_csv_files:
    ...
    if re_result is None:
        print("Couldnt match tokens in {}".format(csv_filename))
        continue
...
print(
    "Successfully mapped {} of {} csv files to camera folders".format(
        len(csv_filename_to_camera_folder), len(valid_csv_files)
    )
)
...
for camera_folder in csv_filename_to_camera_folder.values():
    for image_folder in all_image_folders:
        if image_folder.startswith(camera_folder):
            camera_folders_to_image_folders.setdefault(camera_folder, []).append(
                image_folder
            )
...
for i_row, row in tqdm(input_metadata.iterrows(), total=len(input_metadata)):
    ...
    image_id = (
        image_relative_path.replace("_", "~").replace("/", "_").replace("\\", "_")
    )
...
print(
    "Finished verifying file loop in {}, {} images, {} missing images, {} repeat labels, {} ambiguous labels".format(
        humanfriendly.format_timespan(elapsed),
        len(images),
        len(files_missing_from_file_list),
...
print(
    "Removed {} redundant unlabeled annotations".format(
        len(annotations) - len(valid_annotations)
    )
)
...
print(
    "Finished checking {} images to make sure they're in the metadata, found {} un-annotated images".format(
...
html_output_file, data = visualize_db.process_images(
    output_file, preview_base, file_base, options
)
