...
columns_to_copy = {
    "Total": "count",
    "Horses": "horses",
    "DogsOnLeash": "dogsonleash",
    "DogsOffLeash": "dogsoffleash",
    "AdultFemale": "adultfemale",
    "AdultMale": "adultmale",
    "AdultUnknown": "adultunknown",
    "Subadult": "subadult",
    "YLY": "yearling",
    "YOY": "youngofyear",
}
...
for iRow, row in tqdm(input_metadata.iterrows(), total=len(input_metadata)):
    ...
    if relative_path not in image_relative_paths:
        if relative_path.lower() in image_relative_paths_lower:
            print(
                "Warning: lower-case version of {} in path list".format(relative_path)
            )
        else:
...
print(
    "Finished verifying file loop in {}, {} matched images, {} missing images, {} unlabeled images".format(
...
print(
    "Of {} missing files, {} are due to {} missing folders".format(
...
for i_image, relative_path in tqdm(
    enumerate(image_relative_paths), total=len(image_relative_paths)
):
    if relative_path not in relative_path_to_image:
        unmatched_files.append(relative_path)

print(
    "Finished checking {} images to make sure they're in the metadata, found {} mismatches".format(
...
if db_sampling_scheme == "all":
    ...
    if db_sampling_scheme == "preview":
        ...
        print(
            "Sampling {} of {} unlabeled images".format(n_sample, len(unlabeled_images))
        )
    ...
    print(
        "Sampling {} of {} images, {} of {} annotations".format(
...
htmlOutputFile, _ = visualize_db.process_images(
    json_data, output_dir, input_base, options
)
