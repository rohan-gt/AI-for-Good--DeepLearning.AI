    ...
    for sequence in image_sequences:
        ...
        for researcher_id in researcher_identifications:
            ...
            for id in identifications:
                species_common_name = helpers.clean_species_name(
                    id.findtext("SpeciesCommonName")
                )
        ...
        assert (
            isinstance(seq_num_frames, int) and seq_num_frames > 0
        )  # historical quirk
        ...
        for img in images:
            ...
            img_datetime, datetime_err = helpers.parse_timestamp(
                img.findtext("ImageDateTime")
            )
            if datetime_err:
                print(
                    "WARNING datetime parsing error for image {}. Error: {}".format(
...
coco_formatted_json = {"info": db_info, "images": db_images}
