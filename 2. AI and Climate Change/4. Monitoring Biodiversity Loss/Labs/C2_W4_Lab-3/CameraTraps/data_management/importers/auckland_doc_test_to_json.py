...
def bird_name_to_category_name(bird_name):
    bird_name = (
        ...
        .strip()
        .replace(" ", "_")
        .replace("song_thursh", "song_thrush")
    )
    bird_name = (
        bird_name.replace("pippet", "pipt")
        .replace("pippit", "pipit")
        .replace("nz_pipit", "pipit")
    )
...
for i_row, row in tqdm(input_metadata.iterrows(), total=len(input_metadata)):
    ...
    if not np.isnan(row["Collared_cat"]):
        ...
        annotations_this_image.append(
            create_annotation(im["id"], "cat", row["Collared_cat"])
        )
    ...
    if not np.isnan(row["Bird"]):
        if isinstance(row["Bird_ID"], str):
            ...
            category_name = "unknown_bird"
        annotations_this_image.append(
            create_annotation(im["id"], category_name, row["Bird"])
        )
    ...
    for s in [
        ...
        if isinstance(row[s], float) or isinstance(row[s], int):
            if not np.isnan(row[s]):
                annotations_this_image.append(
                    create_annotation(im["id"], s.lower().replace("_", ""), row[s])
                )
...
print(
    "{} images handled successfully, {} total annotations".format(
        len(image_id_to_image), len(annotations)
    )
)
...
html_output_file, image_db = visualize_db.process_images(
    ...
    output_dir=os.path.join(output_base_dir, "preview"),
...
options.api_output_filename_replacements = {
    "2020.08.01_reformat\\Maukahuka_Auckland_Island\\2_Testing\\": ""
}
