...
def process_images(filenames, image_prefix=None, n_threads=default_n_threads):
    if n_threads <= 1:
        ...
        all_results = list(
            tqdm(
                pool.imap(partial(process_image, image_prefix=image_prefix), filenames),
                total=len(filenames),
            )
        )
...
def process_list_file(
    input_file, output_file, image_prefix=None, n_threads=default_n_threads
):
    ...
    all_results = process_images(
        filenames, image_prefix=image_prefix, n_threads=n_threads
    )
...
if False:
    ...
    process_list_file(
        relative_image_list_file, image_size_file, image_prefix=base_dir, n_threads=4
    )
...
def main():
    ...
    process_list_file(
        args.input_file, args.output_file, args.image_prefix, args.n_threads
    )
