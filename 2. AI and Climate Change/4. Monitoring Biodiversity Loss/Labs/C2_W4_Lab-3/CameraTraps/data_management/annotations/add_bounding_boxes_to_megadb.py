...
def add_annotations_to_sequences(
    annotations_dir: str, temp_sequences_dir: str, sequences_dir: str
):
    """
    ...
    for p in temp_megadb_files:
        ...
        dataset_name = (
            basename.split("_temp.")[0]
            if basename.endswith("_temp.json")
        ...
        with open(
            os.path.join(sequences_dir, f"{dataset_name}.json"), "w", encoding="utf-8"
        ) as f:
...
def main():
    ...
    add_annotations_to_sequences(
        args.annotations_dir, args.temp_sequences_dir, args.sequences_dir
    )
