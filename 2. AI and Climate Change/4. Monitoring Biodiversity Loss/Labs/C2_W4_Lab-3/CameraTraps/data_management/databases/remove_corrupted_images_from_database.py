...
def remove_corrupted_images_from_database(data, image_file_root):
    ...
    if N_THREADS == 1:
        ...
        print(
            "Checked image corruption in {}, found {} invalid images (of {})".format(
                humanfriendly.format_timespan(processingTime),
                len(bValid) - sum(bValid),
                len(bValid),
            )
        )
    ...
    data["annotations"] = [
        ann for ann in data["annotations"] if keep_im[ann["image_id"]]
    ]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Remove images from a .json file that cant be opened in TF"
    )
