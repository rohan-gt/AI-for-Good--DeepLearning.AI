...
def integrity_check_json_db(jsonFile, options=None):
    ...
    if isinstance(jsonFile, dict):
        ...
        print("Reading .json {} with base dir [{}]...".format(jsonFile, baseDir))
    ...
    if len(baseDir) > 0:
        assert os.path.isdir(baseDir), "Base directory {} does not exist".format(
            baseDir
        )
    ...
    for image in tqdm(images):
        ...
        if "height" in image:
            assert "width" in image, "Image with height but no width: {}".format(
                image["id"]
            )

        if "width" in image:
            assert "height" in image, "Image with width but no height: {}".format(
                image["id"]
            )

        if options.bRequireLocation:
            assert "location" in image, "No location available for: {}".format(
                image["id"]
            )
        ...
        assert not ("sequence_id" in image or "sequence" in image), (
            "Illegal sequence identifier"
        )
    ...
    if options.bCheckImageSizes or options.bCheckImageExistence:
        if len(baseDir) == 0:
            print(
                'Warning: checking image sizes without a base directory, assuming "."'
            )
        ...
        if options.nThreads is not None and options.nThreads > 1:
            ...
            results = tqdm(
                pool.imap(check_image_existence_and_size, images), total=len(images)
            )
        else:
            results = []
        ...
        for iImage, r in enumerate(results):
            if not r:
                validationErrors.append(
                    os.path.join(options.baseDir, images[iImage]["file_name"])
                )
    ...
    for ann in tqdm(annotations):
        ...
        assert isinstance(ann["category_id"], int), (
            "Illegal annotation category ID type"
        )
