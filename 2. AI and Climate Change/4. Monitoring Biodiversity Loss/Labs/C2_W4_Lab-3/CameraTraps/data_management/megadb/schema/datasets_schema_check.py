...
def datasets_schema_check(items_json):
    ...
    for dataset in items_json:
        ...
        expiry_date = datetime.strptime(
            fields["se"][0], "%Y-%m-%dT%H:%M:%SZ"
        )  # This is UTC ("Z" denotes Zulu time)

        if expiry_date <= today:
            print(
                "SAS token of dataset {} has expired!".format(dataset["dataset_name"])
            )

        if expiry_date <= today_plus_30_days:
            print(
                "SAS token of dataset {} has will expire in the next 30 days".format(
