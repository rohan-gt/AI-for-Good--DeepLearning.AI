    ...
    if "location" not in entry:
        ...
        if entry["location"] in split_lists[Splits.TRAIN]:  # checks membership in a set
            ...
            if dataset_name not in ["nacti", "wcs"]:
                print(
                    "Entry file {} in dataset {} has location not in predefined splits; moving to train".format(
...
def main():
    ...
    print(
        "Time spent on moving files: {}".format(humanfriendly.format_timespan(elapsed))
    )
    print(counter)
