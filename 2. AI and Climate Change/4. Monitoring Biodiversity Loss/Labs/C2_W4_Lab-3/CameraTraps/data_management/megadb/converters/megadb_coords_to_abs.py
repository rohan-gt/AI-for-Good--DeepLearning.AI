...
def _get_image_dims(storage_client, path_prefix, image_obj):
    ...
    try:
        ...
        print(
            "Exception getting width and height for {}. Exception: {}".format(
                file_name, e
            )
        )
        return image_obj
...
def get_image_dims(mega_db_seqs, dataset_name, datasets_table, n_cores):
    ...
    if n_cores:
        ...
        updated_im_objects = pool.map(
            partial(_get_image_dims, storage_client, path_prefix),
            images_to_get_dims_for,
        )
        ...
        for image_obj in tqdm(images_to_get_dims_for):
            updated_im_objects.append(
                get_image_dims(storage_client, path_prefix, image_obj)
            )
...
def main():
    ...
    parser.add_argument(
        "mega_db_seqs", help="A json containing a list of sequence objects"
    )
    parser.add_argument(
        "out_file", help="Path to store the resulting json to input to megadb_to_cct.py"
    )
    parser.add_argument(
        "--ncores",
        type=int,
    ...
    updated_seqs = get_image_dims(
        mega_db_entries, args.dataset_name, datasets_table, args.ncores
    )
