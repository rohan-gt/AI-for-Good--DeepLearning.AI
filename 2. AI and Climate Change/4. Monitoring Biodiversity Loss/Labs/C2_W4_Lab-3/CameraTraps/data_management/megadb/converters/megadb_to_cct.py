        ...
        for im in seq["images"]:
            ...
            for im_prop in im:
                if im_prop in ["file", "frame_num", "id", "file_name"]:
                    ...
                    im_prop == "class"
                ):  # image-level "species" labels; not the bbox type labels
                    if bbox_only:
                        continue
...
def megadb_to_cct(dataset_name, mega_db, output_path, bbox_only):
    ...
    assert len(mega_db) > 0, "There are no entries from the dataset {}".format(
        dataset_name
    )
    for i in mega_db:
        del i["dataset"]  # all remaining fields will be added to the CCT database
    print(
        "Number of entries belonging to dataset {}: {}".format(
            dataset_name, len(mega_db)
        )
    )
    ...
    for name, num_id in cat_name_to_id.items():
        cct_categories.append({"id": num_id, "name": name})

    print(
        "Final CCT DB has {} image entries, and {} annotation entries.".format(
            len(cct_images), len(cct_annotations)
        )
    )
    cct_db = {
        "info": {
    ...
    cct_db["info"]["description"] = (
        "COCO Camera Traps database converted from sequences in dataset {}".format(
            dataset_name
        )
    )
...
def main():
    ...
    parser.add_argument("out_cct_db", help="Path to store the resulting json")
    parser.add_argument(
        "--bbox_only",
        action="store_true",
