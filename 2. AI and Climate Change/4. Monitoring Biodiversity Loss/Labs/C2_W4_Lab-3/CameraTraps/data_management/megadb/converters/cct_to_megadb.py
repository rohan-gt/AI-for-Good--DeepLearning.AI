...
old_to_new_prop_name_mapping = {"file_name": "file"}
...
def process_sequences(embedded_image_objects, dataset_name, deepcopy_embedded=True):
    ...
    """
    print(
        "The dataset_name is set to {}. Please make sure this is correct!".format(
            dataset_name
        )
    )
    ...
    print("Putting {} images into sequences...".format(len(docs)))
    ...
    for im in tqdm(docs):
        if "seq_id" in im:
            ...
            seq_id = (
                "dummy_" + uuid.uuid4().hex
            )  # if this will be sent for annotation, may need a sequence ID based on file name to group potential sequences together
    ...
    for seq_id, images in sequences.items():
        new_sequences.append(
            {"seq_id": seq_id, "dataset": dataset_name, "images": images}
        )
    ...
    for seq in sequences:
        ...
        for im in seq["images"]:
            locations.append(
                im.get("location", "")
            )  # empty string if no location provided
        assert len(set(locations)) == 1, (
            "Location fields in images of the sequence {} are different.".format(
                seq["seq_id"]
            )
        )
    ...
    for seq in sequences:
        ...
        image_properties = defaultdict(
            set
        )  # property name to stringfied property value
    ...
    for prop_name, values in seq_level_prop_values.items():
        ...
        if len(values) == 1:
            ...
            print(
                "! Sequence-level property {} with value {} should be a dataset-level property. Removed from sequences.".format(
                    prop_name, list(values)[0]
                )
            )
...
def make_cct_embedded(image_db=None, bbox_db=None):
    ...
    if image_db:
        ...
        docs = (
            cct_json_db.image_id_to_image
        )  # each image entry is first assigned the image object
        ...
        print(
            "Number of images with more than 1 species: {} ({}% of image DB)".format(
                num_images_with_more_than_1_species,
                round(100 * num_images_with_more_than_1_species / len(docs), 2),
            )
        )
    ...
    if bbox_db:
        ...
        for (
            image_id,
            bbox_annotations,
        ) in cct_bbox_json_db.image_id_to_annotations.items():
            ...
            for bbox_anno in bbox_annotations:
                item_bbox = {
                    "category": cct_bbox_json_db.cat_id_to_name[
                        bbox_anno["category_id"]
                    ],
        ...
        print(
            "Number of images with more than one bounding box: {} ({}% of all entries)".format(
    ...
    assert len(docs) > 0, (
        "No image entries found in the image or bbox DB jsons provided."
    )
...
def main():
    ...
    parser.add_argument(
        "--image_db", help="Path to the json containing the image DB in CCT format"
    )
    parser.add_argument(
        "--bbox_db", help="Path to the json containing the bbox DB in CCT format"
    )
    parser.add_argument(
        "--docs", help="Embedded CCT format json to use instead of image_db or bbox_db"
    )
    parser.add_argument(
        "--partial_mega_db", required=True, help="Path to store the resulting json"
    )
    ...
    if args.image_db:
        assert os.path.exists(args.image_db), (
            "image_db file path provided does not point to a file"
        )
    if args.bbox_db:
        assert os.path.exists(args.bbox_db), (
