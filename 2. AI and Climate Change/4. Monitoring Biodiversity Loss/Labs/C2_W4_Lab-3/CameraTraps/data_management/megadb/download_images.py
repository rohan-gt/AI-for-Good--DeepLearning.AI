...
def download_file(
    url: str, filename: str, store_dir: str, check_existing_dir: Optional[str] = None
) -> None:
...
def construct_url(
    img_path: str, datasets_table: Mapping[str, Any], dataset_name: Optional[str] = None
) -> str:
...
def process_txt(
    file_list_path: str, existing: Container[str], datasets_table: Mapping[str, Any]
) -> Tuple[Dict[str, str], int]:
    """Processes text file.
    ...
    with open(file_list_path, "r") as f:
        for filename in f:
            ...
            try:
                url = construct_url(img_path=filename, datasets_table=datasets_table)
                filename_to_url[filename] = url
            except Exception as e:  # pylint: disable=broad-except
                exception_type = type(e).__name__
                print(f"construct_url() generated {exception_type}: {e}")
...
def main(
    ...
    print(
        f"file_list has {count} items, still need to download "
    ...
    for filename, url in tqdm(filename_to_url.items()):
        future = pool.submit(
            download_file, url, filename, store_dir, check_existing_dir
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "file_list_json", help="path to JSON file containing list of image entries"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--save-key",
        default="download_id",
        help="(type=json) key inside each image entry corresponding to image save path",
    )
    parser.add_argument(
        "-k",
        "--key",
