...
def get_probabilities_for_img_array(
    img_array: np.ndarray, model: tf.keras.Model, label2cat: Dict[int, str]
...
def draw_bounding_box(
    ...
    detections = megadetector.generate_detections_one_image(
        image, sample_im_file, detection_threshold=0.4
    )
    ...
    if len(cropped_images) > 0:
        ...
        for k in range(len(detections["detections"])):
            ...
            if detection["category"] == "1":
                ...
                preds = get_probabilities_for_img_array(
                    np.array([cropped_images[k]]), model, label2cat
                ).values
                ...
                img1.text(
                    (x1, y1 - interlines * 3 + offset),
                    str(round(preds[0, 1], 2)) + " : " + preds[0, 0],
                    fill=(0, 255, 0, 255),
                    font=FONT,
                )
                img1.text(
                    (x1, y1 - interlines * 2 + offset),
                    str(round(preds[1, 1], 2)) + " : " + preds[1, 0],
                    fill=(255, 165, 0),
                    font=FONT,
                )
                img1.text(
                    (x1, y1 - interlines + offset),
                    str(round(preds[2, 1], 2)) + " : " + preds[2, 0],
                    fill=(255, 80, 10),
                    font=FONT,
                )
            else:
                ...
                img1.text(
                    (x1, y1 - interlines + offset),
                    md_categories[detection["category"]],
                    fill=(0, 255, 0, 255),
                    font=FONT,
                )
