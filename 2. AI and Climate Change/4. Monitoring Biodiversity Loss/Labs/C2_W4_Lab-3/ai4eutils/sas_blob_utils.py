...
def check_blob_exists(sas_uri: str, blob_name: Optional[str] = None) -> bool:
    ...
    if blob_name is not None:
        sas_uri = build_blob_uri(container_uri=sas_uri, blob_name=blob_name)
...
def list_blobs_in_container(
    ...
    if (
        get_sas_token_from_uri(container_uri) is not None
    ...
    with get_client_from_uri(container_uri) as container_client:
        ...
        if blob_suffix is None and rsearch is None:
            ...
            for blob in tqdm(generator, disable=(not verbose)):
                i += 1
                suffix_ok = blob_suffix is None or blob.name.lower().endswith(
                    blob_suffix
                )
                regex_ok = False
                if rsearch is None:
