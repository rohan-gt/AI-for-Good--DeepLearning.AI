...
class CustomLogging:
    def __init__(self):
        ...
        self.blob_container_info_log = (
            "logs/info_logs/blob_container_and_blob_sizes _" + date_part + ".log"
        )
        ...
        self.debug_logger = self.setup_logger(
            "info_log", self.debug_log, self.logformatter, level=logging.DEBUG
        )
        self.storage_info_logger = self.setup_logger(
            "storage_size_log", self.storage_info_log, self.logformatter
        )
        self.blob_container_info_logger = self.setup_logger(
            "blob_container_size_log", self.blob_container_info_log, self.logformatter
        )

        self.csv_storage_size_headers = (
            "entry type, storage account name, size, size (friendly)"
        )
        self.csv_blob_container_size_headers = (
            "entry type, entity name, size, size (friendly)"
        )
