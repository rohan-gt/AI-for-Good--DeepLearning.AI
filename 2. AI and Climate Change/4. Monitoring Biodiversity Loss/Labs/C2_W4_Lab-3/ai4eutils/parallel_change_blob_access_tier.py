...
def get_container_client(account_name, container_name, sas_token_file):
    ...
    blob_service_client = BlobServiceClient(
        account_url=storage_account_url_blob, credential=rw_sas_token
    )
    ...
    if verify_access_tier:
        try:
            ...
            assert tier is not None, (
                "Error retrieving blob tier; is this a GPv1 storage account?"
            )
            ...
            if tier == access_tier:
                ...
                force_tier = (
                    force_tier_on_inferred_blobs and properties["tier_inferred"]
                )
                if not force_tier:
                    print(
                        "Skipping {}, already at tier {}".format(blob_path, access_tier)
                    )
                    return

        except Exception as e:
            print("Error verifying access tier for {}: {}".format(blob_path, str(e)))
    ...
    try:
        ...
        if verbose:
            ...
            if "BlobNotFound" in s:
                ...
                print("Error setting {} to {}: {}".format(blob_path, access_tier, s))
...
if False:
    ...
    container_client = get_container_client(
        account_name, container_name, sas_token_file
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_file", help="List of blobs to change")
    parser.add_argument(
        "access_tier",
    ...
    assert os.path.isfile(args.input_file), "Could not find file {}".format(
        args.iput_file
    )

    assert args.access_tier in valid_tiers, (
        "Invalid access tier: {}, valid values are Hot, Cool, Archive (case-sensitive)".format(
            args.access_tier
        )
    )
    container_client = get_container_client(
        args.account_name, args.container_name, args.sas_token_file
    )
