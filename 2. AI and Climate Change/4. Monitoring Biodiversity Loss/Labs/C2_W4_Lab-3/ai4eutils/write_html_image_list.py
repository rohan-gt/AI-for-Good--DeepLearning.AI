...
def write_html_image_list(filename=None, images=None, options={}):
    ...
    if "defaultImageStyle" not in options:
        options["defaultImageStyle"] = "margin:0px;margin-top:5px;margin-bottom:5px;"
    ...
    if nImages > options["maxFiguresPerHtmlFile"]:
        ...
        if options["fHtml"] != -1:
            raise ValueError(
                "You cant supply your own file handle if we have to page the image set"
            )
