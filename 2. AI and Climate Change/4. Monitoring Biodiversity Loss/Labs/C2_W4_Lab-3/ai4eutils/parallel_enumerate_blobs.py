...
def read_prefix_list(prefix_list_file):
    ...
    print("Read {} prefixes from {}".format(len(prefixes), prefix_list_file))
    return prefixes
...
class Counter(object):
    ...
    def increment(self, n=1):
        ...
        if b_print:
            ...
            print(
                "{}: iteration {}{}".format(
                    time.strftime("%Y-%m-%d %H:%M:%S"), self.val.value, total_string
                ),
                flush=True,
            )
...
def enumerate_prefix(
    prefix, sas_url, output_folder, get_sizes=False, get_access_tiers=False
):
    ...
    with open(output_file, "w") as output_f:
        ...
        while (continuation_token is not None) and (not hit_debug_limit):
            blobs_iter = container_client.list_blobs(
                name_starts_with=prefix, results_per_page=n_blobs_per_page
...
def enumerate_blobs_threads(
    prefixes, sas_url, output_folder, get_sizes=False, get_access_tiers=False
):
    all_threads = []
...
def enumerate_blobs(
    prefix_list_file, sas_url, output_folder, get_sizes=False, get_access_tiers=False
):
    ...
    if use_threads:
        enumerate_blobs_threads(
            prefixes, sas_url, output_folder, get_sizes, get_access_tiers
        )
    else:
        enumerate_blobs_processes(
            prefixes, sas_url, output_folder, get_sizes, get_access_tiers
        )
...
if False:
    ...
    if False:
        ...
        prefix = prefixes[0]
    enumerate_blobs(
        prefix_list_file, sas_url, output_folder, get_sizes, get_access_tiers
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "prefix_list_file", help="Text file containing one prefix per line"
    )
    parser.add_argument(
        "sas_url",
    ...
    enumerate_blobs(
        args.prefix_list_file,
        args.sas_url,
        args.output_folder,
        args.get_sizes,
        args.get_access_tiers,
    )
...
def parse_enumeration_folder(folder_name):
    ...
    for fn in enumeration_files:
        filename_to_size.update(
            parse_filenames_and_sizes(os.path.join(folder_name, fn))
        )
    return filename_to_size
...
def summarize_enumeration_folder(folder_name):
    ...
    print(
        "Read {} files totaling {}".format(
            total_files, humanfriendly.format_size(total_size)
        )
    )
