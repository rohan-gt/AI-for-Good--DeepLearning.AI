...
def download_url(
    ...
    if destination_filename is None:
        ...
        if len(url_as_filename) + n_folder_chars > max_path_len:
            print(
                "Warning: truncating filename target to {} characters".format(
                    max_path_len
                )
            )
            url_as_filename = url_as_filename[-1 * (max_path_len - n_folder_chars) :]
        destination_filename = os.path.join(target_folder, url_as_filename)

    if (not force_download) and (os.path.isfile(destination_filename)):
        if verbose:
            print(
                "Bypassing download of already-downloaded file {}".format(
                    os.path.basename(url_no_sas)
                )
            )
    else:
        if verbose:
            print(
                "Downloading file {} to {}".format(
                    os.path.basename(url_no_sas), destination_filename
                ),
                end="",
            )
