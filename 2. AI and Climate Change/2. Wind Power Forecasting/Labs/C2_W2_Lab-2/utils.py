...
def plot_wind_speed_vs_power(
    ax: plt.Axes, x1: Iterable, y1: Iterable, x2: Iterable, y2: Iterable
):
    ...
    ax.scatter(x1, y1, color="blue", edgecolors="white", s=15, label="actual")
    # Plot the model
    ax.scatter(
        x2, y2, color="orange", edgecolors="black", s=15, marker="D", label="model"
    )
...
def plot_predicted_vs_real(
    ...
    ax.scatter(
        ...
        color="orange",
        edgecolors="black",
        label="Predicted vs. actual values",
        marker="D",
    )
    # Plot straight line
    ax.plot(
...
def fit_and_plot_linear_model(
    data_og: pd.core.frame.DataFrame, turbine: int, features: List[str]
):
...
def neural_network(data_og: pd.core.frame.DataFrame, features: List[str]):
    ...
    def fit_nn(turbine, features):
        ...
        if "Wspd" in features:
            ...
            plot_wind_speed_vs_power(ax1, X_plot, Y_real, None, None)
            print(
                "The model could not be plotted here as Wspd is not among the features"
            )

        plot_predicted_vs_real(
            ax2, y_test_denormalized, test_preds_denormalized, X_eq_Y, X_eq_Y
        )
