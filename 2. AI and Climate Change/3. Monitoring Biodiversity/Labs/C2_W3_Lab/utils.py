...
def get_metadata(data_folder: str) -> pd.core.frame.DataFrame:
    ...
    all_paths = [
        y for x in os.walk(data_folder) for y in glob(os.path.join(x[0], "*.JPG"))
    ]
...
def plot_bar_chart(meta_data: pd.core.frame.DataFrame):
    ...
    cross = pd.crosstab(
        index=meta_data["location"], columns=meta_data["class"], normalize="index"
    )
    cross.plot(kind="bar", stacked=True, figsize=(11, 6), fontsize=12, cmap=cmap)
