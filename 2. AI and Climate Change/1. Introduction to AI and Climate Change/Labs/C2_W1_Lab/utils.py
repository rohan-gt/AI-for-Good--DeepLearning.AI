...
def bar_global(data: pd.core.frame.DataFrame) -> None:
    ...
    clrs = [
        "blue" if (x < 0) else "red" for x in values
    ]  # colors for positive and negative differences
    ...
    plt.title(
        "Station network temperature difference \nwith respect to 1850-1900 average",
        fontsize=12,
    )
...
def slider_global_temp() -> None:
    ...
    slider = widgets.IntSlider(min=1884, max=2020, layout=widgets.Layout(width="95%"))
    widgets.interactive(update, Year=slider)

    layout = widgets.Layout(flex_flow="column", align_items="center", width="700px")
